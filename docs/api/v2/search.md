...
在服务端 `search_log` 表 + `logs/search.jsonl` 追加一条，**存的是完整的响应体**——请求字段（`search_id` / `query` / `where` / `top_k` / `created_at`）和返回给调用方的全部结果（`cards` / `sessions` 两支，含 `snippets` / `score` / `summary` / `tags` / `links` 等）。用意是事后审计能复原"当时呈现给使用者的完整画面"，哪怕后续索引或对象被改也对得回。
