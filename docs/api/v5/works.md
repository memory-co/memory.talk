...
card → work 的接口:开工时注入的目录文本(`GET /api/collect/{layer}/recall` 的别名)。`text/plain`。
...
收件箱:被 `manager.json` 路由过来的变动——Collect 里这个 work 管的那一片的每次提交,以及子 work(或 `manager.json` 指过来的 work)的创建 / 状态变化。append-only。
