...
认知层。层 = 一份 YAML 协议(内置 origin / issue / card;用户层是 `~/.memory.talk/layers/*.yaml`),一个引擎按它校验;对象 = 带后缀的目录 `<path>.<层>/`,里面一组文件,放在树的任何位置,标题就是目录名;origin = 不带后缀的文件。一次写 = 对一个对象目录的一批文件改动,交给层的 check(不过 → 422 `invalid`,`message` 是理由),过了一个 `[层名]` 提交,落在 `layer/<层>` 上再 merge 进 `stack`;碰了别的层的路径被守卫拒绝(409 `guard`)。机制见 [designs metas.md](../../designs/v5/metas/README.md) / [metas-layer.md](../../designs/v5/metas/layer.md) / [manager.md](../../designs/v5/metas/manager.md)。
...
最底在前。`protocol` 就是这一层的 YAML(见 [metas-layer.md](../../designs/v5/metas/layer.md)),前端据此画表单:`object` 是对象目录的规则(名字正则、`name` 那段叫什么、能放在哪),`files` 每种文件的正则、`example`(`{name}` 是用户起的那段)、`fixed`(固定文件至多一个)、`required`、`format.fields`(frontmatter 字段:`string` `text` `number` `bool` `date` `enum` `ref` `list` `object`)、`format.body`、`template`。**没有加层的端点**:用户层是 `~/.memory.talk/layers/<名>.yaml`,重启时载入、登记进 `metas.json`(一次 `[origin]` 提交);`builtin: false`。
...
{"config": {"version": 1, "layers": [{"name": "origin", "builtin": true}, {"name": "issue", "builtin": true}, {"name": "card", "builtin": true},
 ...
 "history": [{"sha": "…", "author": "alice", "date": "…", "subject": "[origin] metas: add layer decision", "body": "Reason: …"},
             {"sha": "…", "subject": "[origin] metas: init", …}]}
```

### GET /api/metas/tree?path=&layer=&recursive=&candidate=

浏览的唯一接口。`items` 是这里有什么(对象折成一项,`path` 不含后缀;普通目录;origin 文件;`manager.json` / `metas.json` 不列);`layer=` 只留那一层的对象 / 文件(目录保留,可以继续往下走);`recursive=1` 往下走到底,`items` 拍平(只有对象和文件,`path` 是全路径)——「某一层的全部对象」就是 `?layer=card&recursive=1`;`can_create` 是这里还能建什么,`candidate=` 问一个名字行不行。
...
`path` 是对象目录(或它的子目录)时,它还是一个文件系统:`items` 列目录里的文件(`kind: file`,`layer` 是对象那一层,多带 `object`(对象 path)和 `rel`(目录内相对路径,如 `positions/只用环境变量.md`))和子目录;`layer` 是那一层,`can_create.objects` 为空(对象不嵌套),`can_create.files` 按这一层的文件种类逐种回答:`layer` / `pattern` / `example` / `label` / `fixed` / `format` / `existing`(已有的)/ `can` / `reason`(固定文件已存在);子目录只列能落到这里的种类。此时 `candidate` 问的是对象目录内的文件路径:匹配哪一种、存不存在、能不能建。

### GET /api/metas/recent?layer=&path=&limit=20&before=
...
`files` 是那次提交里这个对象动了哪些文件(origin 为空)。`layer=` 只看一层,`path=` 只看某个目录之下。`limit`(1–200)条一页,`next` 是下一页的游标(不透明字符串)——再请求时原样带上 `before=<next>`;`null` = 到底了。去重是全局的:翻到后面的页不会再看到前面出现过的对象。机制文件(`metas.json` / `manager.json`)的提交不算。

---

## 对象

### POST /api/metas/{layer}/{path}
...
### DELETE /api/metas/{layer}/{path}?reason=

删对象(整个目录)/ origin 文件。**200**,`data: null`。历史在 git。

### GET /api/metas/history/{layer}/{path}
