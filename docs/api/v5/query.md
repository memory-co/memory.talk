...
- **`library` 定库,完全隔离**:一次查询只见一个库(不 ATTACH、不跨库 join)。mind → reality 的关联靠 `(type, ref, indexes)` **指针两步解析**(先查 proofs,再按 type 去 reality / 文件取原文)。CLI 的 [mind](../../cli/v5/mind.md) / [reality](../../cli/v5/reality.md) 命令各对一个取值;
