...
CLI 对应 [`card review`](../../cli/v4/card.md#card-review) 命令。字段语义详见 [`../../structure/v4/review.md`](../../structure/v4/review.md)。
...
`card_id` + `position` 是路径回显（方便对账，寻址 `card_01jz8k2m#p1`）。
...
review **没有独立读取入口** —— 不存在 `GET /v4/.../reviews/{id}`。想看某 Position 的表态走 [`POST /v4/read`](read.md) `{id: "card_xxx#p1"}`（按 `#p` 分片判型），响应里带这个 Position 的 reviews（按 `created_at` 倒序）。
