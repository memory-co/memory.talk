...
后端 watcher 是 settings-driven 的。**开关** = `settings.sync.enabled`(由 `memory.talk setup` 写,server 启动时 lifespan 读)。API 上只剩**一个端点**:`GET /v4/sync/status`,纯查询。

CLI 对应单命令 [`memory.talk sync`](../../cli/v4/sync.md)。

> **v4 唯一变化**:`/v3/sync*` 路由挪到 **`/v4/sync*`** 前缀,watcher 机制、debounce、endpoints 配置、请求 / 响应体均不变。`settings.json` 是 single source of truth(没有 `start` / `stop` 端点)。
...
| `endpoints[]` | 0.7.x 字段。每个 `(source, location)` 一行,含可达性 (`ok` / `reason`) |
...
| `totals_by_endpoint` | 0.7.x 字段。把 `totals` 拆到每个 `<source>@<label>` 上,方便 CLI 一行一行渲染 |
| `last_event_at` | 最近一次事件时间,无事件则 null |
| `recent[]` | 最近 N 条事件(按时间倒序);**内存 ring buffer**,容量 20,server 重启清空。`endpoint` 字段携带 `<source>@<label>`,以及 `index_partial` / `index_failed` 事件带 `indexed` + `index_failed` 字段 |
...
| API 入口 | `POST /v4/sync/start/stop` + `GET /v4/sync/status` | **只有** `GET /v4/sync/status` |
| 内部 ingest 路径 | whole-session shim | service 层 `ensure_session` + `append_rounds` 直接 in-process 调用,游标在 sync 自己的 sync.db |
