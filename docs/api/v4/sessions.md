...
Round / ContentBlock 结构见 [`../../structure/v4/session.md`](../../structure/v4/session.md)。`round.index` 由服务端按 append 顺序赋值,调用方不传。
...
List session 元数据(**不带 rounds**)。CLI 入口 [`memory.talk session list`](../../cli/v4/session.md#session-list)。
...
## PATCH /v4/sessions/{session_id}/tags

设 / 删 session 的 kv 标签,**PATCH 语义**(只动声明的 key,不传的 key 原样保留)。CLI 入口 [`memory.talk session tag`](../../cli/v4/session.md#session-tag)。
...
- **外部**: 任何想从外部来源往 memory.talk 灌 session 的程序(CI runner / 自定义脚本 / 将来 sync 服务变 remote 后的 client)走这两个 HTTP 路由。

## cursor 协议要点

| | 旧 (whole-session shim) | 现行 (cursor) |
|---|---|---|
| 入口 | 单个 whole-session POST | `POST /v4/sessions/ensure` + `POST /v4/sessions/append` |
...
协议把"上游连接器状态"和"持久化数据状态"切干净 —— ingest 只懂 append,sync 全权负责"哪些是新的"。
