...
|---|---|---|
| `type` | 是 | 边类型,见下表 |
| `target_id` | 是 | 对端 id:多为 `card_<...>`;`suggested_by` 可指 `pos_<...>`。服务端据前缀派生并落 `target_type`(`card` / `position`)列,便于按对端类型过滤 |
...
```json
{"status": "ok", "card_id": "card_01jz8k2m", "type": "specializes", "target_id": "card_01jzaaaa", "target_type": "card"}
```

### 副作用

- 校验路径 `card_id` 存在、`type` 在白名单、`target_id` 前缀合法(`card_` 或 `pos_`)→ 失败不落库;据前缀落 `target_type`(`card` / `position`)列。
