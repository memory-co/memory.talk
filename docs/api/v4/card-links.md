...
| 字段 | 必填 | 说明 |
|---|---|---|
| `card_id` | 是 | **主体卡**（这条边是谁的），`card_<...>`。边是非对称 from/to，主体在前 |
| `type` | 是 | 边类型，见下表 |
...
- **五类型全多值**：同一 `(card_id, type)` 下可挂多条（如 `specializes` 多父）。主键是 `(card_id, type, target_id)`。
...
- 校验 `card_id` 存在、`type` 在白名单、`target_id` 前缀合法（`card_` 或 `pos_`）→ 失败不落库。
