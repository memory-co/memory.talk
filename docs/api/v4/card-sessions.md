...
CLI 一般不直接调——出处由**旁白 / 标注机制**自动写入(也可在 [`POST /v4/cards/{card_id}/positions`](cards.md#post-v4cardscard_idpositions) 带 `source`、或 [session-annotation](../../designs/v4/session-annotation.md))。本页端点用于显式记录和反查。字段语义详见 [`../../structure/v4/card-session.md`](../../structure/v4/card-session.md)。
