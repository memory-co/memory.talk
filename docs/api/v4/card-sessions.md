...
| `position` | 否 | 启发了哪个答案;本卡内序号 `p<n>`(全址 `{card_id}#{position}`)。空 / `""` = 关联到**问题 / 卡本身**(不指向具体答案) |
...
主键 `(card_id, session_id, position)`:同一卡可挂多个 session;同一 session 也可启发同卡的多个答案。重复 → 幂等。

### 响应

```json
{"status": "ok", "card_id": "card_01jz8k2m", "session_id": "sess_abc123", "position": "p1"}
```
...
| `position` 给了但不是本卡的 `p<n>` | 400, `invalid position` |
...
```json
{
  ...
  "sessions": [
    {"session_id": "sess_abc", "position": "p1", "indexes": "11-15", "created_at": "2026-05-24T09:12:03Z"},
    {"session_id": "sess_def", "position": "",   "indexes": "30-31", "created_at": "2026-05-24T09:40:00Z"}
  ]
}
```
...
```json
{
  "session_id": "sess_abc123",
  "cards": [
    {"card_id": "card_01jz8k2m", "position": "p1", "indexes": "11-15", "created_at": "2026-05-24T09:12:03Z"}
  ]
}
```
