...
v4 只重做**卡子系统**（被治理的问题图）：一张卡 = **一个问题（Issue）+ 它的若干答案（Position）**，靠对 Position 的 review 顶/踩竞争出"当下用哪个答案"。其余面（search / read / sessions / sync / status / embedding）**沿用 v3**，不在本目录重复。
...
Cards        POST   /v4/cards                               建卡（一个 issue，可带首个 Position）
...
> 读单卡 / 单 Position / session 走沿用 v3 的 `POST /v3/read`（按 id 前缀判型，`card_` / `pos_` / `sess_` 都认）。检索走 v3 `POST /v3/search`。本目录不重列。
