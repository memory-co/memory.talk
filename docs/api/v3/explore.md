...
CLI 对应 [`explore`](../../cli/v3/explore.md) 的 `pending` / `list` / `detail` 子命令。`auto` / `manual` / `resume` 是本地进程控制,不走 HTTP API。

## GET /v3/explore/pending
...
**聚焦本次抽取产出** —— 不展开 round 内容。要看对话原文走 `POST /v3/read {id: "sess_01k..."}`。
