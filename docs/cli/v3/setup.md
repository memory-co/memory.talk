...
**纯交互式**的幂等安装 / 配置 / 重启。第一次跑是 wizard,引导用户初始化 settings.json + 启动服务;后续再跑就是"修改配置 + 必要时重启服务"。同一个命令覆盖两个场景,用户不需要记两套姿势。
...
setup 命令**无参数** —— 所有配置都通过 wizard 内交互式 prompt 收集,不接受命令行参数。

## data root

v3 的**数据根目录固定在 `~/.memory.talk`** —— 所有命令(setup / sync / search / read 等)都用这个路径,**不暴露 `--data-root` 标志**。这是 v3 的有意简化:

- 90% 用户的数据都落在 `~/.memory.talk`,无需配置
- 多 data root 场景(同机多副记忆)极罕见;真有需要 → 跑两套 user account 各自配,比改 CLI 参数干净
- 后续若加多 data root 配置,**只在 setup 里加** —— 其它命令不动,通过 settings 或环境变量自动读取

setup 进来时,会**就地确认** `~/.memory.talk` 路径并继续 —— 不询问用户改路径(因为本来就不该改)。**`setup` 不支持 `--json`** —— 它的核心价值是逐项交互式确认,脚本化场景请直接写 `settings.json` + 调 `server start`。

## 行为分支

setup 进来先看 `~/.memory.talk/settings.json` 是否存在,据此分两种走法。
...
2. **安装模式**(detect → 必要时询问):
   - **标准模式(推荐)**:在 `~/.memory.talk/.venv/` 下创建独立 venv,把 `memory.talk` 装到里面,跟系统 / 全局 Python 完全隔离。后续所有命令通过软链(下面第 10 步)走这个 venv,不会被系统级 `pip` 或别人改的 Python 包污染。
   - **使用当前**:沿用当前 `memory.talk` 所在 Python 环境(可能是系统 Python、`pip --user`、用户自己的 venv……)。适合"我自己管 Python 环境"的开发者。
   
   **检测规则**:`shutil.which("memory.talk")` 拿到的脚本路径如果是 `~/.memory.talk/.venv/bin/memory.talk`,**已经在标准模式,不询问**,跳过此步。否则提示:
   
   ```
   检测到当前 memory.talk 在: /usr/local/bin/memory.talk
   推荐切换到标准模式(在 ~/.memory.talk/.venv 装独立 memory.talk),避免环境耦合。
   选择: [1] 标准模式 (推荐)  [2] 使用当前
   ```
   
   选 [1] 后 setup 会:
   - `python3 -m venv ~/.memory.talk/.venv` 建 venv
   - `~/.memory.talk/.venv/bin/pip install memory.talk` 安装
   - 后面第 11 步的软链**指向** `~/.memory.talk/.venv/bin/memory.talk`,而不是当前 PATH 上的那个
...
4. 根据选择填细节:
   - `openai` → endpoint / `auth_key` / `model` / `dim`,**实时打 HTTP 探针**验证 endpoint + key + dim
...
9. `ensure_dirs()` 把 `sessions/ cards/ vectors/ logs/search/` 全部创建出来
10. 询问"现在启动 server 吗?",`yes` 则等价于跑一次 `server start`,`no` 则给用户**复制粘贴的下一步命令**
11. **创建 `memory.talk` 软链**:setup 最后一步,在 `memory.talk` 脚本所在目录(标准模式下是 `~/.memory.talk/.venv/bin/`,使用当前模式下是检测到的安装目录)给 `memory.talk` 创建一个 `memory.talk` 同目录软链,这样用户输 `memory.talk read ...` 跟输 `memory.talk read ...` 完全等价。详见 [软链一节](#命令别名-memorytalk-软链)。
...
3. **embedding dim 改了** → 现有 card 的向量索引是旧 dim,**所有 hybrid 搜索都会失败**。setup 会**就地重算**所有 card 的 embedding(阻塞执行,进度走 stderr),完成后摘要里报告"重算 N 张 card"。中途 `Ctrl-C` 会停在已重算的部分;下次 setup 再跑时继续。
4. **server 在跑** + 任何字段变了 → 设置文件被 server 在 lifespan 启动时读过一次,运行时不重读;必须**重启 server** 才生效。提示 + 询问是否立即重启(`server restart`)。
5. **server 在跑** + 没字段变 → 不动 server。

## 幂等保证

- `settings.json` 写入是 **atomic**(tmp 文件 → rename),Ctrl-C 不会留下半个 JSON
- 已存在的目录 / 文件 / 数据**完全不动**;sessions / cards 不会被 setup 触碰
...
| alias | `~/.memory.talk/.venv/bin/memory.talk → memory.talk` |
| changed | settings.json (4 fields), server restarted |
````

约定:
- `changed` 那一行说明本次跑实际**做了什么**:
  - 首次安装:`changed | settings.json (created), dirs (sessions/, cards/, vectors/, logs/), server started`
...
```bash
memory.talk read card_xxx       # 短横线版,Python 包惯例
memory.talk read card_xxx       # 点号版,跟项目品牌名一致
```

### 如何定位脚本目录

- **标准模式**:软链放在 `~/.memory.talk/.venv/bin/memory.talk`,指向同目录的 `memory.talk`。**用户的 PATH 里需要有 `~/.memory.talk/.venv/bin/`** —— setup 跑完会提示用户(若不在 PATH)在 shell rc 里加这一行:
  ```bash
...
- 写 / 改 `~/.memory.talk/settings.json`(atomic)
- 创建数据目录(只补缺失的,已有不动)
- (可选, 标准模式)在 `~/.memory.talk/.venv` 创建 venv + `pip install memory.talk`
...
- **embedding dim 变了**:就地重算所有 card 的 embedding,写回向量库(原向量被覆盖)。
- **不动 sessions / cards / events.jsonl / search_log** —— 数据是用户的,setup 不擦除(重算 embedding 只动向量库这一列,card 的其它字段保持原样)

## 错误

| 情况 | 行为 |
|---|---|
| `~/.memory.talk` 是文件不是目录 | 报错退出,exit 1 |
...
- **embedding 重算**内嵌在 setup 里(仅 dim 改变时触发),没有独立 CLI 入口 —— 这一步逻辑上"跟着 setup 走",拆出去既很少手动跑、又容易跟 setup 状态打架。
