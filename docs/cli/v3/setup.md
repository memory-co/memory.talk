...
11. **创建 `memory.talk` 软链**:setup 最后一步,在 `memory-talk` 脚本所在目录(标准模式下是 `<data_root>/.venv/bin/`,使用当前模式下是检测到的安装目录)给 `memory-talk` 创建一个 `memory.talk` 同目录软链,这样用户输 `memory.talk read ...` 跟输 `memory-talk read ...` 完全等价。详见 [软链一节](#命令别名-memorytalk-软链)。
