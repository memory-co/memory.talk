...
- 输出整段是一个 `bash` 代码块,目的:
  ...
  2. 每条命中**直接以可执行命令形式呈现** —— `memory-talk read <card_id>  # <insight>`。LLM 只要识别到"这张卡可能有用",对应的展开命令就在它眼前,可以直接复制执行(harness 通常允许 LLM 调 shell 工具)。
  3. insight 写在 `# ` 注释里,语义上跟 bash 习惯吻合 —— 注释 = 不影响执行的元信息。
