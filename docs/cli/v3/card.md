...
3. **list 只接结构性 / metadata 过滤,不接 stats**。"按 review_up 找 / 找 shadow card / 找争议 card" 是论坛动力学的查询,继续走 [`search "" -w '...'`](../../designs/v3/dsl.md)。`card list` 的位置是:**按物理属性(tag / 创建时间)找子集做批量维护**,不跟 search 的动力学查询重叠。
...
> 不接 stats 过滤(`review_up >= N`、`read_count` 等)—— 这类查询走 [`search "" -w 'DSL'`](../../designs/v3/dsl.md),不在 `card list` 重复实现。
>
