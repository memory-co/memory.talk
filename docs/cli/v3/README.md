...
├── server start | stop | restart | status      # 本地 API 服务(沿用 v2 契约,后续补 v3 文档)
├── sync start | stop | status                  # 后端 watchdog,实时把 Claude Code session 落到 backend
├── card create | list | tag                    # 写一张帖子 / 列 card / 给 card 打 kv 标签
...
| `card create` | 0.7.x 曾经收编为 `card '<json>'`(取消 `create` 子命令);**0.8.x 起又改回 `card create '<json>'`** —— 因为 `card` 这层要同时挂 `list` / `tag` 等维护子命令,顶层语义混淆 |
...
| `tag` | v2 的"tag 是一级一等公民,自动从 sync / explore 注入"那套机制被砍。**0.8.x 重新加回**一个轻量 tag —— **只 user-side 手动写**(`memory.talk card tag` / `memory.talk session tag`),不再有任何自动注入,纯组织标签不参与论坛动力学。详见 [card.md](card.md) / [session.md](session.md) |
...
memory.talk card create '{
  ...
  "rounds": [{"session_id": "sess-15f0a7fb-…190b0", "indexes": "11-15"}]
}'
...
memory.talk card create '{
  ...
  "rounds": [{"session_id": "sess-15f0a7fb-…b81c", "indexes": "3-8"}],
  "source_cards": [
...
> 数据结构 / schema 见 [`../../structure/v3/`](../../structure/v3/);HTTP API 见 [`../../api/v3/`](../../api/v3/);**机制 / pipeline / 设计决策** 见 [`../../designs/v3/`](../../designs/v3/)。
