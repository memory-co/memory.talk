...
走 v3 改名后的端点 `/v3/insights`(见 [`../../designs/v4/card.md`](../../designs/v4/card.md#9-与-v3--insight-的共存与迁移))。数据结构沿用 v3 talk-card(`insight` / `rounds` / `stats` / `reviews`),只是前缀 `card_` → `insight_`。
