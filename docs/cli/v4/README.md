...
- **Card(问题)和 Position(答案)是两个对象,两条命令**:`card create --issue` 建问题(没答案也合法);`card position --card <cid> --answer` 给问题加一个答案。
...
├── card create | position | view              # v4:建问题 / 加答案 / 看一张卡的所有答案
...
**v4 重写 / 新增的命令**:`card`(含 `card position`)/ `review` / `link` / `read` / `search` / `recall` / `insight`——卡的数据模型变了(问题 + 答案 + IBIS 边 + 出处),这些命令行为都跟 v3 不同。**真·沿用 v3 不变的只有基础设施**:`setup` / `server` / `sync` / `session`(见 [`../v3/`](../v3/),本目录不复制);`explore`(抽 v4 卡的工作台)**留下一轮设计**。命令详情见 [#六、命令详情](#六命令详情)。
...
| Position(候选答案) | `card position --answer` 落的那个;`claim`(答案文本,内联) |
...
| 创建 | `card create '<json>'`(insight + rounds) | 建问题 `card create --issue`;加答案 `card position --card --answer [--source ...]` |
...
memory.talk card position --card card_01jz8k2m \
    --answer '默认简洁、要点优先' --source sess_abc:11-15

# 3. 再补一个竞争答案
memory.talk card position --card card_01jz8k2m \
    --answer '调试场景下要详细、带完整命令' --source sess_def:3,7,12
...
| 命令 | 文档 |
|---|---|
| `card`(create / position / view) | [card.md](card.md) |
