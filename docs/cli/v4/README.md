...
- **Card(问题)和 Position(答案)是两个对象,两条写命令**:`card create '<问题>'` 建问题(没答案也合法);`card position <cid> '<答案>'` 给问题加一个答案(主内容走位置参数)。
...
├── card create | position | review            # v4:建问题 / 加答案 / 对答案表态
├── link create | list                          # v4:卡间 IBIS 边(card_links)
├── read <id>                                    # v4:读 card_ / pos_ / sess_(按前缀判型)
├── search <query> [--where DSL]                # v4:撞问题检索(无沉浮,DSL 换计数字段)
├── recall <session_id> <prompt>                # v4:hook 无意识召回,撞问题 → 取答案 → 注入
...
| 校验(对不对) | `card review <pid> <+1/0/-1>`:`argument` 累成 `up_count` / `down_count` / `neutral_count` |
...
| 出处(谁启发的) | `card_sessions`:card↔session,`--source` 落,支持多 session |
...
| 创建 | `card create '<json>'`(insight + rounds) | 建问题 `card create '<Q>'`;加答案 `card position <cid> '<A>' [--source ...]` |
...
memory.talk card create '用户偏好什么回答风格?'

# 2. 给这个问题加第一个答案
memory.talk card position card_01jz8k2m '默认简洁、要点优先' --source sess_abc:11-15

# 3. 再补一个竞争答案
memory.talk card position card_01jz8k2m '调试场景下要详细、带完整命令' --source sess_def:3,7,12

# 4. 后续会话里对某个答案表态(顶 / 踩 / 中立)
memory.talk card review pos_01jzr5kq +1 --cite sess_ghi:20-25 --comment '又一次验证,简洁版接住了'

# 5. 看这张卡现在长什么样(所有答案 + 各自 credence)
...
```bash
memory.talk card position card_01jz8k2m \
    '其实用户要的是"可调档",默认简洁但一句话能展开' \
    --source sess_xyz:4-9
memory.talk card review pos_OLD -1 --cite sess_xyz:4-9 --comment '纯简洁漏了细节场景'
```
...
| `link`(create / list) | [link.md](link.md) |
