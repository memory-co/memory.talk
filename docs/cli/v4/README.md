...
- **Card(问题)和 Position(答案)是两个对象**:**问题(卡)不手动建** —— 由 [`session mark`](session.md#session-mark) 里的 `#…？` 在读 session 时自动建 / 关联;`card position --card <cid> --claim '<答案>'` 给已有问题加一个答案。
...
├── card position | review | link            # v4:加答案 / 表态 / 连边(IBIS);问题(卡)由 session mark 的 #…？ 自动建
...
| 创建 | `card create '<json>'`(insight + rounds) | 问题(卡)由 [`session mark`](session.md#session-mark) 的 `#…？` 自动建;加答案 `card position --card <cid> --claim '<A>' [--source ...]` |
...
```bash
# 1. 问题(卡)在读 session 打 mark 时由 #…？ 自动建(见 session mark);这里假设已有 card_01jz8k2m
