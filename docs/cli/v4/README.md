...
- **Card(问题)和 Position(答案)是两个对象,两条写命令**:`card create --issue '<问题>'` 建问题(没答案也合法);`card position --card <cid> --answer '<答案>'` 给问题加一个答案。
...
| 校验(对不对) | `card review --position <pid> --vote <+1/0/-1>`:`argument` 累成 `up_count` / `down_count` / `neutral_count` |
...
| 创建 | `card create '<json>'`(insight + rounds) | 建问题 `card create --issue '<Q>'`;加答案 `card position --card <cid> --answer '<A>' [--source ...]` |
...
# 2. 给这个问题加第一个答案
memory.talk card position --card card_01jz8k2m --answer '默认简洁、要点优先' --source sess_abc:11-15

# 3. 再补一个竞争答案
memory.talk card position --card card_01jz8k2m --answer '调试场景下要详细、带完整命令' --source sess_def:3,7,12

# 4. 后续会话里对某个答案表态(顶 / 踩 / 中立)
memory.talk card review --position pos_01jzr5kq --vote +1 --cite sess_ghi:20-25 --comment '又一次验证,简洁版接住了'
...
memory.talk card position --card card_01jz8k2m \
    ...
    --source sess_xyz:4-9
memory.talk card review --position pos_OLD --vote -1 --cite sess_xyz:4-9 --comment '纯简洁漏了细节场景'
```
