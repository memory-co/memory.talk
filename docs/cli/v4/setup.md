...
> `setup` 是纯沿用的基础设施:行为、参数、wizard 流程跟 v3 一字不改,连本地端点前缀都不涉及(setup 直接读写 `settings.json` + 起停 server,不是 `/v3`→`/v4` 那类 HTTP 命令)。配置文件结构见 [`../../structure/v4/settings.md`](../../structure/v4/settings.md)(v4 向量 collections 有增量)。
