...
需要 `server` 在跑;CLI 通过 HTTP 调 `GET /v4/sync/status`,连不上就报错退出。

> **v4 唯一变化**:CLI 打的本地端点从 `/v3/sync*` 挪到 **`/v4/sync*`**(命令用法一字不改;HTTP 契约见 [`../../api/v4/sync.md`](../../api/v4/sync.md))。watcher 机制 / debounce / cursor 语义照 v3。
...
memory.talk sync ─── HTTP ───→ GET /v4/sync/status
```
