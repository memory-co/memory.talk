...
filter.py 是纯 selector：从 stdin / 命令行参数啥都不读，**输出 subject_ids 到 stdout**，每行一个。空行和 `#` 开头的注释行忽略。
...
执行约定：
- 框架用 `sys.executable` 启动子进程（跟 memory-talk 同一个 venv）
- cwd = `<data_root>/filters/<name>/`
- stdin 不传任何东西
- stdout 的非空、非注释行 = subject_ids
- exit 0 = 成功；非零 = 失败，`run` 返回错误
...
### 为什么 filter.py 输出每行一个 id 而不是 JSON

最薄的契约：awk / grep / sqlite3 / shell 都能写 filter.py，不强制 Python。memory-talk 这一侧解析也最简单。

如果以后需要附加元数据（如 score、reason），可以加 `filter.py --json` 模式，但不作为 v1 契约。
