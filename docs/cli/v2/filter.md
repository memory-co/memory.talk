...
```python
"""new-session: 框出最近 sync 触达的 session。

`sync_session` tag 由 `SessionService.ingest` 自动打：fresh import 是
`sync_session: new`，rounds 追加是 `sync_session: update`。filter 按
key 选择（值随便）。`mark` 调走 sync_session tag —— 这条离开框；下次
sync 触达又会被打回来。
"""
...
- `sess_aa`: -`sync_session`
- `sess_bb`: -`sync_session`
- `sess_cc`: -`sync_session`
```
...
```json
{
  "filter": "new-session",
  "applied": [
    {"subject_id": "sess_aa", "added": [], "removed": ["sync_session"]},
    {"subject_id": "sess_bb", "added": [], "removed": ["sync_session"]},
    {"subject_id": "sess_cc", "added": [], "removed": ["sync_session"]}
  ]
}
```
...
**取景框含义**：所有最近被 sync 触达过、还没处理过的 session。

`sync_session` tag 是 sync 流程自动打的：

- 新导入 session → `sync_session: new`
- rounds 追加（同 session 又来新轮次）→ `sync_session: update`

`mark` 把 tag 调走，这条离开取景框。下一次 sync 又触达它时（无论是 `new` 还是 `update`），tag 重新被打回来，又出现在框里。

**用法**：周期性扫一遍最近 sync 进来的 session 抽 base card —— 抽 card 这事儿你**自己**做（人工 / LLM / 别的脚本，反正不归 memory-talk 管）。filter 只负责：
1. 给你看哪些没处理过（`run`）
...
# 6. 下次 sync 进 5 条新 session（或既有 session 来了新 round），再 run 一次
$ memory-talk sync
$ memory-talk filter run new-session
...
# 7. 想从头再来？unmark 全部（重新打回 sync_session tag）
$ memory-talk filter unmark new-session
```
