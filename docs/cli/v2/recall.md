...
memory-talk view card_01jz8k2m  # 选定 LanceDB 做向量存储
memory-talk view card_01jzp3nq  # 异步数据库连接池实现
memory-talk view card_01jzq7rm  # 搜索引擎核心原理
```
````

约定:
- **不打 H2 / H3 标题**(像 `## Memory recall (3)` 这种)。recall 的输出会被 harness 直接 inline 进 LLM 的 context,头部加个标题等于强制下游 LLM 看到一个不属于业务的"段落标识",反而干扰。
- 输出整段是一个 `bash` 代码块,目的:
  1. 第一行 `# Relevant memories — run any to expand detail:` 用 bash 注释告诉 LLM 这一段是什么。
  2. 每条命中**直接以可执行命令形式呈现** —— `memory-talk view <card_id>  # <summary>`。LLM 只要识别到"这张卡可能有用",对应的展开命令就在它眼前,可以直接复制执行(harness 通常允许 LLM 调 shell 工具)。
  3. summary 写在 `# ` 注释里,语义上跟 bash 习惯吻合 —— 注释 = 不影响执行的元信息。
- 命中数 0 → **整段不输出**(stdout 空字符串)。harness 拿到空就当本次没召回,不打"无结果"占位文字干扰 prompt。
- TTY 渲染下这一段会被 rich 当 fenced code block 上色;管道 / 直接喂 LLM 都是 raw bash 文本。两边都好读。
...
```json
{
  ...
  "recalled": [
    {"card_id": "card_01jz8k2m", "summary": "选定 LanceDB 做向量存储"},
    {"card_id": "card_01jzp3nq", "summary": "异步数据库连接池实现"},
    {"card_id": "card_01jzq7rm", "summary": "搜索引擎核心原理"}
  ],
