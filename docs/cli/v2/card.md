...
- `from_search_id`（可选）：如果这张 card 是基于某次 search 的结果提炼出来的，可以带上那次 search 的 `search_id`。服务端写入 log 的 `created` 事件时会回填到 detail 里，方便从 AI 的 tool-use 对话对照"是哪次 search 启发了这张 card"。
