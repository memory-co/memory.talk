...
| SQLite 表 | `cards` / `card_stats` / `card_source_cards` / `reviews` | `insights` / `insight_stats` / `insight_source_cards`;**`reviews` 直接 DROP**(未用,不留 `insight_reviews`——腾名给 v4) |
| LanceDB collection | `cards` | `insights`(目录级 `rename_collection`,不重灌) |
| id 前缀 | `card_<ulid>` | **不变,仍 `card_<ulid>`**(两段式:`/v3` vs `/v4` 路由命名空间区分,共享前缀不歧义;彻底改 `insight_` 留作后续可选) |
| 文件罐 | `cards/<bucket>/...` | `insights/<bucket>/...` |
| 代码符号 | `service/cards.py` 等 | `service/insights.py` 等 |

> **完整迁移机制单独成篇**——SQLite 表改名 + `reviews` DROP、LanceDB collection **目录级 `rename_collection`**(新增的 `AdminBackend` 原语,零成本不重灌)、**`card_` id 前缀保留不重写**(两段式)、文件罐顶层目录 move、幂等 / 失败语义,见 [**insight-migration.md**](insight-migration.md)。

### 步骤二:v4 card 全新起(复用腾出来的名字)

insight 让出 `card` 这个名字(`cards` 表 / `card` 命令 / `/v3/cards` 路由 / `cards` collection;`reviews` 直接 DROP 腾名)之后——但 insight 行的 `card_` id 前缀**保留不变**,靠 `/v3` vs `/v4` 命名空间与 v4 共享——v4 在干净地基上建:`cards`(问题)/ `positions`(答案 + 内联文本,`pos_`)/ `reviews`(对 Position 表态,`review_`,`argument` ±1/0)/ `card_links`(card↔card)/ `card_sessions`(card↔session 出处),**卡复用 `card_` 前缀 + `card` 命令**。**v4 与 insight 物理隔离**(不同表、不同 collection、不同前缀),互不干扰地共存。

### 步骤三:慢慢下掉 insight(后话,本稿不强求)

- insight **只读可搜**:老数据继续能 `search` / `read` / 回看,但**新抽卡只写 v4**。
- 可选**投影**:把每条 insight **投影**进 v4 图——一条 insight →(一张**卡** =「这条洞见在答的问题」+ 一个 **Position** = 洞见本身那个答案,其 `rounds` → 一条 `card_sessions`(这条 insight 来自哪个 session))。v3 的 review 已 DROP(从未真正用起来),无可投影的表态;投影完的 insight 标记为已投影、不再重复投。
