# metas —— 认知层的容器:层是一个 check(v5 设计)

> **状态:框架稿,未实施。** 本篇引入 **Metas** 这个抽象:memory.talk 的认知层不再是「issue 和 card 两套存储」,而是一个 **Metas**——一个 [collectbase](https://github.com/memory-co/collectbase) 仓库;issue 和 card 各是其中的一个 **layer**;layer 由 **schema** 定义,用户写清 schema 就能加自己的 layer。字段 / 命令后续分篇。总定位见 [README.md](../README.md)。

相关:
- v5 store(git 存认知层——本篇把「怎么用 git」交给 collectbase): [store.md](store.md)
- v5 issue / card(Metas 里内置的两个 layer): [issue.md](issue.md) / [card.md](card.md)
- v5 metas layer(用户怎么设计一个自己的层): [layer.md](layer.md)
...
代码里对应的改名:`services/metas/`、`/api/metas/`、`MetasService`。

---

## 1. 一句话:metas 是认知层,layer 是一个 check
...
> **Metas** = memory.talk 的认知层。它是一个 collectbase 仓库;它的每一个 **layer** = 一个名字 + 一段路径 + 一个 **check**(这次提交过不过)。

于是三层结构里「记事 / 议事」那两层,在存储上统一成一个东西:

```
Metas(一个 collectbase 仓库,~/.memory.talk/memory/)
...
三个理由,都是 metas-store.md 想要而裸 git 给不了的:
...
- **layer 是可加的,不改包里的代码。** collectbase 用根上的 `layers` 文件当锚定;memory.talk 对应的是根上的 **`metas.json`**——整个 metas 的配置 + `layers[]` 清单(最底在前,只有名字和 `builtin`);用户层是 `~/.memory.talk/layers/<名>.py` 里一个 `Layer` 子类,启动时载入并登记进去。加层就是加一项、开一条从始祖出发的分支;`git log metas.json` 是层的变化史。
...
| **协议** | 一份 YAML:目录里允许哪些路径、每个文件的 formatter(字段 / 枚举 / 引用 / 正文)、哪些必需;后端用它校验,前端用它画表单([layer.md](layer.md)) | `readme.md`(字段 links + 正文)+ `positions/*.md`(字段 links / rank / verdict + 正文);别的文件拒绝 | `readme.md`(字段 context / links / issue + 正文);别的文件拒绝 |

schema 决定的事:

- **读**:通用的读 / 列 / 历史 / 检索,按 schema 解析文件就能做,不需要为每个 layer 写代码。
- **写**:通用的建 / 改 / 删,按 schema 校验后落盘 + `[layer 名]` 提交。删也是提交——文件没了,历史在。
- **引用**:schema 里标出「这个字段指向哪个 layer 的对象」,于是 card.issue、issue.card、card.links 这些跨 layer 的边有了统一的表达;顺链接走(metas-store.md §5 的检索方式)靠它。
...
metas 的每个动作是一个 commit,做它的人(登录态里的名字,[auth.md](../auth.md))就是 commit 的 **author**——立场谁提的、论证谁给的、卡谁改的,全在 `git log` / `git blame` 里,对象里不另存 user 字段。agent 做的提交挂在驱动它的 user 名下,body 的 `Work:` 记它在哪个 work 里做的。服务自己做的提交(比如启动时登记新层)用服务配置的默认名。详见 [user.md §5](../user.md)。

---

## 6. 「一个决定一个 commit」怎么变

metas-store.md 说:争出结果写卡,issue 记结论 + card 建正文,**同一个 commit**。collectbase 说:**一个提交只能属于一层**——跨层的提交恰好就是把过程和结论搅在一起的那个动作,必须拆。
