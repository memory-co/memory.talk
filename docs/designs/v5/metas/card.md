...
- `sync` 从 Claude Code 的 `~/.claude/projects` 目录读工作单元。
...
注入的是卡的正文,连同它的链接——agent 觉得不够,顺着链接去看讨论页(issue)和出处(work)。**注入的永远是 card,不是 issue 原文,不是别的 work 的工作单元**:词条才值得占 context,讨论页不值得。
