...
> **状态:框架稿,未实施。** 本篇在 Metas 里再加一个内置的 layer:**origin**。外部来的各种东西——文档、网页、别人给的材料、导出的聊天记录——还没被消化成 issue 或 card 之前,先原样放在这里。它是 Metas 的**最下层**,collectbase 意义上的事实层:上层引用它、改不动它。总定位见 [README.md](../README.md)。
...
[metas.md §4](README.md) 曾说:memory.talk 的 Metas 里没有 collectbase 意义上的「事实层」——最底下是 issue,而 issue 已经是推论(争的过程)了。现在补上:
...
origin **没有形态约束**:Metas 目录树里任何**不带层后缀**的文件或目录,都是 origin。一份 markdown、一个 PDF(软链到 blob)、一个装着十几个文件的文件夹——放进来就是。
...
按 [metas-layer.md §2](layer.md) 的规则「引用谁就排在谁上面」:所有层都会引用 origin(它是出处),没有层被 origin 引用,所以它在最底。用户自定义的层同样排在它之上。

---

## 9. 这篇有意不定的事

- **rounds 摘录的粒度**(§6):按轮、按段、还是整个工作单元;摘录要不要保留工具调用的原始输出。
...
- **采集器**:谁把网页抓成 origin、谁把外部工具的工作单元同步进来——是 memory.talk 的一部分,还是外部脚本直接往 Metas 里 commit(`[origin]`)。collectbase 说「不做采集」;倾向 memory.talk 先只提供「放进来」的端点,采集是外部的。
