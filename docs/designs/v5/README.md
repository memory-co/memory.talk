...
>
> 分篇:[work.md](work.md)(做事层:work 树)、[issue.md](issue.md)(议事层:树上节点管、派活取证)、[card.md](card.md)(记事层:维基式事实条目,issue 是它的讨论页)、[store.md](store.md)(存储:git 存 card / issue,裸文件存 work,没有数据库)、[protocol-server.md](protocol-server.md)(每个协议背后把现场建出来的 server)、[session.md](session.md)(work 里的一个现场:身份脱离布局)、[member.md](member.md)(人:谁在操作 / 操作过某个 work,不做权限)、[collections.md](collections.md)(认知层的容器:collectbase 仓库,issue / card 各是一层,layer 由 schema 定义、可自定义)、[collections-layer.md](collections-layer.md)(怎么设计一个自己的层:四个问题、两个文件、系统替你做什么 / 不做什么)、[origin.md](origin.md)(最底层:外部来的、未消化的原文,上层改不动的地板)、[manager.md](manager.md)(目录下的 manager.json 把变动打给绑定的 work,work 干活往下推;取代 issue 上的 manager_work 字段)。
>
...
## 2. 三层抽象:work / issue / card

| 层 | 对象 | 它是什么 | 主语在干什么 | 来源 |
|---|---|---|---|---|
| 做事 | **work** | 一个工作单元,里面盛放若干个 **code agent session**;复杂的事是一棵 **work 树** | 干活 | 在 memory.talk 里**原生实现**,底层逻辑与 [shellbase](https://github.com/memory-co/shellbase) 完全一致 |
...
三层的分工一句话:**work 装已经定下来、正在做的事;issue 装还没定的事;card 装已经站住、可以带走的认知。** 三层里只有 work 有状态和完成——它是过程,做完就结束(复杂的事是一棵树,叶子先完、往上收拢);issue 是过程里冒出来、可以跨 work 一直开着的争论;card 是争论沉淀下来的、能被反复召回的结论。
...
- **session 从「事后导入的对象」变成「在 work 里原生发生的对象」**。v3 的 sync(从平台目录抄会话)退成兼容路径——work 里跑的 session,memory.talk 本来就看得见。
- **work 承接 v3/v4 的 explore**。explore 原本是「一个目录 + 一条时间分割线」的抽卡工作区;v5 里每个 work 天然就是这样一个工作区:work 里的 session 是它的先验素材,之后的 work 是它的后验证据。explore 不再单独存在。
- **复杂的事是一棵 work 树**。work 可以有子 work:根是「把 X 做出来」,叶子是真正坐下来干的一件小事。拆分、顺序、状态、完成——执行的结构全部落在 work 上;树上每个节点都可以有自己的画布。
...
- **issue 从 work 里冒出来**。做事过程中的每个「为什么」「该不该」「哪个更好」都是一个 issue;v4 设计的逐 round 标注 + `#问题` 自动建问题([session-annotation.md](../v4/session-annotation.md))就是 issue 的主要入口。
...
1. **work 里干活**:若干 agent session 在 work 的画布上跑,过程按 round 留下。
2. **work → issue**:对 session 逐 round 标注(以写代读),标注里 `#` 出来的问题经检索判定——新问题建 issue,老问题挂到既有 issue;标注里给出的回答落成 position,后续证据落成 argument。
...
- **沉默 ≠ 确认**:一个 work 没提到某个 issue,不给它任何分。
