...
>
> 分篇:[work.md](work.md)(做事层:work 树)、[issue.md](issue.md)(议事层:树上节点管、派活取证)、[card.md](card.md)(记事层:维基式事实条目,issue 是它的讨论页)、[store.md](store.md)(存储:git 存 card / issue,裸文件存 work,没有数据库)、[protocol-server.md](protocol-server.md)(每个协议背后把现场建出来的 server)、[session.md](session.md)(work 里的一个现场:身份脱离布局)、[member.md](member.md)(人:谁在操作 / 操作过某个 work,不做权限)、[collect.md](collect.md)(认知层的容器:collectbase 仓库,issue / card 各是一层,layer 由 schema 定义、可自定义)、[collect-layer.md](collect-layer.md)(怎么设计一个自己的层:四个问题、两个文件、系统替你做什么 / 不做什么)、[origin.md](origin.md)(最底层:外部来的、未消化的原文,上层改不动的地板)、[manager.md](manager.md)(目录下的 manager.json 把变动打给绑定的 work,work 干活往下推;取代 issue 上的 manager_work 字段)。
>
