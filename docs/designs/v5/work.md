...
> **状态:框架稿,未实施。** 本篇只立 work 这一层的大框架:它是什么、为什么是一棵树、跟 shellbase 的 window 是什么关系(继承其底层逻辑、原生实现)、里面装什么、session 怎么进来、做完留下什么。字段 / 端点 / 命令等后续分篇。总定位见 [README.md](README.md)。
...
在树的任何一层,work 都做同一件事:**把为了这件事用到的现场收拢到一起**——一个跑 Claude Code 的会话、一个跑 Codex 的会话、一个裸终端、几个正在看的网页、一个文件目录。从哪开始、中途用了什么、做到哪了、最后怎么样,都在这里。

它是 v5 的**顶层入口**:打开 memory.talk 看到的是 work 树,而不是 session 列表。session 退到 work 里面,成为 work 的会话。
...
**shellbase 到此为止,不再作为独立项目存在;它的 window 在 memory.talk 里原生实现,就是 work 的画布。** work 不另起一套运行时——shellbase 的画布、块、块即 URI、终端 attach、无中生有 + 重入、后端是状态唯一权威,这套底层逻辑 work **完全一致地**实现一遍,只是搬进 memory.talk 自己的进程和存储里。打开一个 work,就是打开它的画布;在 work 里开一个 Codex 会话,就是在画布上放一个 `codex://` 块——这个块的协议决定去找哪个 **server** 把现场建出来,见 [work-server.md](work-server.md)。
...
| | window(画布) | work |
|---|---|---|
| 关心什么 | 屏幕怎么分:几列、每列从上到下摆哪些会话、哪些收起 | 有哪些现场、它们是为了什么事汇在一起、事做到哪了 |
| 布局 | 是它的核心状态,精确持久化 | **只是一个视图**,可以随时重排、重排不影响 work 本身 |
| 会话身份 | 由 URI 里的 `window` + `block` 位置参数决定 | 由「它属于这个 work」决定,跟摆在画布哪里无关 |
| 生命周期 | 页面开着就在 | 有目标、有状态、有开始、有做完,还有父子 |

所以:**画布是 work 的显示层,work 是画布的意义层**。布局丢了、重排了、换个设备打开分割方式全不一样——work 一点没变,因为 work 记的是**会话、目的和进度**,不是列和顺序。画布的形状很朴素:默认一列,会话从上到下摆;想并排看就加一列,把会话挪过去;暂时不看的收起成一行(turbome.ai 的 explore 页也是这个思路)。

> 这条决定了原生实现时**唯一有意偏离 shellbase 的地方**:shellbase 里终端身份挂在 `block` 序号上,序号又跟布局绑着;work 里的会话身份要**脱离布局**——一个会话是这个 work 的会话,不因为它被移到别的格子就变成另一个会话。其余底层逻辑照搬,只有身份这一点在 URI 模型上要改,见 [session.md](session.md)。

---

## 4. work 里装什么:各种「现场」,不只是 agent 会话

work 的会话是**现场**(一个活着的、可以回去看的东西),不限于 code agent:

| 会话类型 | 例子 | 它对 work 的意义 |
|---|---|---|
| **agent 会话** | `claude:///proj`、`codex:///proj` | 干活的主力;它们的 round 是 work 留下的主要痕迹 |
...
## 5. session 怎么成为 work 的会话:在 work 里打开,就是它的

这是 work 相对 v3 / v4 最实在的一个改变。以前 memory.talk 认 session 归属靠**事后推断**:sync 从平台目录把会话抄进来,再拿 `cwd` 这类物理信号猜它属于哪个 explore。v5 里不用猜——

- **在 work 里开一个 agent 块,这个会话从诞生那一刻就是 work 的会话。** memory.talk 拉起它、看着它跑、记它的 round,归属是原生的。
- 同一个目录,可以在不同 work 里各开各的会话,它们互不相干——归属看 work,不看目录。
- **一个会话只属于一个 work,而且是树上的一个确定节点**,不属于「整棵树」。要在别的事里用到它的结论,走 issue / card,不是把会话搬家。
- 从外部平台导入的历史会话(v3 的 sync 路径)继续保留,作为没有 work 归属的会话存在;要不要给它们补一个 work、怎么补,是兼容问题,后面再说。
...
第 4 段就是 v3 explore 想要的东西,在 work 里天然成立:**一个 work 的会话是它自己提出的问题的先验素材;后面的 work 是这些问题的后验证据**。不用再画一条时间分割线——work 的边界就是那条线。
