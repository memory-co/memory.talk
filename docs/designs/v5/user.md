...
> **状态:框架稿,已有实现。** 本篇把「人」立成一个顶层对象:**user**。它是**注册的实体**——和 work 平级,有自己的存储(fs 或数据库,走 [provider.md](provider.md) 的仓储),有档案(名字、显示名、邮箱)。它出现在三处——work 是谁**建**的、work 现在谁**在动 / 动过**、collections 里每个提交是谁**做的**。它**几乎不做权限**:整个实例给一个团队用,团队内不限制;注册解决的是「你是谁」,不是「你能干什么」。「你是谁」由 [auth.md](auth.md) 的登录证明,admin 只多管账号这一件事。原来叫 member 的那套「谁在操作 / 操作过某个 work」的可见性记录,并入本篇,名字统一叫 user。字段见 [`../../structure/v5/work.md`](../../structure/v5/work.md),端点见 [`../../api/v5/works.md`](../../api/v5/works.md)。
