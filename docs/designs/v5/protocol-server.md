...
- v5 store(work 的登记是裸文件;server 不存 work 状态): [store.md](store.md)
...
server **不做**的事同样重要:它**不记 work**——哪个块属于哪个 work、块在画布哪个位置、什么时候开的,这些全在 work 层的裸文件里([store.md §4](store.md));server 只管「这个 id 的现场活没活着」。它**不做认知**——round 怎么标注、问题怎么建,跟它无关。

---

## 4. 请求流:一个块从 URI 到现场

1. **work 画布上放一个块**,块有一个 URI。work 层给这个块一个**稳定的会话 id**——脱离布局的那个([work.md §3](work.md)),不是格子序号。
...
4. **server 交回窗和把手**。窗的 URL 给画布 iframe 嵌进去;把手留给 work 层——观测(这个会话跑到哪了、新的 round)、驱动(往里发一句话)、销毁(块关闭时)。
5. **work 层记登记**:会话 id ↔ URI ↔ 这个 server ↔ 现场是否活着。这份登记是 work 目录里的裸文件,是唯一权威;server 重启后,work 层拿登记去 server 那里把现场一个个取回来。
