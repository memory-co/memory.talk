...
- **往回:issue → task**。issue 可以绑定一个 **manager task** 来推它,也可以为某条论证的取证**派出**一个新 task;派出去的 task 做完,结果落回 issue 当论证。task 本身不因此变成另一种东西——它只是多知道一件事:「我是为哪个问题开的」。见 [issue.md](issue.md) §3–§4。
- **往下:card → task**。task 开工时,recall 把命中的 card 注入到 task 里的 agent 会话中。注入的是 card,不是 issue,也不是别的 task 的会话原文。

task 之间**不直接连**。两个 task 有关系,是因为它们碰到了同一个 issue、被同一个 issue 派出、或者用了同一张 card——关系走认知层,不走任务层。这样 task 可以一直很简单:一个容器、一句目标、一堆现场。
