...
3. **寻址 ↔ 复合键**:`card_x#p1` ↔ `(card_id,'p1')`、`card_x#l2` ↔ `(card_id,'l2')`、`sess_y#m2` ↔ `(session_id,'m2')`;
4. **对 reality 只有软引用**:`session_id` / `indexes` 指向 [reality 库](reality-data.md),无 FK、容忍悬空,mind 永远不写 reality。
...
CREATE TABLE reviews (               -- 表态 = 一等事件(计数/credence 都从这现算)
  ...
  session_id  TEXT NOT NULL,         -- 证据(软引用 reality)
  indexes     TEXT NOT NULL,         -- 证据 rounds('11-15' / '3,7,12')
...
CREATE TABLE link_sessions (         -- 边的出处
  card_id TEXT NOT NULL, link TEXT NOT NULL, session_id TEXT NOT NULL,
  indexes TEXT NOT NULL, created_at TEXT NOT NULL
);

-- ═══ 结晶标注(mark:一次「读完整个 session」的逐 round 判断)═══
CREATE TABLE marks (
  session_id  TEXT NOT NULL,         -- 标注的是 reality 里哪个 session(软引用)
  mark        TEXT NOT NULL,         -- 'm<n>',服务端分配,session 内递增
  last_index  INTEGER NOT NULL,      -- 乐观锁基线(标注时 session 到第几轮)
  description TEXT NOT NULL,
  created_at  TEXT NOT NULL,
  deleted_at  TEXT,
  PRIMARY KEY (session_id, mark)     -- ↔ sess_y#m2
);
CREATE TABLE mark_rounds (           -- 逐 round:index 从 1 起严格递增,覆盖 ≥90%
  session_id TEXT NOT NULL, mark TEXT NOT NULL, idx INTEGER NOT NULL,
  comment TEXT,                      -- 可空 = 读了没东西标(只占覆盖)
  PRIMARY KEY (session_id, mark, idx)
);
