...
- v5 store(task 的痕迹是裸文件,不在 Collect——它和 origin 的边界见 §6): [store.md](store.md)
...
- **认知要有地板。** issue 的论证说「证据在这」,card 说「这是事实」——「这」得指向一个不会被上层改掉的东西。没有 origin,它们只能指向 task 的 rounds(裸文件,不在 git 里、没有层的保护)或者互相指(推论指推论,正是 collectbase 说的「记忆腐烂」那条回路)。
- **外部材料得有地方放。** 现在一份外来的文档要么被塞成一张卡(那就把「材料」当成了「断言」),要么丢在 task 目录里(不进 git、没人管、做完就冻结)。都不对。它该有自己的位置,而且是最底的位置。
...
| 导出的会话记录 | 从别的工具带过来的对话(不是本实例 task 里跑出来的——那些见 §6) |
| 人写下的给定 | 「这个项目必须用 Python 3.12」这类不需要争的约束——**注意它是原文,不是卡**;卡是消化后写的 |

三条判据,缺一不进:**外部来的**(不是本实例的推论)、**原样的**(不改写)、**还没消化的**(或者消化了也留着——见 §5)。

不进 origin 的:memory.talk 自己产生的任何东西(issue、card、决定、摘要)——那些是上层;task 的过程痕迹(rounds、屏幕、事件)——那些是裸文件(§6)。
...
| 在不在 git | 不在 | 在,最底层 |
| 谁引用 | issue 的 `origin` / `evidence` 指 `(task_id, rounds)` | issue 的 `evidence`、card 的出处指 origin 对象 |

一段 round **值得长期当证据**(比如「基准跑出来就是这个数」),而它所在的 task 做完就冻结、不进 git——那就把那几轮**摘录**一份放进 origin:一个 origin 对象,`source` 写明来自哪个 task 的哪几轮,内容原样复制。这是唯一一种「本实例产生的东西进 origin」——因为它已经不是推论,是被选出来当证据的原文。

> issue 里那个叫 `origin` 的字段(「从哪个 task 的哪些 round 冒出来」)和这一层同名,不是巧合:它们说的都是**出处**。字段指向 task 的 rounds 或指向 origin 层的对象,两种都行;长期要留的,走后者。
