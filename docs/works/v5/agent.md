...
> **状态:设计中。** [README §5 北极星](README.md)的落地实体叫 **agent**:一个**实例化**的记忆管家。每个 agent = `name` + **`harness` 字段**(底座:`claude-code` / `codex` / `quickjs` 自进化〔QuickJS + WASM〕)+ **一个独立的 mind 库** + 常驻 server(可对话)。**reality 只有一份(共享经验),mind 每个 agent 一个(各自的信念)**。
...
| **harness** | agent 的一个**字段**:loop 跑在哪个底座上——`claude-code` / `codex`(租现成 executor harness)或 `quickjs`(自研自进化:JS 循环跑在 QuickJS 上,QuickJS 跑在 WASM 沙箱里) |
...
memory.talk agent create sandbox --harness quickjs
```

- **每个 agent 一个独立的 mind 库**(自己的 seekbase 实例):信念、proofs、engine_versions(quickjs)全在自己库里——**信念有主人**:谁结晶的、谁治理、谁负责;
...
| `quickjs` | 自研:**JS 循环跑在 QuickJS 上,QuickJS 跑在 WASM 沙箱里** | 注入的 `llm()` | **是**(§3) | **无** |
...
- **为什么自研用 QuickJS + WASM**:要**自进化**,而且进化的代码是 `llm()` 生成的——两层各答一半:
  - **WASM = 硬沙箱边界**:capability-based——模块里**什么都不存在,import 什么才有什么**(没有 fs / net / os 可摸);沙箱不是加锁,是**从没有钥匙**;宿主还能在 VM 边界上**计量强制**(fuel / 指令数 / 内存上限),看门狗不再靠自觉;
  - **QuickJS = 引擎壳**:极小、可嵌入、整机编译成 WASM;跑的是 **JS——LLM 写得最熟的语言**(自进化的每一版都由 `llm()` 起草,语言命中率直接决定进化质量,这是弃 Lua 选 JS 的主因);解释执行、代码即数据,引擎读改自己天然顺手。
...
可用(对共享 reality):
  ...
  llm(prompt)           ← quickjs harness 经 WASM import 注入;claude-code / codex 自带
没有:
...
## 3. 自进化(quickjs harness 独有)

- **引擎代码是数据**:JS 源码存进**该 agent 自己的 mind 库**(`engine_versions` 表)——版本化、可 grep、时光机可回放进化史;
- **自改 = 受治理的写**:新版本 append-only 落表 → **影子对照**(起一个影子 agent 在同一 reality 上干跑,比 [metrics §3](metrics.md) 的护栏组 + 目标线)→ 达标才切换,坏了回滚版本号;
- **沙箱保证进化不越界**:改的是 JS,跑在同一个「只 import interface」的 WASM 实例里——能改得更聪明,**改不出新的 import**(能力面在 VM 边界上封死);
- **看门狗在 VM 边界上强制**:fuel / 指令数 / 内存上限由 WASM runtime 计量,预算 / 超时 / 异常率熔断是宿主(daemon)的——引擎碰不到,也**骗不过**(计量在它下面一层)。
...
- `quickjs`:**没有 session 这个概念**(循环 + 状态全在 mind 库里);
...
```
        ...
        │ harness: cc    │   │ harness: codex│   │ harness: qjs   │
...
- **quickjs + wasm 细节**:WASM runtime 选型(wasmtime / wasmer / 嵌入 daemon 的绑定)、QuickJS 发行版(quickjs-ng?)、fuel 与内存配额、宿主 import 面的最小集(interface + `llm()` + 时钟?)、`llm()` 的模型路由与预算;
