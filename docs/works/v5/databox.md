# databox — 数据库抽象层(v5 设计)

> **状态:设计中。** 工作名 **databox**(数据底座,命名待定,见 §8)。定位与 [searchbase](../v3/searchbase-extraction.md) 同一路数、**更进一步**:searchbase 把「向量检索」封成一个端口;databox 把**整个数据层**封成一个端口——一个**类 supabase 的通用 ORM**,底层 **DuckDB + LanceDB**,而且比 supabase 多一件事:**模糊(语义)查询是 ORM 里的一等算子**——查询链里写 `search(...)` 就自动语义检索。
...
- v5 立意(memory system,databox 是它的数据层): [README.md](README.md)
...
这三个痛点是同一个病:**没有一个统一的数据端口**。searchbase 已经证明了「封端口」这条路(上层不认识向量、不算 embedding);databox 把同样的手法**推广到整个数据层**。
...
```python
db = await Databox.open(data_dir, schema=SCHEMA, embedder=embedder)
...
- **`search()`(supabase 没有的那部分)**:查询链里的一个**算子**,不是另一条 API。出现 `search(text)` 时,databox 自动:① 用注入的 embedder 把 `text` 变向量;② 到 LanceDB 检索;③ 与链上其余结构化谓词**组合**(过滤下推,见 §4);④ 返回带 `_score` 的行,按相关性排序。**调用方永远不见向量、不算 embedding**——这条纪律直接从 searchbase 继承。
- **schema 声明式**:表结构 + 哪些列可搜,声明一次,DDL / 迁移 / 双引擎同步全由 databox 管:
...
- **LanceDB 原位保留**(向量引擎):searchbase 现在管的那摊(embed、ANN、集合)整体**下沉为 databox 的 search 引擎**;searchbase 作为独立端口**被吸收**——对外只剩 databox 一个端口(见 §7)。
- **双引擎同步是 databox 的内政**:`insert` 一行 → DuckDB 写行 + (有 `searchable` 列则)LanceDB upsert 向量;`delete` → 两边一起删;id 对齐、批量、重试都在端口后面。**上层从此没有「双写」这个概念。**

---

## 4. search() 与结构化谓词怎么组合

`search()` + `eq()/gte()/…` 混在一条链上时,执行策略(databox 内部,调用方无感):
...
v5:     files(canonical) → databox(DuckDB + LanceDB,一个端口)
```

- 重建(reindex)= 读文件 → 灌 databox,一条路;
- 无 FOREIGN KEY、容忍悬空、表丢了能从文件重建——这些不变性照旧,由 databox 统一兑现;
- **databox 自己不读写 canonical 文件**(那是 Storage/repository 的事)——它只是那个「更好的派生索引」。
...
- searchbase 的**实现下沉**:`LocalSearchBackend` 那摊(LanceDB 管理、auto_split、超长截断、维护协程)成为 databox 向量侧的内部实现。
- searchbase 的**端口退役**:上层不再 import `SearchBackend`;`service/searchbase_schema.py` 那层「业务对象 → 集合」映射,被 §2 的声明式 SCHEMA(`searchable` 列)取代。
- **迁移路径**(粗):databox 先以新端口出现 → repository 逐个名词从「手写 SQL + searchbase 双调」搬到 databox → 搬完退役 aiosqlite 与 searchbase 端口。SQLite→DuckDB 的数据不用迁(派生层,从文件重建即可)。

---

## 8. 待定

- **命名**:databox 是工作名;候选 databox / membase / datastore /(直接叫)base。
...
- **`searchable` 进阶**:多列 search、跨表 search(v4 unified search 那种 cards+insights+rounds 合并按相关排,在 databox 里怎么表达——`db.search("…", tables=[…])`?)。
- **迁移节奏**:v5 全新起 or 兼容期双跑;DuckDB 文件放哪、要不要和 LanceDB 同目录成一个「databox 实例目录」(像 searchbase 的 `name="v1"` 实例化路数)。

## 与其他 v5 文档的关系

- [README.md](README.md):databox 是 **memory system 的数据层**——问题图、session 索引、召回日志都落在它上面;memory system 的「能力」句子(结晶 / 治理 / 召回)最终都翻译成 databox 上的读写。
- 嵌入契约(待写):宿主(CC)经 CLI / API 驱动 memory system,不直接碰 databox;databox 永远是进程内的实现细节。
