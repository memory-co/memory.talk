...
- **append-only**:标注**只增不改**(跟 card / argument / session 一个不变性)。改主意 = 追加新的一条,不覆盖。
...
- 再有别的证据 顶/踩 这个 Position(argument),沉浮排序——回到 [card.md §3](card.md#3-第二推credence--salience校验轴和显著轴是两回事) 的论坛动力学。
