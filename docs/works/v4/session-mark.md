...
**一份提交 = 这个 session 的一批 mark**(标注整个 session,不绑单一 round),格式是 **YAML**:顶层 `description`(这份 mark 的场景:为什么读这段、读的时候在想什么)+ `marks`(数组,元素是一条 mark〔字典〕,字段 `id` + `indexes` + 可选 `mark`)。
...
- **`indexes` = 这条覆盖 / grounding 的 round(s),每条必给**:语法同 `reviews.indexes`(`36-37` / `3,7,12`)。含 `#…？` 时它是问题的 grounding(落 `card_sessions.indexes`);不含正文时它纯粹记**覆盖了哪几轮**。**覆盖率就是数它**(下条)。
- **`mark` = 自由的「感悟」(可选)**:为什么有这段对话、它在干嘛、我意识到什么。其中**问题用 `#…？` 就地标出来**(§3)。**省略 / 为空 = id-only 标记**:「这几轮读了、没什么可记」——只补覆盖率,**不解析 `#…？`、不建卡 / 连卡**,但照样落盘(`marks/m<n>.yaml` 正文空、`issues` 空 + 一行 `session_marks`),让读回 & append-only 记录一致。`#…？` **只在有正文的 mark 上解析**。
- **round 覆盖率 ≥ 90%(以写代读)**:「以写代读」要的是**读完整条 session**——只标 1–2 轮的长 session 等于没读完。所以一份提交里所有 mark 的 `indexes` 并集(落在 `[1, last_index]` 的 distinct round 数)必须 **≥ 90%** 的总 round 数(`len(covered) >= ceil(0.9 × last_index)`,阈值常量 `MARK_COVERAGE_THRESHOLD = 0.9`,在 `service/session_marks.py` 里好调)。不够 → **整份拒绝**(`MarkServiceError` → 400,`coverage 23% (12/52 rounds) < 90%`),**在任何写入之前**。id-only 条目正是为此而设:一段「无须记录」的轮次用一条范围(`1-35,38-41`)就补满覆盖率,不必每轮一条。交互模式逐轮走到底(标或跳每轮)自然就是 100%。
...
> 为什么 YAML + 数组:读一段常常**不止一个感悟**(一个 `#…？` 问题 + 一句普通观察 + 另一个问题),数组让它们**各自独立成条、各带 id**,而不是糊成一段。YAML 的多行 `|` 块对「带换行、带 `#`、带引号」的自由文本最省转义(对照 [card 命令的 `@file`/`@-`](../../cli/v4/card.md#文本传文件--stdin) 同理)。
...
写入这份 mark 时,系统对**有正文**的每条 mark 的 `mark` 文本(**id-only 条目无正文 → 跳过这步,不建卡 / 不连卡**):
...
3. mark ↔ card 的关联**写进这条 mark 的 `issues[]`**(`card_id` + `is_new` + `indexes`:建新卡 / 关联老卡 + 这问题 grounding 的 round)——这是 **canonical**(file,§6);SQLite 里的 `card_sessions` 表就是**从这些 `issues[]` 派生出来的可 join 索引**。
...
读「`sess_def456` 的**第 1 条 mark**(`sess_def456#m1`)的 `#…？` 建 / 连了 `card_01jz8k2m`」。**不指向具体 Position**——这是卡级关系。
...
一个 `mN.yaml` = 一条 mark,**用 YAML 存**(跟提交同语种,免转换);`description` / `last_index` / `indexes` 从这次提交带下来,`issues` 是写入时解析 `#…？` + 撞库的结果:
...
> **id-only 条目也落盘**:无正文的 mark 写出来 `mark: ""`、`issues: []`,但 `indexes` 照样在——这样读回 & append-only 记录跟有正文的 mark 一致,且它的 `indexes` 仍计入覆盖率。

> **提交 vs 落盘**:一次提交是**一份 round 级 YAML**(顶层 `last_index` + `description` + `marks: [{id, indexes, mark?}, …]`,§2);写入时把 `marks` **拆开**——每条按它的 `id` 落成 `marks/<id>.yaml`,把这次提交的 `last_index` / `description` 一并带进每个文件(`issues` 现解析)。wire 是一份带头信息的数组、盘上是一文件一 mark,两边都用 YAML。
...
5. 全过 → 放行:`marks/m<n>.yaml` 落盘 + `session_marks` 各插一行(含 id-only 条目)。
...
- **mark ↔ card 的链写在那条 mark 的 `issues[]` 里**(canonical · YAML);[`card_sessions`](../../structure/v4/card-session.md) 是从这些 `issues[]` 派生的 card→session 可 join 索引(`card_id` + `session_id` + `mark` id + `indexes`〔这问题 grounding 的 round〕,**无 `position`**——答案级出处在另一条链路 `position_sessions`)。**`session_marks` 管 mark 自己的元信息,`card_sessions` 管 mark→card 的出处边**,两张表各司其职。
...
- **`id`(`m<n>`)谁给**:**提交方逐条显式给**,系统**不默认分配**——每条 mark 必带自己的 `id`。系统只校验:`m<n>` 形态、session 内单调、不跳号 / 不复用(续标接着上次最大序号往后;提交方不知道当前最大就先 `GET …/marks`)。`m<n>` 在 session 内唯一,直接当文件名(§6)。
