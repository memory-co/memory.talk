...
3. mark ↔ card 的关联**写进这条 mark 的 `issues[]`**(`card_id` + `is_new`:建了新卡还是关联老卡)——这是 **canonical**(file,§6);SQLite 里的 `card_sessions` 表就是**从这些 `issues[]` 派生出来的可 join 索引**。
...
- **mark ↔ card 的链写在那条 mark 的 `issues[]` 里**(canonical · YAML);[`card_sessions`](../../structure/v4/card-session.md) 是从这些 `issues[]` 派生的 card→session 可 join 索引(`card_id` + `session_id` + `mark` id,**无 `position`、无 `indexes`**——答案级出处在另一条链路 `position_sessions`)。**`session_marks` 管 mark 自己的元信息,`card_sessions` 管 mark→card 的出处边**,两张表各司其职。
