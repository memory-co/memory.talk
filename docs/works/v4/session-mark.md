...
**一个 round = 一份完整的 mark 提交**,格式是 **YAML 数组**,数组里每个元素是一条 mark(字典),字段两个:`id` + `mark`。

```yaml
# round 37 的 mark 提交(= sess_def456 的第 1、2 条 mark)
- id: 1
  mark: |
...
- **每轮一份**:一次提交对应**一个 round**(挂在 `(session_id, round_index)` 上)。一份提交里可以有**多条 mark**(数组),各自一个 `id`、一段 `mark` 内容。
- **`id` = session 内的序号,mark 不是一等 id**:mark **附属于 session**,寻址形如 **`<session_id>#<id>`**(如 `sess_def456#1` = 这个 session 的**第 1 条 mark**)。`id` 是 session 内递增的序号(append 顺序,session 内唯一),**不是** `mark_<ULID>` 那种独立前缀 id。**`card_source` 就指 `(session_id, id)`**(§4)——这是「出处指 mark 而非仅 session」的落点。读它走 `read sess_def456#1`(§7)。
- **`mark` = 自由的「感悟」**:为什么有这段对话、它在干嘛、我意识到什么。其中**问题用 `#…？` 就地标出来**(§3)。
- **append-only**:mark **只增不改**(跟 card / review / session 一个不变性)。改主意 = 追加新的一条(新 `id`),不覆盖旧的。
- **可标很多遍**:整段对话可被**反复重读**,后一遍带着更多上下文 / 更多已建的卡回头看,常冒出新感悟——**一遍存一个文件**(`marks/pass-N.*`,§6),越后面往往越深。
...
2. 每个问题走 [card.md §6](card.md#6-写路径每轮对话怎么变成卡--旁白--惊讶--question) 的三岔(embed 问题文本 → 撞 `cards` / issue 向量库):
   ...
   - 两种情况都**记一条 [`card_sessions`](../../structure/v4/card-session.md)**:`card_id` + `session_id` + **`mark`(那条 mark 的序号,即 `sess_xxx#n` 里的 `n`)**,这就是「这个 session 的**这条 mark** 启发 / 碰到了这张卡」的出处(§4)。
...
这是本次相对旧设计最关键的调整。旧 `card_sessions` 的出处是 `(session_id, indexes)`——只能说「这卡来自这个 session 的第 11–15 轮」,**粒度到 round、且要靠 round index 间接定位**。新设计让出处**直接指那条 mark**——而 mark **不是一等 id,是 session 的附属**,寻址 `<session_id>#<n>`,所以 `card_sessions` 里出处 = `session_id` + 那条 mark 的**序号 `mark`**:
...
读「`sess_def456` 里**第 1 条 mark**(`sess_def456#1`)启发 / 生出了 `card_01jz8k2m`」。
...
CREATE TABLE card_sessions (
  ...
  mark        INTEGER NOT NULL,          -- 第几条 mark(session 内序号;寻址 = session_id#mark)
...
- 把旧的 `indexes TEXT`(round 区间)**换成 `mark INTEGER`**(session 内 mark 序号)。出处寻址 = `session_id` + `#` + `mark`(如 `sess_def456#1`)。round 不再冗存——它是 mark 的属性,要时从 mark 文件取。
- PK 加进 `mark`:同一卡可被多条 mark(乃至多 session)启发;同一条 mark 也可同时启发问题 + 答案(不同 `position_id`)。
- `idx_card_sessions_mark`(`session_id, mark`):支持「拿一条 mark(`sess_xxx#n`),反查它建/连了哪些卡」。
...
- **后面的轮 / 后面的遍**:某条 mark 写出「他其实要的是可重连会话」——这是对那张卡的一个**答案**,落成它底下的一个 **Position**(答案文本内联在 Position 上),`card_sessions` 再记一条(`position_id` = 这个答案、`mark` = 写出它的那条 mark 序号)。卡从「只有问题」变成「有候选答案」。
...
mark 是**大段自由文本**,量不小——所以**和 `rounds.jsonl`(round 正文)一个处理法**:**落文件、不进 SQLite**。一次「重读一遍」= 一个文件,append-only。

```
sessions/<source>/<sid[0:2]>/<sid>/
  rounds.jsonl              ← v3:round 正文(canonical · append-only)
  marks/
    pass-1.jsonl            ← 第一遍读这段对话的 mark(每行 = 某 round 这一遍提交的 YAML 数组)
    pass-2.jsonl            ← 第二遍重读……新感悟进新文件,旧文件不动
    …
```

一行 = 某 round 在这一遍提交的那份 mark(YAML 数组解析后存 JSON;`questions[]` 是写入时解析 `#…？` + 撞库的结果):
...
{"round_index": 37,
 "items": [
   ...
   {"id": 2, "mark": "这段在排查 EMFILE,跟句柄上限有关。", "questions": []}
 ],
 "created_at": "2026-06-16T08:30:00Z"}
```

`id`(`1`/`2`)= 这条 mark 在 **session 内**的序号(append 顺序、session 内唯一),寻址 `sess_<sid>#<id>`(如 `sess_def456#1`)。**不是** `mark_<ULID>` 那种独立 id。
...
- **mark ↔ card 的链就写在那条 mark 的 `questions[]` 里**;`card_sessions`(SQLite)是**从这些 `questions[]` 派生**的可 join 索引(`card_id` + `session_id` + `mark` 序号 + `position_id`)。

> **wire 格式 vs 存储**:提交进来的是 **YAML**(对 AI / 人最省转义);canonical 文件按上面的 jsonl 存(一行一 round,数组内嵌),跟 v3 sidecar 一个路子。两者一一对应,YAML 只是入口形态。
>
> **命名小注**:`pass-N` 而非 `vN`——本仓 `v` 已被迁移版本号和产品代号(v3/v4)占了两层意思,「第几遍」用 `pass-N` 不撞名。

---

## 7. mark 的寻址:`<session_id>#<n>` —— session 的附属,不是一等 id

mark **不进 id 前缀体系**(没有 `mark_` 前缀、没有 `IdKind.MARK`、不 mint 独立 ULID)。它是 **session 的附属**,靠 session 的 id + `#` + 序号寻址:

```
sess_def456#1     ← sess_def456 的第 1 条 mark(= 第一次标注)
sess_def456#2     ← 第 2 条
```

- **`read sess_def456#1`**:读时先按 `#` 拆成 `(session_id, n)`——`parse_id` 见到 `card_` / `pos_` / `sess_` 等前缀照旧判型;**额外认 `sess_…#<n>` 这种带 `#` 分片的形态 → 定位「这个 session 的第 n 条 mark」**,返回它(`mark` 文本 + 所属 round + `questions[]` + 它建/连了哪些卡)。不需要新前缀,复用 session id + 分片。
...
- **`id`(session 内序号)谁分配**:append-only 下天然单调,**系统按 append 顺序分配下一个序号**最稳(提交方不必知道当前计数);若提交方自带序号则校验单调、不允许跳号 / 复用。`#n` 的 `n` 是 session 内唯一。
