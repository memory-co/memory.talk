...
3. mark ↔ card 的关联**写进那一轮的 `issues[]`**(`card_id` + `is_new` + `indexes`:建新卡 / 关联老卡 + 这问题 grounding 的那几轮,默认就是这条标注的 `index`、也可给范围)——这是 **canonical**(file,§6);SQLite 里的 `card_sessions` 表就是**从这些 `issues[]` 派生出来的可 join 索引**。
...
rounds:                     # 从 index 1 起,严格递增,覆盖 ≥90%
  ...
  - index: 37
    comment: |
      配 pty 时用户突然提了 tmux。#为什么 pty 会让用户想到 tmux？
      他其实想要可重连会话。
    issues:                 # 这一轮 comment 里 #…？ 的撞库结果
      - issue: 为什么 pty 会让用户想到 tmux
        card_id: card_01j…
        is_new: true
        indexes: "36-37"     # 这问题 grounding 的 round(s)(默认 = 本轮 index,也可范围)
...
> **提交 vs 落盘**:一次提交是**一份 YAML = 一次 mark**(顶层 `last_index` + `description` + `rounds: [{index, comment?}, …]`,§2);服务端给它分配 `m<n>`,整份落成 **一个** `marks/m<n>.yaml`(`issues` 写入时现解析,挂在各 round 下)。wire 与盘上都是「一次 mark 一份 YAML」。
