...
- **mark ↔ card 的链写在那条 mark 的 `questions[]` 里**(canonical · YAML);[`card_sessions`](../../structure/v4/card-session.md) 是从这些 `questions[]` 派生的 card→session 可 join 索引(`card_id` + `session_id` + `mark` id,**无 `position`、无 `indexes`**——答案级出处在另一条链路 `position_sessions`)。**`session_marks` 管 mark 自己的元信息,`card_sessions` 管 mark→card 的出处边**,两张表各司其职。
...
- **`read sess_def456#m1`**:读时先按 `#` 拆成 `(session_id, mark_id)`——`parse_id` 见到 `card_` / `sess_` 等前缀照旧判型;**额外认带 `#` 分片的形态:`sess_…#m<n>` → 定位「这个 session 的 mark `m<n>`」(直接读 `marks/m<n>.yaml`),`card_…#p<n>` → 定位「这张卡的 Position `p<n>`」(同构)**,返回它(`description` 场景 + `last_index`〔标它时 session 到第几轮〕 + `mark` 文本 + 所属 round + `questions` + 它建/连了哪些卡)。不需要新前缀,复用 session id + 分片。
