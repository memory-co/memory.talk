...
一个 `mN.yaml` = 一条 mark,**用 YAML 存**(跟提交同语种,免转换);`description` / `last_index` 从这次提交带下来,`issues` 是写入时解析 `#…？` + 撞库的结果:
...
> **提交 vs 落盘**:一次提交是**一份 round 级 YAML**(顶层 `last_index` + `description` + `marks: [{id, mark}, …]`,§2);写入时把 `marks` **拆开**——每条按它的 `id` 落成 `marks/<id>.yaml`,把这次提交的 `last_index` / `description` 一并带进每个文件(`issues` 现解析)。wire 是一份带头信息的数组、盘上是一文件一 mark,两边都用 YAML。
