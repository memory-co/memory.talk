# session mark — 以写代读的 session 标注(v4 设计)

> **状态:已实现并发布(v1.1.x)。** 这是 v4 抽卡的**写路径前端**:总结一个 session 时,不让 AI 整段扫一遍就下结论(必走神),而是**逐 round 读、每轮写一条标注(整份 = 一次 mark)**——**以写代读**,逼它真读;标注里用 `#…？` 把问题就地标出来,写入时**自动建卡 / 关联老卡**,卡的出处**精确指向那次 mark(+ `#…？` grounding 的那几轮 `indexes`)**。于是「建卡」不再是一个单独的、容易脑补的步骤,而是**认真读的自然副产物**。
>
> 跟 [card.md](card.md) 的关系:card.md §6 讲写路径的**逻辑**(旁白→惊讶→question、命门 = 惊讶 grounding 在检索);**本篇讲它落地成什么 ergonomics** —— mark 就是那个「旁白」,`#…？` 就是那个 question,「新卡 vs 关联」复用 card.md §6 的三岔,出处落 [`card_sessions`](../../structure/v4/card-session.md)。
>
> 本篇取代旧设计 `session-annotation.md`(annotation→**mark**、自由 jsonl 行→**一份 YAML(标注整个 session)**、`#问题` 到行尾→**`#…？` 收尾**、出处指 (session,round)→**指 (mark, round)**;一份 YAML = 一次 mark,内含逐 round 标注)。

相关:
- card 设计(问题图 + 写路径逻辑,本篇是它的写路径前端): [card.md](card.md)
- 出处数据结构(`card_sessions`,本篇把它改成指 mark): [../../structure/v4/card-session.md](../../structure/v4/card-session.md)
- explore 抽卡工作区(在先验 session 上跑 mark 的工作台): [../v3/explore.md](../v3/explore.md)
- session rounds 写入(append-only;mark 是 session 级、只读 round 不改): [../v3/session-rounds-write.md](../v3/session-rounds-write.md)
- file-canonical 模式(mark = sidecar 文件 + SQLite 派生索引): [../v3/file-canonical-pattern.md](../v3/file-canonical-pattern.md)

---

## 1. 问题:整段总结必走神 —— 用「以写代读」治

把一个几十轮的 session 丢给 AI、让它「总结出 cards」,它会**走神**:扫读、跳读、脑补,漏掉真正有信息的轮次,还容易把没发生的事总结进去(confabulation)。根因是**读和写脱钩**——它不必逐轮证明自己读过,就能产出一段看似合理的总结。

**以写代读**(用写驱动读):**逐 round 读**——交互窗口逼你一轮轮往前过、不跳读,边读边把感悟写成 **mark**。你没法跳过 round 37 还把它读懂——**逐轮过 = 「读过」的证据**。把「总结」从一次性顶层扫读,改成**自底向上、逐轮沉淀**,走神的空间被压没了。

| | 整段总结(v3 抽卡) | session mark(逐 round 读,本篇) |
|---|---|---|
| 读写关系 | 脱钩:扫一遍 → 直接产 card | 绑定:逐 round 读 + 写 mark = 强制逐轮过 |
| 失败模式 | 走神 / 跳读 / confabulation | 想脑补也得先逐轮落字 |
| 产出粒度 | 顶层几张 card | 逐 round 标注(一次 mark),卡是副产物 |

> **逐 round 读 + mark ≠ card.md §6 否决的 v0**。v0 的毛病是「每条旁白都想直接变成 card」→ 噪声。这里**mark 本身不变卡**:只有 mark 里 `#…？` 出来的问题、且经检索判为**新问题**(miss)才建卡(§3),噪声在下游被过滤掉。逐轮过、把感悟写下来,是为了**以写代读防走神**,不是把每轮都当信号。

---

## 2. 一份 YAML = 一次 mark:逐 round 的标注、≥90% 覆盖

**一份提交 = 这个 session 的一次 mark。** mark = 一次「完整读一遍 + 逐轮标」的产物;它的 id `m<n>` **由服务端自动分配**(第一遍 `m1`、第二遍 `m2`…,session 内递增),**用户不填**。一次 mark 里,对这个 session 的**每个 round** 都有一条标注(理想全覆盖,实际 ≥90%)。

```yaml
# 一份提交 = sess_def456 的一次 mark(服务端自动给它 m1;下一遍 m2)
last_index: 41          # 乐观锁:提交时我读到的 session 最新 round index
description: 在配 pty、用户突然提 tmux 的那几轮——想搞清他到底要什么
rounds:                 # 从 index 1 起逐 round 往下,严格递增,覆盖 ≥90%
  - index: 1            # 从 session 第一轮开始读
  - index: 2
    comment: 用户要给 pty 配上终端。
  - index: 3
  # …一路顺着往下读…
  - index: 37
    comment: |
      配 pty 时用户突然提了 tmux。#为什么 pty 会让用户想到 tmux？
      他其实想要的是「可重连的会话」,而不是 pty 本身。
  - index: 38           # 读了,没东西标(只占覆盖)
  # …一直到 ≥90%…
```

- **一份 YAML = 一次 mark,id `m<n>` 由服务端自动分配**:整份提交就是「这个 session 的一次读 + 标」。mark 是 session 的附属,寻址 `<session_id>#m<n>`(如 `sess_def456#m1`,§7)。**用户不填 id**——填了反而是上一稿的错。
- **`rounds` = 逐 round 的标注**,每条 `{index, comment?}`:
  - **`index` 指向 round**(第几轮),**不是 mark 的身份**(mark 的身份是 `m<n>`)。**一次 mark 里每个 round 一条。**
  - **`index` 从 `1` 起、严格递增、不重复**:从 session **第一轮**开始顺着读(`rounds` 首条必须 `index: 1`)——若从 `index: 36` 起,就等于**偷偷跳过了前 35 轮**,不算读完整条 session。允许 gap(中间 ≤10% 没标的轮可不出现),但**首轮必须是 1**。乱序 / 重复 / 不从 1 起 → 整份拒绝。
  - **`comment` = 这一轮的感悟(可选)**,其中 `#…？` 就地标问题(§3)。**省略 = 这轮读了、没东西标**——只占覆盖,**不解析 `#…？`、不建卡**。
  - **issue 两种来路**:① 默认从 `comment` 的 `#…？` **自动解析**(grounding = 本轮 `index`);② 也可在条目里**主动声明** `issues: [{issue: <文本>, indexes?: <grounding 的轮>}]`——这时 `indexes` 才有用(把问题 ground 在指定的几轮,而非默认本轮)。
  - **提交方只给「问题」,不给「结果」**:`comment` / `issue` / `indexes` 是提交方写的;每个问题撞库后落在**哪张卡**(`card_id`)、是**新卡还是连老卡**(`is_new`),由**服务端 embed 撞库后回填**——**提交方不填、也不需要知道**(那是写入的产物,落在 canonical 文件里,见 §6)。
- **round 覆盖率 ≥ 90%(以写代读的命门)**:「以写代读」要的是**读完整条 session**;只标 1–2 轮的长 session 等于没读完。所以 `rounds` 里 distinct `index` 数 ÷ `last_index` 必须 **≥ 90%**(`covered >= ceil(0.9 × last_index)`,阈值常量 `MARK_COVERAGE_THRESHOLD = 0.9`,在 `service/session_marks.py` 好调)。不够 → **整份拒绝**(400,`coverage 23% (12/52 rounds) < 90%`),**在任何写入之前**。交互模式逐轮走到底(每轮标或留空)自然 100%。
- **`description`** = 这次 mark 的场景(顶层一句话,跟着落盘),让事后回看能秒懂当时心境。
- **`last_index` = 乐观锁**:提交时填我读到的 session 最新 round 数;跟服务端当前比,**不一致 = 标注期间又来了新 round** → 拒绝(带新轮重读再标)。也落进 `session_marks`,记「这次 mark 是在 session 长到第几轮时标的」。
- **`#…？` 只在有 `comment` 的条目上解析**:miss 建新卡 / hit 连老卡,出处 `card_sessions(card_id, session_id, mark, indexes)` —— `indexes` = 这问题 grounding 的 round(s)(默认就是它那条标注的 `index`,也可给范围)(§4)。
- **append-only + 可标很多遍**:mark 只增不改;同一 session 重读 = **追加新 mark**(`m2`、`m3`…),后一遍带着更多上下文 / 已建的卡回头看,往往更深;`m<n>` session 内单调、不复用。

> **`index` 不是 mark 的身份。** mark 的身份是 `m<n>`(一次 mark 一个),`index` 只是这次 mark 里**每条标注指向哪一轮**。若让 `index` 当身份(一轮一个 mark),整套「以写代读」(一次读一整遍)就没意义了。
>
> 为什么逐 round + YAML:一次 mark 就是「完整读一遍」的产物,逐 round 一条天然对齐「以写代读」;`index` 严格递增 = 顺着读的轨迹,覆盖率一眼可算。YAML 多行 `|` 块对「带换行、带 `#`、带引号」的 `comment` 最省转义。

---

## 3. `#…？` → 自动建卡 / 关联老卡

mark 内容里只要冒出一个问题,就用 **`#…？` 语法**就地标:`#` 起头、**到第一个 `？`(或半角 `?`)收尾**,中间就是问题文本。

```
配 pty 的时候用户突然提了 tmux。#为什么 pty 会让用户想到 tmux？他其实想要可重连会话。
                              └──────────── 一个问题 ────────────┘
```

写入这次 mark 时,系统对**有 `comment`** 的每条 round 的 `comment` 文本(无 `comment` 的轮 → 跳过这步,不建卡 / 不连卡):

1. **解析**出所有 `#…？` 问题(`#` 起、`？`/`?` 止;一条 mark 里可有多个)——于是「这条 mark 里有几个问题」是**自动数出来的**,不靠 AI 自己报。
2. 每个问题走 [card.md §6](card.md#6-写路径每轮对话怎么变成卡--旁白--惊讶--question) 的三岔(embed 问题文本 → 撞 `cards` / issue 向量库):
   - **新问题(检索 miss)→ 建一张新卡**(`issue` = 问题文本,还没答案、在等答案)。
   - **老问题(检索 hit)→ 关联**到那张已有卡(**不动老卡任何分数**——相关性只在召回时算,见 [card.md §3](card.md#3-第二推credence--现算的质量分相关性只在召回时算))。
   - 两种情况都**记一条 [`card_sessions`](../../structure/v4/card-session.md)**:`card_id` + `session_id` + **`mark`(那次 mark 的 id `m<n>`)** + **`indexes`(`#…？` grounding 的那几轮,可范围)**,这就是「这次 mark 在哪几轮启发 / 碰到了这张卡」的出处(§4)。
3. mark ↔ card 的关联**回填进那一轮的 `issues[]`**(服务端写 `card_id` + `is_new`〔建新卡 / 连老卡〕;`indexes` 是提交方给的 grounding 那几轮,默认 = 本轮 `index`、也可范围)——这是 **canonical**(file,§6);SQLite 里的 `card_sessions` 表就是**从这些 `issues[]` 派生出来的可 join 索引**。

**这一步把「判断惊讶」从 AI 手里拿走了**:AI 只管**读 + 自然地 `#…？` 标问题**;「这是不是个新问题(= 惊讶)」由**检索**算(miss = 新卡)。这正是 [card.md §6 命门](card.md#6-写路径每轮对话怎么变成卡--旁白--惊讶--question) 最自然的落地——**惊讶 grounding 在检索,而不是 AI 自评**。

---

## 4. mark 建的是 **card→session** 出处(`mark` 必选);position→session 的 `mark` 可选

**两条出处链路要分清**(别混成一张表):

| 链路 | mark 颗粒度 | 谁建的 | 表 |
|---|---|---|---|
| **card → session** | **经 mark + indexes** | 某次 mark 的 `#…？` 建 / 连了这张卡 | `card_sessions(card_id, session_id, **mark, indexes**)` |
| **position → session** | **可选**(主出处是 `indexes`) | `card position --source <sid>:<idx>` 给答案标来源 | `position_sessions(card_id, position, session_id, **indexes**, mark?)` |

mark 写路径**只产 card→session**:`#…？` miss 建新卡 / hit 连老卡,各记一条 `card_sessions`,出处就是**那条 mark**。**同一 card↔session 可以有多条**——不同 mark(不同感悟)各碰到 / 建了这张卡,各记一条:

```json
{
  "card_id": "card_01jz8k2m",
  "session_id": "sess_def456",
  "mark": "m1",
  "indexes": "36-37",
  "created_at": "2026-06-18T14:30:00Z"
}
```

读「`sess_def456` 的**第 1 次 mark**(`sess_def456#m1`)的 `#…？`(grounding 36–37 轮)建 / 连了 `card_01jz8k2m`」。**不指向具体 Position**——这是卡级关系。

**为什么两条链路 mark 颗粒度不同**:
- **card→session 必选 mark**——卡是 mark 里 `#…？` **建/连**出来的,要支撑 `#…？` 的**双向关联**(card↔mark 互查),所以非到 mark 不可。
- **position→session 可选 mark**——Position(答案)**本身就是「观点」主体**,它的主出处是「从哪几轮长出来的」(round `indexes`);到底是不是某条 mark 触发的可以**顺手记一下(可选)**,但不强求——再硬挂一条 mark 容易跟「答案本身这个观点」重复。所以答案出处走 [`position_sessions`](../../structure/v4/position-session.md):`indexes` 必填、`mark` 选填。

### `card_sessions` 形态(设计)

```sql
-- card → session 出处:哪次 mark 的哪一轮 #…？ 建/连了这张卡;同一对可多条
CREATE TABLE card_sessions (
  card_id      TEXT NOT NULL,   -- 哪张卡
  session_id   TEXT NOT NULL,   -- 哪个 session(扁平,可 join;无 FK)
  mark        TEXT NOT NULL,   -- 哪次 mark(m1 / m2 …;寻址 = session_id#mark)
  indexes     TEXT NOT NULL,   -- #…？ grounding 的 round(s)(可范围,语法同 reviews.indexes:"37" / "36-37" / "3,7,12")
  created_at  TEXT NOT NULL,
  PRIMARY KEY (card_id, session_id, mark)
);
CREATE INDEX idx_card_sessions_session ON card_sessions(session_id);
CREATE INDEX idx_card_sessions_mark    ON card_sessions(session_id, mark);
```

- 出处记到**那次 mark + `#…？` grounding 的 `indexes`**:这问题从哪几轮冒出来的,一清二楚。
- 旧设计的 `(session_id, indexes)` + 指向 Position 的一张表**拆成两条链路**:卡级 → `card_sessions`(经 `mark`,带 `indexes`,本表);答案级 → [`position_sessions`](../../structure/v4/position-session.md)(`indexes` 必选 + `mark` 可选)。
- **PK `(card_id, session_id, mark)`、不指向具体 Position**:`indexes` 是数据列(不进 PK);同一卡可被同一 session 的**多条不同 mark** 各记一条。

> **已实现**:代码现为拆分版 —— `card_sessions(card_id, session_id, mark, indexes)`(卡级,经 mark)+ `position_sessions(card_id, position, session_id, indexes, mark?)`(答案级,经 indexes),与本篇、[card.md §8](card.md#8-存储file-canonical-罐--sqlite-瘦索引--searchbase)、[card-session.md](../../structure/v4/card-session.md)、[position-session.md](../../structure/v4/position-session.md) 一致。

---

## 5. mark 建问题,答案 / 表态各走各的

`#…？` 建 / 连的是**卡(问题)**,一开始**只有问题、还没答案**。三条线分开,别都塞进 mark:

| 动作 | 命令 / 入口 | 落哪 | 出处 |
|---|---|---|---|
| **提问 → 建/连卡** | `session mark` 里的 `#…？` | 卡 + `card_sessions` | **mark**(`sess#m<n>`) |
| **给问题加答案** | [`card position --claim … --source <sid>:<idx>`](../../cli/v4/card.md#card-position) | Position + [`position_sessions`](../../structure/v4/position-session.md) | **round `indexes`**(mark 可选) |
| **对答案顶/踩** | [`card review --argument … --cite <sid>:<idx>`](../../cli/v4/card.md#card-review) | `reviews` | **round `indexes`**(证据) |

- **提问**:某条 mark 的 `#…？` miss → 建新卡(只有问题)/ hit → 连老卡;记 `card_sessions`(出处 = 那条 mark)。
- **答它**:`card position` 给这张卡加一个 **Position**(答案文本内联),它的出处 = `position_sessions`(那个答案从哪几轮长出来,`indexes` 必填;`mark` 可选,见 §4)。
- **评它**:`card review` 顶/踩某 Position,`argument` 累成计数,按 `credence`(现算)排序——回到 [card.md §3](card.md#3-第二推credence--现算的质量分相关性只在召回时算)。

> mark 写路径**只负责把问题从认真读里捞出来**(`#…？` → 卡);答案和表态是后续显式动作,各有各的出处链路。这正是「启发用 mark、出处用 indexes、证据用 indexes」三线分工。

所以一条龙都长在「逐 round mark」这个自然动作上:**mark 提问(`#…？`)→ 建卡(只有问题)+ 记 `card_sessions` 出处(指 mark);mark 答它 → Position;证据顶踩 → credence**。

---

## 6. 存储:mark 文件(canonical)+ `session_marks`(SQLite 派生索引)

跟 session 一个分法:**正文落文件、元信息进表**。一次 mark 的逐 round 标注**落 YAML 文件**(canonical,像 `rounds.jsonl`);mark 的**元信息**(`m<n>` / `last_index` / 时间)进 **`session_marks` 表**(SQLite 派生索引,像 `sessions` 表),用来撑乐观锁、寻址、反查。

### 文件(canonical):一次 mark 一个 `m<n>.yaml`

**一次 mark 一个文件,文件名就是它的 id `m<n>`**(服务端分配),append-only:

```
sessions/<source>/<sid[0:2]>/<sid>/
  rounds.jsonl              ← v3:round 正文(canonical · append-only)
  marks/
    m1.yaml                 ← 第 1 次 mark(一次读 + 逐 round 标;文件名 = id)
    m2.yaml                 ← 第 2 次(重读续标)
    …                       ← 接着加 m3、m4…
```

一个 `mN.yaml` = 一次 mark,**用 YAML 存**(跟提交同语种,免转换);`description` / `last_index` 从这次提交带下来,`rounds` 是逐 round 的标注,每条带 `index` + 可选 `comment` + 写入时解析 `#…？` 撞库得到的 `issues`:

```yaml
last_index: 41
description: 在配 pty、用户突然提 tmux 的那几轮——想搞清他到底要什么
rounds:                     # 从 index 1 起,严格递增,覆盖 ≥90%
  - index: 1                # 从第一轮开始
  - index: 2
    comment: 用户要给 pty 配上终端。
  # …顺着往下…
  - index: 37
    comment: |
      配 pty 时用户突然提了 tmux。#为什么 pty 会让用户想到 tmux？
      他其实想要可重连会话。
    issues:                 # 服务端解析 #…？(或主动声明的 issue)撞库后回填
      - issue: 为什么 pty 会让用户想到 tmux
        card_id: card_01j…   # ← 服务端撞库回填(提交体里没有)
        is_new: true         # ← 服务端回填:true=建新卡 / false=连老卡(提交体里没有)
        indexes: "36-37"     # 这问题 grounding 的 round(s)(提交方给;默认 = 本轮 index,也可范围)
  - index: 38               # 读了没东西标(只占覆盖)
created_at: 2026-06-16T08:30:00Z
```

> **无 comment 的轮也落盘**:`{index: 36}` 这种「读了没东西标」的条目照样写进 `rounds`(没有 `comment` / `issues`),这样读回 & append-only 记录完整,且它的 `index` 仍计入覆盖率。`#…？` 只在有 `comment` 的轮上解析。

> **提交 vs 落盘(关键:谁填什么)**:提交体的 round 条目是 `{index, comment?, issues?: [{issue, indexes?}]}` —— 提交方只给 `comment` / `issue` / `indexes`。写入时服务端:分配 `m<n>`、把 `comment` 里 `#…？` 与主动声明的 `issue` 都拿去 embed 撞库、**回填每条 issue 的 `card_id` / `is_new`**,整份落成 **一个** `marks/m<n>.yaml`(上面那个含 `card_id`/`is_new` 的形态就是**落盘 canonical**,不是提交体)。**`card_id` / `is_new` 只在盘上有,提交方不填、不需要知道。**

### 表(派生索引):`session_marks`

```sql
-- mark 元信息(派生自 marks/*.yaml);撑乐观锁 + 寻址 + 反查。mark 正文不进表(留 YAML)
CREATE TABLE session_marks (
  session_id  TEXT NOT NULL,        -- 哪个 session
  mark        TEXT NOT NULL,        -- mark id(m1 / m2 …);寻址 = session_id#mark
  last_index  INTEGER NOT NULL,     -- 标这条 mark 时 session 的最新 round index(乐观锁基线 + 当时情况)
  created_at  TEXT NOT NULL,
  PRIMARY KEY (session_id, mark)
);
CREATE INDEX idx_session_marks_session ON session_marks(session_id);  -- 列一个 session 的所有 mark / 取最大序号
```

- **无 FOREIGN KEY**(SQLite 派生索引,容忍悬空;canonical 是 `marks/*.yaml`,表丢了能从文件重建)。
- `mark` 正文**不进表**(大文本留 YAML);表只放**能 join / 能比 / 能排**的元信息。
- 取「session 下一个 mark 序号」「session 当前 `last_index` 基线」「某条 mark 标在第几轮」都走这张表,不必扫文件。

### 乐观锁:`last_index` 怎么校验

提交一份 mark 时,系统:

1. 读 session 当前**最新 round index**(`max(round_index)`,来自 sessions/rounds)。
2. 跟提交里的 `last_index` 比:
   - **相等** → 进下一步校验。
   - **不等**(session 又长了新 round)→ **拒绝**(409 / conflict):你这份是基于旧视图标的,带着新 round **重读再标**。
3. **id + indexes 校验**:每条 `id`(`m<n>`)单调、不跳号 / 不复用,`indexes` 必给且合法。
4. **覆盖率校验(以写代读)**:所有 mark 的 `indexes` 并集覆盖 `[1, last_index]` 的 round 数 < 90%(`ceil(0.9 × last_index)`)→ **拒绝**(400,`coverage … < 90%`),**在任何写入之前**。
5. 全过 → 放行:服务端分配 `m<n>`,整次 mark 落成一个 `marks/m<n>.yaml` + `session_marks` 插一行(一次 mark 一行)。
6. 同一 session 并发两份提交,只有 `last_index` 仍等于当前的那份能进;另一份因序号 / 基线已变而被挡——**乐观锁,不上行锁**。

### 不变性(照搬 round 正文那套)

- **一条 mark 一个文件,写一次就不动**(append-only);改主意 = 加新 `m<n>`,不覆盖旧文件 / 旧表行。
- **序号单调、session 内唯一**:`m1`→`m2`→…,不复用、不跳号;重读续标接着往后加。
- **mark 不进向量库**:进 `cards` 向量库的是它 `#…？` 出来的**问题(卡)**,卡走正常 card 写路径。
- **mark ↔ card 的链写在那次 mark 各 round 的 `issues[]` 里**(canonical · YAML);[`card_sessions`](../../structure/v4/card-session.md) 是从这些 `issues[]` 派生的 card→session 可 join 索引(`card_id` + `session_id` + `mark` + `indexes`〔`#…？` grounding 的 round(s)〕,**无 `position`**——答案级出处在另一条链路 `position_sessions`)。**`session_marks` 管 mark 自己的元信息,`card_sessions` 管 mark→card 的出处边**,两张表各司其职。

---

## 7. mark 的寻址:`<session_id>#<id>` —— session 的附属,不是一等 id

mark **不进 id 前缀体系**(没有 `mark_` 前缀、没有 `IdKind.MARK`、不 mint 独立 ULID)。它是 **session 的附属**,靠 session 的 id + `#` + mark id(`m<n>`)寻址:

```
sess_def456#m1    ← sess_def456 的第 1 条 mark(= 第一次标注;盘上 marks/m1.yaml)
sess_def456#m2    ← 第 2 条
```

- **`read sess_def456#m1`**:读时先按 `#` 拆成 `(session_id, mark_id)`——`parse_id` 见到 `card_` / `sess_` 等前缀照旧判型;**额外认带 `#` 分片的形态:`sess_…#m<n>` → 定位「这个 session 的 mark `m<n>`」(直接读 `marks/m<n>.yaml`),`card_…#p<n>` → 定位「这张卡的 Position `p<n>`」(同构)**,返回它(`description` 场景 + `last_index`〔标它时 session 到第几轮〕 + `mark` 文本 + 所属 round + `issues` + 它建/连了哪些卡)。不需要新前缀,复用 session id + 分片。
- **`card_source` 指 `(session_id, mark)`**(§4):出处从「指 session+round 区间」升级成「指那条 mark」,点开就是「当时怎么想到这个问题的」。
- **review / Position 的 evidence** 仍引 round(`indexes`),但「出处 / 启发」这条线统一指 mark——**启发用 mark(`sess#n`),证据用 round(`indexes`)**,各司其职。

> 为什么不给 mark 独立 id:mark 离开 session 没有意义(它是「读这个 session 的第几条感悟」),天然从属于 session。用 `sess_…#n` 这种**分片寻址**,既能精确点到、又不往全局 id 空间里多塞一类前缀——跟「章节#段落」一个直觉。

---

## 8. 与 v3 / card / explore 的关系

- **card.md**:本篇是 card 写路径的**前端 ergonomics**;「新卡 vs 关联」的判定、卡 / Position / credence 全在 card.md,本篇不重复,只负责「从认真读里**自然冒出** `#…？`」+「出处指 mark」。
- **explore.md**:explore 是**在先验 session 上跑 mark** 的工作台——你在 explore 目录里逐 round 读、写 mark,`#…？` 建 / 连卡,产物盖 `explore_id` 戳。mark 就是 explore 里「抽卡」的那个具体动作(v4 版工作台另案,见 cli README)。
- **v3 session / round**:一次 mark 是**对整个 session 的一次读 + 逐 round 标注**(每条标注绑一轮 `index`,整次覆盖 ≥90% 的轮);它只读 `rounds.jsonl`(**不改 round 本身**,round 仍 append-only);mark 落在 session 目录下**新增的 `marks/` sidecar**,不动 v3 既有结构。
- **v3 tag**:`#…？` 是**类 tag 的就地标记**,但语义不同——v3 `key=value` tag 是死字符串;`#…？` 写入时被**解析成卡 id**(建 / 连),是活的。

---

## 9. 诚实边界 + 待定

- **核心价值是「以写代读」,不是 mark 本身**:mark 的存在意义首先是**逼 AI 逐轮真读**(防走神),其次才是留痕 + 精确出处。哪怕 mark 写完没人再看,这个 forcing function 已经赚到了。
- **`#…？` 不判惊讶,检索才判**:AI 标 `#…？` 只是「我这觉得有个问题」;到底是不是**新**卡(惊讶)由检索 miss/hit 决定。别让 AI 自己说「这是新发现」。
- **别把走神搬进 mark**:逐 round 标注能防顶层扫读的走神;但若允许「一条标注糊一大段多轮」,走神会**搬家**。约束:`rounds` 里**一轮一条**(`index` 严格递增),别把多轮糊进一条 `comment`。

**待定**:

- **`？` 分隔**:`#…？`(全角)默认;半角 `?` 也认。若问题文本里本身要带问号,用 `#{…}` 包(行内更稳)留作备选。
- **同一问题的归一化**:`#为什么 pty 会让用户想到 tmux？` 和 `#pty 为啥让人想到 tmux？` 该不该 match 到同一张卡——靠 embed 近重还是更严的判定,本质是 card.md §6「`#` 问题撞到哪张卡」的匹配阈值。
- **`id`(`m<n>`)谁给**:**服务端自动分配**——一次 mark(一份提交)= 一个 `m<n>`(session 内递增:第一遍 `m1`、续标 `m2`…)。**提交方不填 id**(填了是上一稿的错)。`m<n>` 在 session 内唯一,直接当文件名(§6)。
- **`pass` 怎么触发**:第二遍是人 / AI 主动发起,还是系统在「新建了 N 张卡后」提示回看?留作 explore 工作流细节。
- **mark 的最小产率**:是否每个 round 都必须有 mark(强制逐轮),还是允许「这轮无话可说」跳过——但要显式记「看过、空 mark」,别和「没看」混。
