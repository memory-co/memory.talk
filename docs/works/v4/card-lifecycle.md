...
1. **顶/踩(review)**:[`card review --position <card_id>#p<n> --argument <+1/0/-1> --cite <sid>:<idx>`](../../cli/v4/card.md#card-review)。`argument` 累成 `up/down/neutral_count`;`--cite` 的 round `indexes` 是**这次表态的证据**(append-only,错了再写一条反向)。
