...
session ──逐 round 打 mark──▶ #…？ ──撞 issue 库──▶  ① 卡(问题 Issue)
   ...
   └──主观分析、形成观点──▶ 写 Position(claim) ──引用 session 作证据──▶ ② 答案(Position)
                                                  └─ 出处:position_sessions(经 indexes,无 mark)
                                                          │
...
| 出处是什么 | **那条 mark**(`#…？` 在哪条感悟里冒出来)→ `card_sessions`(经 mark) | **答案从哪几轮长出来 + 引用哪些 session 作证据** → `position_sessions`(经 `indexes`,**无 mark**) |
| 可不可竞争 | 一个问题就是一个问题(撞重了就是同一张卡) | **多个答案并存竞争**,靠顶/踩的 credence 现算排序 |

> **为什么答案的出处不经 mark**:mark 是「认真读时冒出的感悟 / 问题」,本身已是一种「观点」。答案(Position)也是观点。若让答案的出处再挂一条 mark,等于**一个观点指向另一个观点**——重复、且把「客观问题来源」和「主观答案证据」两件事混了。所以答案直接 `position→session`(我从这几轮得出这个观点),不过 mark。
...
3. **引用 session 作证据 / 来源**:`--source <session_id>:<indexes>`(可多 session)→ 落 `position_sessions(position_id, session_id, indexes)`。**这是答案的出处:它从哪几轮对话长出来的**;**无 mark**(理由见 §1)。
...
| 线 | 答什么 | 经 mark? | 落哪 |
|---|---|---|---|
| **启发 / 问题来源** | 这张卡是哪条 mark 的 `#…？` 建/连的 | **是** | `card_sessions(card_id, session_id, mark)` |
| **答案来源** | 这个答案从哪几轮长出来 | **否**(`indexes`) | `position_sessions(position_id, session_id, indexes)` |
| **表态证据** | 这次顶/踩拿哪几轮当证据 | **否**(`indexes`) | `reviews(position_id, session_id, indexes, argument)` |

**问题用 mark、答案用 indexes、证据用 indexes** —— 因为问题是从认真读里**客观捞**的(指那条感悟最准),答案 / 证据是**主观引用对话**(指 round 区间最准)。
