...
4. **作答 → 写 Position**:[`card position --card <cid> --claim '<答案>' --source <sid>:<idx> ...`](../../cli/v4/card.md#card-position),`claim` 内联在 Position 上(append-only,创建即冻);`--source` 把**刚才分析过的那几个 session(哪几轮)**落成 `position_sessions(position_id, session_id, indexes, mark?)` —— 这就是答案的出处(`indexes` 必填、`mark` 可选)。
