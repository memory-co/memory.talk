...
> IBIS 关系集**不完备**,可按需补 `depends_on` / `part_of`。存储上是**两套抄**:**本体抄 IBIS**(节点 + 边的类型学),**投票那套机制抄 Stack Overflow**(**review 顶/踩 → credence**;但**不抄它的「采纳答案」**——IBIS 的 Issue 允许多 Position 长期竞争,「当下用哪个」只是召回时取 credence 最高的,不钉成已采纳状态)。这些 issue↔issue 的边落地为 `card_links`(§8):**主体卡 `card_id` + `target_id`**(非对称,没有 from/to)、**五类型全多值**(同一卡同一类型可多条,如 specializes 多父 → 统一边表、不内联成列)、`target_id` 前缀多态(`suggested_by` 可指 `pos_` Position)+ `target_type` 列(`card` / `position`,从前缀派生,便于过滤)。
...
`Review`(`target = position_id` + `indexes` + `argument`(`+1`/`0`/`−1`);沿用 v3 review,`argument≠0` 即 IBIS Argument)、`CardLink`(主体卡的边:`card_id` + `type` + `target_id` + `target_type`,对端 `card_` 或 `pos_`)、`CardSession`(card↔session 出处:`card_id` + `session_id` + `position_id` + `indexes`,支持多 session)各自是独立 BaseModel。**答案文本 `claim` 内联在 Position 上(不单独建 Claim 节点);治理只挂 Position;Card 只管问题 + 连边;证据/表态是 Review。**
...
CREATE TABLE card_links (                  -- 主体卡的有向边(= IBIS issue↔issue,因 card≡issue)
  ...
  target_type TEXT NOT NULL,              -- 'card' | 'position',从 target_id 前缀派生,便于按对端类型过滤
  created_at  TEXT NOT NULL,
...
insight 让出 `card`/`cards`/`reviews`/`cards` collection/`cards/` 目录、且 id 前缀已彻底改成 `insight_` 之后,**同一个迁移 `v3`** 就地建 v4:`cards`(问题)/ `positions`(答案 + 内联文本,`pos_`)/ `reviews`(对 Position 表态,`review_`,`argument` ±1/0)/ `card_links`(card↔card)/ `card_sessions`(card↔session 出处),**卡用 `card_` 前缀 + `card` 命令**。**v4 与 insight 物理隔离**(不同表、不同 collection、不同前缀 `card_` vs `insight_`),单一 `/v4` 前缀靠 `parse_id` 全局分派,互不干扰地共存。
