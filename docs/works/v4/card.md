...
- **端点**(复数):`/v4/cards`(建卡 / 列卡,只管 issue)、`/v4/cards/{id}/positions`(给卡加答案)、`/v4/positions/{pid}/reviews`(表态 ±1/0)、`/v4/card-links`、`/v4/card-sessions`(出处)、`/v4/read`(按前缀读 card_/pos_/sess_)、`/v4/search`(撞问题检索 + DSL)。读路径 `/v4/recall` 走「撞问题 → 取 Position → 现算校验分排序(平手按最近更新)→ 连 `scope` 注入」(scope 是软提示,不挡)。
- **CLI**:`memory.talk card`(v4 复用此命令),管 **Card(问题)** 和 **Position(答案)** 两类对象——**建问题和加答案是两条命令**(Card 和 Position 是两个对象,且没答案的问题也合法,见 §2)。最小面:
  - `card create '<问题文本>' [--card_id]` → 建一张卡(**只有问题**,可无答案)。问题走位置参数。
  - `card position <cid> '<答案文本>' [--source <sid>:<indexes> ...] [--scope '<场景>']` → 给卡加一个 **Position(答案)**;卡 + 答案走位置参数;`--source` 可多次,每个 session 落一条 `card_sessions`(出处,支持多 session)。
  - `card review <position_id> <+1|0|-1> --cite <sid>:<indexes>` → 对某个答案表态(沿用 v3 review,target 从 card 变 position;`review` 并入 `card` 组,不独立)。
  - `card link create <from> <type> <target>` / `card link list <card>` → 卡间 IBIS 边(`card_links`);连边也收在 `card` 组下。
