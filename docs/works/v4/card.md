...
> **别再把 Position 当成整张卡**(这是上一稿的错):**card = 一个 Issue + 若干 Position**。Position 是**答案候选**,卡是**问题 + 这些候选的集合**。「哪个 Position 是这张卡的答案」不是写死的、也不冻成状态——**召回时取 credence 最高的 active Position** 就是当下要用的那个(随顶/踩浮动)。**不设 `accepted` 标志**:IBIS 是复杂问题的本体,一个 Issue 本就允许多个 Position 长期并存竞争,不该被钉成「已采纳/已解决」(呼应上面「不需要 open/closed」)。
>
...
- **「本卡当下用哪个答案」不存字段、不钉状态**:召回时取 `credence` 最高的 active Position 即可——不靠创建时拍板,也不设 `accepted` 标志(那会把一个本该长期竞争的 Issue 钉成「已解决」,与 IBIS 相悖)。哪个浮上来纯随顶/踩变。
