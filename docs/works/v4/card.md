...
>
> **旁白机制也正是 `card_sessions`(和 `reviews`)的写入口**:每条 `card_sessions` / review 的 `(session_id, indexes)` 就是那几条**被标注的 round**——`card_sessions` 记「这个 session 的这些旁白**启发/生出**了卡/答案」(miss→新卡、冲突→新竞争 Position),review 记「这些旁白对某答案**顶/踩/中立**」(hit)。所以 `card_sessions.indexes` 永远指向 session-mark 里的旁白 round,不是凭空来的。
...
`Review`(`target = card_id` + `position`(`p<n>`) + `indexes` + `argument`(`+1`/`0`/`−1`);沿用 v3 review,`argument≠0` 即 IBIS Argument)、`CardLink`(主体卡的边:`card_id` + `type` + `target_id` + `target_type`,对端 `card_` 或 `card_…#p<n>`)、`CardSession`(card↔session 出处:`card_id` + `session_id` + `position`(`p<n>`,可空) + `indexes`,支持多 session)各自是独立 BaseModel。**答案文本 `claim` 内联在 Position 上(不单独建 Claim 节点);治理只挂 Position;Card 只管问题 + 连边;证据/表态是 Review。**
...
CREATE TABLE card_sessions (               -- card ↔ session 出处(哪个 session 启发了这张卡/哪个答案);支持多 session
  ...
  position    TEXT NOT NULL DEFAULT '',    -- 启发了哪个答案(卡内序号 p<n>;'' = 关联到问题/卡本身)
  indexes     TEXT NOT NULL DEFAULT '[]',  -- 那个 session 里**标了 #问题 的旁白 round**(同 reviews.indexes;源自 session-mark,§6)
  created_at  TEXT NOT NULL,
  PRIMARY KEY (card_id, session_id, position)  -- 同一卡可挂多 session;同一 session 也可启发多答案
);
...
insight 让出 `card`/`cards`/`reviews`/`cards` collection/`cards/` 目录、且 id 前缀已彻底改成 `insight_` 之后,**同一个迁移 `v3`** 就地建 v4:`cards`(问题)/ `positions`(答案 + 内联文本,无独立 id,寻址 `card_id#p<n>`)/ `reviews`(对 Position 表态,`review_`,`argument` ±1/0)/ `card_links`(card↔card)/ `card_sessions`(card↔session 出处),**卡用 `card_` 前缀 + `card` 命令**。**v4 与 insight 物理隔离**(不同表、不同 collection、不同前缀 `card_` vs `insight_`),单一 `/v4` 前缀靠 `parse_id` 全局分派,互不干扰地共存。
