...
> **完整迁移机制单独成篇**——SQLite 表改名、LanceDB collection「拷贝而非改名」(`AdminBackend` 无 `rename_collection`)、`card_*` id 前缀重写(含 `recall_event` / `search_log` 里的嵌套 id + 文件罐 bucket 重排)、一次到位 vs 分两段降风险、幂等 / 失败语义,见 [**insight-migration.md**](insight-migration.md)。
