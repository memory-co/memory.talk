...
- **位不再是门禁**:召回命中的 Position 按 `credence` 排序后注入,**`scope`(适用场景)随答案一起喂给 LLM 当软提示**——让模型自己判当前语境合不合,而不是机械挡掉(§7 读路径)。「跨界」默认放行,只在 `scope` 文本写明的负边界上提醒。
...
| 检索结果 | 判定 | 动作 |
|---|---|---|
| **miss**(最近邻距离 > θ) | 真·新问题 | 建**新卡**(新问题),起 why→Q→A 的第一个 Position,`birth_surprise` = 距离 + 记一条 `card_sessions`(`session_id` + `indexes` = 触发它的那条 `#问题` 旁白所在 round) |
...
排序   : 命中的 Position 按 credence 排序;一张卡通常只注入最高的那个(当下答案,非「采纳」状态)
注入   : 按 credence 进 context——每个 Position 连同它的 `scope`(适用场景)一起给 LLM
...
- **不可变核**(create 即冻,canonical 在文件罐):Card 的 `issue`、Position 的 `(card_id, claim, birth_surprise, created_at)`(`claim` = 答案文本);card↔session 出处单独进 `card_sessions`。
- **可变运行态**(SQLite 实时维护):`credence / up_count / down_count / neutral_count`、治理两维(位 + 变)、两张关系表 `card_links`(card↔card)/ `card_sessions`(card↔session)。SQLite 同时**照抄一份不可变字段**供查询,崩了能从文件罐重建。

```
cards/<bucket>/<card_id>/card.json            ← canonical:issue + created_at(问题不可变)
cards/<bucket>/<card_id>/positions/<pid>.json ← canonical:claim(答案文本)+ birth_surprise(答案核不可变)
...
- **端点**(复数):`/v4/cards`(问题 + 它的 Positions)、`/v4/cards/{id}/positions`、`/v4/positions/{pid}/reviews`(表态 ±1/0)、`/v4/card-links`。读路径 `/v4/recall` 走「撞问题 → 取 Position → credence 排序 → 连 `scope` 注入」(scope 是软提示,不挡)。
- **CLI**:`memory.talk card`(v4 复用此命令),语义 = 「给一个问题立一个答案」。最小面:
  - `card create (--card <cid> | --issue '<问题文本>') --answer '<答案文本>' [--cite <sid>:<indexes> ...]` → 没给卡就先建卡(新建 issue);`--answer` 落成这张卡下的一个 Position;`--cite` 可多次,每个 session 落一条 `card_sessions`(出处,支持多 session)。
