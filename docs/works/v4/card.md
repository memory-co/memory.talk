...
- **端点**(复数):`/v4/cards`(建卡 / 列卡,只管 issue)、`/v4/cards/{id}/positions`(给卡加答案)、`/v4/positions/{pid}/reviews`(表态 ±1/0)、`/v4/card-links`、`/v4/card-sessions`(出处)。读路径 `/v4/recall` 走「撞问题 → 取 Position → 现算校验分排序(平手按最近更新)→ 连 `scope` 注入」(scope 是软提示,不挡)。
