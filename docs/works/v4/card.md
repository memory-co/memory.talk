...
> **对照 v3**:v3 的 review/read/recall 挂在**整张卡**、还揉进一个沉浮公式;v4 **只把 review(顶/踩)下放到 Position 变成 `credence`,把 read/recall 整个踢出存储**:同一个问题下的**不同答案各自被顶踩、按 credence 竞争**,于是「这个问题到底哪个答案对」第一次有了**结构**(各答案竞争、按 credence 排序),而不是糊在一张卡、也不被「热不热」干扰。
...
针对墙 4 的后半(场景误用)。IBIS 给了**结构**,但没说一张卡**在哪能用、怎么变迁**。把这层显式建出来,对上两个工程维度——助记叫「位 / 变」(去魅:就是 *适用域门禁 / 答案 append-only*,「易经」只是助记标签):
...
1. **「不该用」≠「错」**。一张卡可以没被反驳、`credence` 还高,但**用错了场景**(位错),照样不该用。「不该用(位错)」和「对不对(credence)」是两件独立的事,别用 credence 去判该不该用。(注:v3 的「时间过期」也是「不该用」的一个独立来源,但本稿**不建模时间**,见 §12;过期目前没有专门机制,只能靠后续被踩把 credence 压下去。)
2. **易经的「位」≠ IBIS 的 Position 节点**。这里的「位」是**适用域门禁**,本稿叫它 **Scope**;它和「Position(卡底下的答案候选)」是两回事,**别混**。
3. **这两维里真正比 IBIS 多出来的增量**:**位** = Scope/适用域 + exclusions(IBIS 没有适用域),和 **变**里的 **fork**(`forked_from_position_id`:信念分叉时**分支而非覆盖**,保住认知史)。原来的 `change_state` 状态机(active/dormant/archived)和 `superseded_by` 已删——「不覆盖」靠 Position append-only、「停用」靠 credence 自然掉下去,都不需要状态位。
...
| **hit** 某卡、且与它 credence 最高的答案一致 | 不惊讶 | 给那个 Position 加一条 **`argument=+1` 的 review**(→ `up_count++`,轻推 credence) |
...
class Position(BaseModel):                  # 卡底下的一个答案候选;被顶踩、按 credence 竞争的就是它
    ...
    # 治理:位 / 变(§5)
    scope: Scope                            # 位:domain×role×authority×layer + exclusions
...
排序   : 幸存 Position 按 credence 排序;一张卡通常只注入最高的那个(当下答案,非「采纳」状态)
...
- **可变运行态**(SQLite 实时维护):`credence / up_count / down_count / neutral_count`、治理两维(位 + 变)、`card_links`。SQLite 同时**照抄一份不可变字段**供查询,崩了能从文件罐重建。
...
CREATE TABLE positions (                   -- 卡底下的答案候选;被顶踩、按 credence 竞争的就是它
  ...
  forked_from_position_id TEXT, birth_surprise REAL  -- 变:Position append-only;forked_from_position_id 记分叉血缘
);
...
- 可选**投影**:把每条 insight **投影**进 v4 图——一条 insight →(一张**卡** =「这条洞见在答的问题」+ 一个 **Position** = 洞见本身那个答案,其 `rounds`→`cited_rounds`;原 insight 的 review(`insight_reviews`)跟着 target 到这个 Position,成为 v4 review(`argument` = 原 `score`))。投影完的 insight 标记为已投影、不再重复投。
...
- **CLI**:`memory.talk card`(v4 复用此命令),语义 = 「给一个问题立一个答案」。最小面:
  ...
  - `card view <card_id>` → 问题 + 它所有 Position(各自 credence / 顶踩 / 治理),credence 最高的那个高亮。
...
**易经治理的真实成分**:本稿只留**位 / 变**两维。**真增量只有 Scope(适用域)和 fork**(`forked_from_position_id`);原来的 `change_state` 状态机和 `superseded_by` 已删——「不覆盖」靠 Position append-only、「停用」靠 credence 自然掉下去,不需要状态位。原设计的**时(过期/半衰)**和**势(沉浮排序量)两维已删**——时间过期暂不建模,相关性改由召回时检索现算(§3),不再回写成字段。v3 本就**没有** `salience` / `validity` 字段——它只有 Review(`review_up/down/neutral`,质量)、Read(`read_count`,engagement)、Recall(`recall_count`,popularity)三轴 + `age_days`,v4 只继承其中的 Review。
