...
>
> **三条出处/证据线都带 round 级 `indexes`,但语义不同**(别混):
> - **`card_sessions`**(mark 写路径产):key 是**那条 mark**(`session_id` + `mark`),`indexes` = 这条 `#问题` **grounding 的 round**(问题从哪几轮读出来的);**无 `position`**。
> - **`position_sessions`**(`card position --source` 写):key 是 `indexes` = **答案**从哪几轮长出来;`mark` 可选。
> - **`reviews`**(`card review --cite` 写):`indexes` = 这次**表态**拿哪几轮当证据。
>
> 三条各司其职——问题来源 / 答案来源 / 表态证据,详见 [session-mark.md](session-mark.md)。
...
`Review`(`target = card_id` + `position`(`p<n>`) + `indexes` + `argument`(`+1`/`0`/`−1`);沿用 v3 review,`argument≠0` 即 IBIS Argument)、`CardLink`(主体卡的边:`card_id` + `type` + `target_id` + `target_type`,对端 `card_` 或 `card_…#p<n>`)、`CardSession`(**card→session** 出处:`card_id` + `session_id` + `mark`,经 mark;同一对可多条)、`PositionSession`(**position→session** 出处:`card_id` + `position` + `session_id` + `indexes` + `mark?`,经 indexes)各自是独立 BaseModel。**答案文本 `claim` 内联在 Position 上(不单独建 Claim 节点);治理只挂 Position;Card 只管问题 + 连边;证据/表态是 Review。**
...
- **不可变核**(create 即冻,canonical 在文件罐):Card 的 `issue`、Position 的 `(card_id, claim, created_at)`(`claim` = 答案文本);card→session 出处(经 mark)的 canonical 是 mark 的 `questions[]`,`card_sessions` 表是其派生索引。
- **可变运行态**(SQLite 实时维护):`up_count / down_count / neutral_count`(credence 不存,排序现算)、治理两维(位 + 变)、关系表 `card_links`(card↔card)/ `card_sessions`(card→session,经 mark)/ `position_sessions`(position→session,经 indexes)。SQLite 同时**照抄一份不可变字段**供查询,崩了能从文件罐重建。
...
-- review:对 Position 的带证据表态(沿用 v3 review;`argument≠0` 即 IBIS Argument),target 从整张卡下放到 card_id + position(card_id#p<n>)
CREATE TABLE reviews (
  review_id   TEXT PRIMARY KEY,            -- review_<ulid>
  card_id     TEXT NOT NULL,               -- target 所属卡(= positions.card_id);源不可变(答案不换卡)→ 永不漂移,省「这张卡所有 review」的 join
  position    TEXT NOT NULL,               -- target 答案在卡内的序号 p<n>(target = card_id#position)
...
CREATE INDEX idx_reviews_position ON reviews(card_id, position, created_at DESC);
...
insight 让出 `card`/`cards`/`reviews`/`cards` collection/`cards/` 目录、且 id 前缀已彻底改成 `insight_` 之后,**同一个迁移 `v3`** 就地建 v4:`cards`(问题)/ `positions`(答案 + 内联文本,无独立 id,寻址 `card_id#p<n>`)/ `reviews`(对 Position 表态,`review_`,`argument` ±1/0)/ `card_links`(card↔card)/ `card_sessions`(card→session 出处,经 mark)/ `position_sessions`(position→session 出处,经 indexes),**卡用 `card_` 前缀 + `card` 命令**。**v4 与 insight 物理隔离**(不同表、不同 collection、不同前缀 `card_` vs `insight_`),单一 `/v4` 前缀靠 `parse_id` 全局分派,互不干扰地共存。
