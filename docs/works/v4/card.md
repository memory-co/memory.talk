...
| 检索结果 | 判定 | 动作 |
|---|---|---|
| **miss**(最近邻距离 > θ) | 真·新问题 | 建**新卡**(新问题,**只有问题、还没答案**)+ 记一条 `card_sessions`(`session_id` + **`mark`** = 触发它的那条 `#问题` 旁白) |
| **hit** 某卡 | 不惊讶 / 已问过 | **连**这张老卡 + 记一条 `card_sessions`(`session_id` + **`mark`**)——同一 card↔session 可多条 mark |
...
>
> **旁白机制正是 card→session 出处的写入口,但 round 级 `indexes` 不在这条链路上**:mark 写路径只产 **`card_sessions`**,出处是**那条 mark**(`session_id` + `mark`,**无 `indexes`、无 `position`**)——记「这个 session 的这条 mark **建/连**了这张卡」(miss→新卡、hit→连老卡)。round 级的 `indexes` 活在**另外两条**(主观)链路:**答案的出处** `position_sessions`(`indexes` 必填,由 `card position --source` 写)和**证据** `reviews`(`indexes`,由 `card review --cite` 写)。三条线各司其职,详见 [session-mark.md](session-mark.md)。
...
CREATE TABLE card_sessions (               -- card → session 出处:哪条 mark 的 #问题 建/连了这张卡(经 mark;无 position/indexes)
