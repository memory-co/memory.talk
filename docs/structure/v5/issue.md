...
> **存储 / id 已变**:issue 现在是 Collect 的一个 layer,对象是任意位置的 `<path>.issue/` 目录,id 就是 path;本篇里关于 `iss_…` id、`issues/` `cards/` 目录、`manager_work` 字段、`status` 的说法**作废**,以 [collect.md](collect.md) 为准。字段语义仍有效。

议事层:一个问题、它的立场、每个立场的论证、问题之间的 IBIS 边。一个 issue 一个 JSON 文件,住在 git 仓库里;**立场和论证只增不改**(service 保证,git 兜底)。机制见 [`../../designs/v5/issue.md`](../../designs/v5/issue.md)。
  ...
  "positions": [
    {
      ...
      "arguments": [
        {"id": "a1", "stance": 1, "comment": "试了一遍,环境变量够用",
         "evidence": {"work_id": "work_try", "rounds": [9]}, "work_id": "work_try",
...
| `origin` | Origin \| null | 不改 | 从哪个 work 的哪些 round 冒出来。**出处不是归属**——冒出它的 work 冻结了,issue 还活着 |
| `manager_work` | string \| null | 改 | 树上管它的 work;`null` = 还没人管(这是有用的状态) |
...
**Origin**:`{"work_id": "...", "rounds": [int, ...]}`。`rounds` 是 `rounds.jsonl` 里的下标(0 起),可空。work 层未必还在——读时不校验。
...
| `comment` | 一句话 |
| `evidence` | Origin \| null:证据在哪个 work 的哪些 round |
| `work_id` | 若来自派出的论证 work,记它;与 `evidence.work_id` 可同可不同 |
| `created_at` | |
...
**issue 图和 work 树是两套层级**:work 树表达事怎么拆(有状态、有完成),issue 图表达问题怎么细化(没有)。别拿一个代替另一个。
...
issue: argue <id>#p2 +1                     表态(body 带 Work: / Rounds:)
issue: manage <id> by <work_id>             绑 manager   / issue: unmanage <id>
issue: link <id> specializes <target>       连边
issue: spawn <id>#p2 -> <work_id>           派活
decide: <id>#p2 -> card <card_id>           写卡(同一 commit 也动了 cards/…)
discuss: card <card_id> -> <id>: <question> 对卡开讨论页(同一 commit 也动了 cards/…)
```

commit body 里 `Reason:` 来自请求的 `reason`,`Work:` / `Rounds:` 来自 `origin` / `evidence`。历史 = `git log -- issues/<id>.json`。
