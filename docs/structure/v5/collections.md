...
> **设计已改**:[designs issue.md](../../designs/v5/issue.md) 把它改成多文件目录——`readme.md`(问题的展开,标题 = 目录名)+ `links.yaml`(issue 间的边)+ `positions/p<n>.md`(一个立场一个文件,论证在里面);不再有 `origin` / `card` 字段,不再有 `decide` / `spawn`。下面是**代码现状**,代码跟上后本节重写。
