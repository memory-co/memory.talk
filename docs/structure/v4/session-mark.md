...
**逐 round 打的注解** —— session 的附属,不是一等对象。一条 mark = 一段「以写代读」的感悟,`mark` 文本里 `#…？` 自动建卡。正文落 YAML 文件(canonical),元信息进 `session_marks` 表(派生索引)。

机制见 [`../../works/v4/session-mark.md`](../../works/v4/session-mark.md);命令见 [`../../cli/v4/session-mark.md`](../../cli/v4/session-mark.md);出处边 see [CardSession](card-session.md)。
...
```yaml
# marks/m1.yaml
last_index: 41                # 标这条 mark 时 session 的最新 round index(乐观锁基线 + 当时情况)
description: 在配 pty、用户突然提 tmux 的那几轮——想搞清他到底要什么
round_index: 37               # 标的是哪一轮
mark: |                       # 自由感悟正文;#…？ 就地标问题
  配 pty 时用户突然提了 tmux。#为什么 pty 会让用户想到 tmux？
  他其实想要可重连会话。
questions:                    # 写入时解析 #…？ + 撞库的结果(canonical)
...
| `round_index` | 这条 mark 标的是第几轮 |
| `mark` | 自由文本感悟;`#…？`(`#` 起、`？`/`?` 止)= 就地标的问题 |
| `questions[]` | 解析 `#…？` + 撞 `cards` 库的结果:`{raw, card_id, is_new}`(新卡 / 关联老卡)。**canonical**——`card_sessions` 由它派生 |
| `created_at` | ISO 8601 |
...
| [`card_sessions`](card-session.md) | mark → card 的出处边(`card_id` + `session_id` + `mark` + `position_id`) |
