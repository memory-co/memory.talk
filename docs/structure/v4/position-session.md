...
读「答案 `card_01jz8k2m#p1` 来自 `sess_def456` 的第 11–15 轮」。`indexes` 是主出处;`mark` **可选**(给了就额外指明「这答案是哪条 mark 触发的」,`""` = 没记)。

## 字段

| 字段 | 类型 | 必填 | 说明 |
|---|---|---|---|
| `card_id` | string | 是 | 答案属于哪张卡;`card_<...>` |
| `position` | string | 是 | 哪个答案;卡内序号 `p<n>`(寻址 `card_id#position`)。Position 无独立 id |
...
CREATE TABLE position_sessions (
  card_id     TEXT NOT NULL,             -- 答案所属卡
  position    TEXT NOT NULL,             -- 哪个答案(卡内序号 p<n>;寻址 card_id#position)
...
- **PK `(card_id, position, session_id, mark)`**:`mark=''` 是「只记到 round」的那条;给了 mark 则可与无-mark 行 / 不同 mark 行并存。多个 `--source` 给不同 session → 多条。
