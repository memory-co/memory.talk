...
| 字段 | 类型 | 必填 | 说明 |
|---|---|---|---|
| `position_id` | string | 自动 | `pos_<ULID>` |
...
| `forked_from_position_id` | string\|null | **变** —— 信念分叉时记「从哪个 Position 分出来」(血缘指针,保认知史);`pos_<...>` 或 `null` |
...
└── positions/
    ├── <pid_a>.json             # canonical:claim + created_at(答案核不可变)
    └── <pid_b>.json
```

`<bucket>` = `card_id` ULID 部分前 2 字符(代码 `card_id[5:7]`,跳过 `card_` 前缀)。Position 文件放在所属卡目录下 —— 一张卡 = 一个问题 + 它的答案们,物理聚在一起。
...
CREATE TABLE positions (
  ...
  forked_from_position_id TEXT              -- 变:Position append-only;分叉血缘
);
