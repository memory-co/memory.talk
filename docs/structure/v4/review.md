...
v4 **沿用 v3 的 review**,只把 target 从整张卡(`card_id`)下放到答案粒度——target 是某张卡的某个 **Position**(`card_id` + `position`,寻址 `card_id#p<n>`)。其中 **`argument≠0` 的 review 就是一条 IBIS Argument**(pro/con);`argument=0` 是中立观察,不是 argument。
...
| `card_id` | string | 是 | **被表态 Position 所属的卡**;`card_<...>`。跟 `position` 一起定位 target(寻址 `card_id#position`) |
| `position` | string | 是 | **被表态的 Position** 在卡内的序号 `p<n>`(target = `card_id#p<n>`)。Position 无独立 id |
...
**不去重**。同一对 `(card_id, position, session_id)` 允许多条 review —— 一次会话里在不同位置可能对同一答案表态多次(早期反对、深入后转支持),每条由 `indexes` 区分。
...
| target | `card_id` + `position`(`card_id#p<n>`) | `card_id`(+ 可选 `position` = `p<n>`) |
...
CREATE TABLE reviews (
  ...
  card_id     TEXT NOT NULL,               -- target 所属卡(= positions.card_id)
...
- `idx_reviews_position(card_id, position, ... created_at DESC)` 同时服务两件事:累加某个 Position 的计数、以及排序平手时取「最近更新」(最后一条 review 时间)做 tiebreak。
...
| | v3 review | v4 review |
|---|---|---|
| target | `card_id`(整张卡) | `card_id` + `position`(某张卡的某个答案,`card_id#p<n>`) |
...
| 冗余键 | — | `card_id`(= 被表态 Position 的卡,扁平存,省 join) |
