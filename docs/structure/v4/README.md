...
| Review | append-only,对 **Position 或 CardLink** 的一次表态(`argument` ±1/0 + 证据 indexes;`target_kind` 派生) | [review.md](review.md) |
| CardLink | card↔card 的 IBIS 有向边,**受治理对象**(有 `claim` + 顶踩 + 现算 credence + 证据 + review,寻址 `<card_id>#l<n>`);存在即合理(每边各自成立) | [card-link.md](card-link.md) |
...
> **Position / CardLink 都没有全局前缀 id** —— 都是卡的附属,寻址 `<card_id>#p<n>`(Position)/ `<card_id>#l<n>`(CardLink),跟 mark `<session_id>#m<n>` 同构(见 [session-mark.md](session-mark.md))。CLI / API 按 `card_` id 上的分片判型:`#p<n>` → 那张卡的 Position、`#l<n>` → 那张卡的 CardLink,分发到对应对象(`read` / review 都按此判型)。
...
边(受治理,在 card_links):     card_xxx#l1 --specializes--> card_yyy   (claim + up/down + 现算 credence,过阈值才显示)
出处·卡(在 card_sessions):     card_xxx ←── sess_def#m1 的 #问题 建/连了 card_xxx(经 mark)
出处·答案(在 position_sessions):card_xxx#p1 ←── sess_def #11-15(经 indexes)
出处·边(在 link_sessions):     card_xxx#l1 ←── sess_def #30-34(经 indexes)
表态(不内联,在 reviews):       review_* --argument=+1--> card_xxx#p1 或 card_xxx#l1(target_kind 判型)
```
...
│   └── links/l<n>.json                  # canonical:type + target_id + claim + created_at(边核不可变),文件名 = l<n>
...
1. **不可变核 create 即冻** —— Card 的 `issue`、Position 的 `(card_id, claim, created_at)`、CardLink 的 `(card_id, type, target_id, claim, created_at)` 一旦写入不可改;canonical 落文件罐。要「翻新」只能**新增一个竞争 Position**(同卡内)或**新建一张卡**(`replaces` 边指回)或**踩掉一条边**(credence 现算掉下去 → 隐藏),不改旧的。
...
| 量 | 怎么来 |
|---|---|
| `credence`(Position 校验分) | `f(up_count, down_count)`,排序时算 |
| `credence`(CardLink 边的校验分) | 同函数 `f(up_count, down_count)`,过滤时算;**低于阈值的边淡出 / 隐藏**(存在即合理,不竞争) |
...
| 关联 | `source_cards`(创建即冻) | `card_links`(card↔card)+ `card_sessions`(card→session,经 mark)+ `position_sessions`(position→session,经 indexes) |
...
|---|---|---|
| 主对象 | card, review | **card(Issue), position, review, card_link, card_session, position_session, link_session** |
| 卡的粒度 | 一句陈述 | 一个问题 + 多个答案候选 |
| 顶踩挂在 | 整张卡(`card_stats`) | **每个 Position / 每条 CardLink** 各自计数 |
| 质量分 | `card.stats` 6 计数 + 沉浮公式 | Position / Link 各存 `up/down/neutral_count` 三计数;credence 现算 |
| 关联 | `source_cards`(card→card,内联) | `card_links`(card↔card)+ `card_sessions`(card→session,经 mark)+ `position_sessions`(position→session,经 indexes),都独立表、可 join |
