...
└── sessions/<source>/<id[0:2]>/<sid>/     # 沿用 v3;session mark 落在这里(见下)
    ...
    │   ├── m1.yaml                        #   last_index + description + mark + questions[]
...
| `reviewed` | 某 Position 收到一条 review | `review_id`, `position`(p<n>), `argument`, `session_id`, `indexes` |
| `card_linked` | 本卡被另一张卡在 `card_links` 里引用 | `from_card`, `type` |
| `session_cited` | 一条 `card_sessions` 关联了本卡(经 mark) | `session_id`, `mark` |
...
| `reviews` | 一行一表态 | `card_id` / `position`(p<n>) / `session_id` / `indexes` / `argument` / `comment` | [review.md](review.md) |
...
索引:`idx_reviews_position`(card_id, position, created_at DESC)、`idx_reviews_card`(card_id)、`idx_card_sessions_session`(session_id)、`idx_card_sessions_mark`(session_id, mark)、`idx_position_sessions_session`(session_id)、`idx_session_marks_session`(session_id)。(`positions` 已按主键 `(card_id, position)` 聚簇,列某卡的 Position 直接走 PK 前缀,无需独立索引。)
...
| | v3 | v4 |
|---|---|---|
| 卡目录 | `cards/<bucket>/<card_id>/card.json`(整卡一个 payload) | 同前缀,但 `card.json`(问题)+ `positions/p<n>.json`(每答案一份) |
| 卡子系统 SQLite 表 | `cards` / `card_stats` / `card_source_cards` / `reviews` | `cards` / `positions` / `reviews` / `card_links` / `card_sessions` / `position_sessions` / `session_marks` |
