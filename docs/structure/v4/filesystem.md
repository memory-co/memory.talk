...
│   ├── cards.lance/                       # 表:cards(embed issue + vector)—— v4 检索主单元
│   ├── insights.lance/                    # 表:insights(= v3 cards collection 改名 copy,只读可搜)
│   └── rounds.lance/                      # 表:rounds(session round,沿用 v3)
└── sessions/<source>/<id[0:2]>/<sid>/     # 沿用 v3;旁白标注落在这里(见下)
    ├── meta.json
    ├── rounds.jsonl
    ├── annotations.jsonl                  # 逐 round 旁白 + questions[](card_sessions 的 canonical)
    └── events.jsonl
```

设计原则:**SQLite 是查询主路径**(计数 / 现算排序 / 关系 join / DSL);**文件罐是 canonical**(可移植 / 审计,db 丢能重建);**LanceDB 是检索召回的唯一来源**(embed `cards.issue`)。
...
| `session_cited` | 一条 `card_sessions` 关联了本卡 | `session_id`, `position_id?`, `indexes` |
| `vector_index_failed` | LanceDB 写 issue 向量失败(best-effort) | `error` |
...
| collection | 行 | 字段 | 用途 |
|---|---|---|---|
| `cards` | 一行一卡 | `card_id` / `text`(= `issue` 分词)/ `vector` | **v4 检索主单元**(读路径撞 issue、写路径拿生成的 question 来匹配,都打它) |
...
只 embed `cards.issue`(问题就是答案们的适用条件,是天然检索锚点)。`position.claim` **不单独 embed**。
...
| card embedding | — | — | `cards` collection |
...
| card↔session 出处 | `card_sessions`(派生) | `annotations.jsonl` 的 `questions[]`(canonical) | — |
...
| 卡子系统 SQLite 表 | `cards` / `card_stats` / `card_source_cards` / `reviews` | `cards` / `positions` / `reviews` / `card_links` / `card_sessions` |
| engagement 计数 | `card_stats.read_count` / `recall_count` 落库 | 不存(召回时现算) |
| 质量分 | 沉浮公式吃多列 | credence 现算 = f(up, down),不落列 |
| 出处 | `rounds[].session_id` 内联 | `card_sessions` 表 + 旁白 `questions[]` |
| LanceDB | `cards`(embed insight)+ `rounds` | `cards`(embed issue)+ `insights`(v3 遗产)+ `rounds` |
