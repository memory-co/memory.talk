...
v4 写路径 = **逐 round 旁白(以写代读)**,落在 session 目录的 `annotations.jsonl`(每行一 round 的旁白 + 解析出的 `questions[]`)。`questions[]` 是 card↔session 关联的 **canonical**;SQLite `card_sessions` 是它的派生索引。详见 [`../../designs/v4/session-annotation.md`](../../designs/v4/session-annotation.md) 与 [card-session.md](card-session.md)。
