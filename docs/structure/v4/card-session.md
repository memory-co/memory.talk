...
读「`sess_def456` 的**第 1 条 mark**(`sess_def456#m1`)的 `#…？` 建 / 连了 `card_01jz8k2m`」。**不指向具体 Position** —— 这是卡级关系(答案的出处走 [PositionSession](position-session.md))。
...
CardSession **不是原始真相**:它的 canonical 是逐 round mark(`marks/m<n>.yaml`)里每条 mark 的 `issues[]`(`card_id` + `is_new`)。`card_sessions` 表是**从这些 `issues[]` 派生出来的可 join 索引**,回答「这个 session / 这条 mark 建 / 连了哪些卡」。详见 [`../../works/v4/session-mark.md`](../../works/v4/session-mark.md)。
...
- **`mark` 必选**:这条链路颗粒度**必须到 mark**——因为要支撑 mark 里 `#…？` 的**双向关联**(从卡查「哪些 mark 提了它」、从 mark 查「建/连了哪些卡」,两边互查)。对比 [position_sessions](position-session.md):答案级那条链路 `mark` **可选**(主出处是 round `indexes`)。
