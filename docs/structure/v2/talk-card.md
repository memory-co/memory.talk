...
card 也支持 tag（kv 形态），跟 session 共享 sqlite `tags` 表（靠 `type` 字段区分）。

落盘镜像：`cards/{bucket}/{card_id}/tags.json`（dict 形态 `{"key": "value", ...}`）。card schema 自身不持有 tag 数据，但 `view card` 响应里会带一个 `tags: list[{key, value}]` 字段（service 层 join 出来）。
