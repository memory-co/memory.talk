...
| `ttl` | integer | 自动 | 剩余生命值。`0` = 默认 link（跟随 parent card），`>0` = 用户 link（独立计时） |
...
通过 [`link create`](../../cli/v2/link.md) 写入。创建时 `ttl` 取 settings 里的初始值（默认 100），被 view 命中时按 `ttl.link.factor` 续命，超时后独立过期（不影响两端对象）。
...
| ttl | 含义 |
|-----|------|
| `> 0` | 活跃（用户 link），出现在查询结果里 |
| `= 0` | 默认 link，永远跟随 card 出现，card 被遗忘它一起遗忘 |
| `< 0` | 用户 link 已过期，不再出现在查询结果中（数据保留，不删除） |
