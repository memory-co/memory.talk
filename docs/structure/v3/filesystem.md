...
| `memory.db` | SQLite — 业务数据(sessions / rounds 元数据 / cards / reviews / 日志) | server lifespan 启动时 `init_schema` 幂等建表;各 service 写入 | 用户手动(重建走 `setup`) |
| `sync.db` | SQLite — sync 连接器自己的游标库(`sync_session_checkpoint` 表) | 由 `SyncCheckpointStore` 在 lifespan 启动时建表;sync watcher 写入 | 删掉 = 触发下一次 server start 时全量冷扫(数据安全,因为 ingest 是 append-only + UNIQUE 兜底) |
...
| `memory.db` 里的 `rounds_index` / `ingest_log` 表 | v0.6 之前的 sync 增量元数据。**`rounds_index` 已删**(append-only 改写后不需要按 round 做内容 diff);**`ingest_log` 已迁去独立 `sync.db`**,字段也变了(`sha256 + last_round_id + line_offset`)。两张表在 `init_schema` 里都有显式 `DROP TABLE IF EXISTS`,旧数据库重启时自动清理 |
...
| 表 | 粒度 | 用途 | 详见 |
|---|---|---|---|
| `sessions` | 一行一 session | 元数据 + `cwd`(explore namespace)+ `last_round_id`(ingest 游标) | [session.md](session.md) |
...
**round 全文不在 SQLite** —— 落在 `rounds.jsonl`(完整结构) + LanceDB `rounds` 表(text + vector)。SQLite 这一层只追踪每个 session 的 `last_round_id` 游标(用于 ingest 的乐观锁),不存 round 内容,也不存 round 索引表。
...
| 表 | 粒度 | 用途 |
|---|---|---|
| `sync_session_checkpoint` | `(source, session_id)` | 每个上游 session 的 `sha256` + `last_round_id` + `line_offset` + `updated_at`。**source 是 adapter 名(`claude-code` / ...),`session_id` 是平台原始 id 不含 `sess_` 前缀** |
...
| event | 触发点 | 关键字段 |
|---|---|---|
| `imported` | 首次 ingest 一个 session | `round_count`, `added` |
| `rounds_appended` | 追加 ingest,有新增 round | `added`, `round_count` |
...
`memory.db` 和 `sync.db` 由各自的 store 在首次连接时自动创建;`{source}` / `{bucket}` / `{card_id}` 等子目录在第一次写入时按需建。
...
| session 游标(`last_round_id`) | `sessions.last_round_id`(`memory.db`) | — | — |
| session 事件 | — | `events.jsonl` | — |
| sync 上游游标(sha256 + last_round_id + line_offset) | `sync_session_checkpoint`(`sync.db`) | — | — |
