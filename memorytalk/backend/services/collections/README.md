# Collections：用 Git 分支组织分层认知

Collections 是认知层的容器：`origin` 保存外部原文，`issue` 保存问题、立场与论证，`card` 保存可复用的事实词条。它们共享一个 Git 仓库；Git 同时保存当前内容、变更历史和层之间的边界。用户定义的层也遵循同一套结构。
...
分支提供独立的内容与历史，**路径归属守卫负责保证分层约束**。写入前同时检查路径按规则应归哪层，以及已有文件实际由哪层持有；修改或删除其他层的路径会被拒绝。因此，上层可以引用原文，但无法以自己的层身份改写原文。

跨层业务动作拆成各层自己的提交。例如将 issue 的结论写成 card，会分别提交 issue 的关联变更和 card 的新内容，并用共同的 `Decision` 标记关联。业务上是一件事，历史中仍保留每层明确的责任边界。

这套约束由服务写入入口执行，当前以服务进程作为仓库唯一写者。新增层时，只需登记 schema 并建立对应分支，即可沿用统一的读写、历史与归属机制。

## 模块分工

- [`__init__.py`](__init__.py)：业务入口，装载层定义、组织对象操作与领域行为，并投递变更通知。
