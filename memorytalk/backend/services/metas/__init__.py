"""MetasService:认知层。层 = 一份 YAML 协议(引擎据此校验);对象 = 一个目录里的一组文件,按 path 读写;一次写 = 一批文件改动 → 层的 check → 一个 `[layer]` 提交;变动投递给 manager。"""
...
from memorytalk.backend.models.metas import (InboxItem, LayerInfo, Manager, Obj, RecentItem, RecentPage, Revision, TreeItem,
                            TreeView)
...
CONFIG_FILE = "metas.json"


class MetasError(RuntimeError):
...
class MetasService:
    ...
    def _load_layers(self) -> None:
        """内置 YAML + <home>/layers/*.yaml;metas.json 里缺的补上(一次最底层提交),多出来的(文件没了)报错。"""
        builtin = layer_pkg.load_builtin()
        known = {l.name: l for l in builtin}
        try:
            user = layer_pkg.load_user(self.config.layers_dir)
        except ValueError as e:
            raise MetasError("bad_layer", str(e), 500) from None
        for l in user:
            if l.name in known:
                raise MetasError("bad_layer", f"层名重复:{l.name}", 500)
            known[l.name] = l
        ...
        if missing:
            raise MetasError("bad_layer", f"metas.json 里有层 {', '.join(missing)},但既不是内置的,{self.config.layers_dir} 下也没有它的 .yaml", 500)
    ...
    def anchor_history(self):
        """metas.json 的提交历史 = 层的变化史(在最底层分支上)。"""
    ...
    def layer(self, name: str) -> Layer:
        try:
            ...
            raise MetasError("no_layer", f"没有这一层:{name}(有:{', '.join(self.order)})", 404) from None
    ...
    def get(self, layer: str, path: str, rev: str | None = None) -> Obj:
        ...
        if files is None:
            raise MetasError("not_found", f"{layer}:{path} 不存在", 404)
    ...
    def search(self, q: str, limit: int = 20, layer: str | None = None) -> list[SearchHit]:
        ...
        for h in self.repo.grep(q):
            ...
            if layer and lyr != layer:
                continue
            hits.append(SearchHit(kind="meta", id=path, title=path.rsplit("/", 1)[-1], snippet=h.text, layer=lyr, file=h.file, line=h.line))
            if len(hits) >= limit:
                break
        return hits
    ...
    def commit(self, layer: str, subject: str, puts: dict[str, bytes], deletes: list[str], reason: str, ctx: Ctx) -> str:
        try:
            ...
            raise MetasError("guard", str(e), e.status) from None
    ...
    def put(self, layer: str, path: str, files: dict[str, str | bytes | None], reason: str, ctx: Ctx,
        ...
        path = path.strip("/")
        if not path:
            raise MetasError("invalid", "path 不能为空", 400)
        if spec.suffix is None:
            content = files.get("")
            if content is None:
                raise MetasError("invalid", "origin 要有 content", 400)
            if self._context(path)[0] is not None:
                raise MetasError("invalid", f"{path} 在一个对象目录里,不是 origin 文件", 400)
        ...
        if not cur and (why := spec.check_object_path(path, inside_object=self._context(path.rsplit("/", 1)[0] if "/" in path else "")[0] is not None)):
            raise MetasError("invalid", f"[{layer}] {path}:{why}", 422)
        after = dict(cur)
        changes: list[Change] = []
        for rel, content in files.items():
            rel = rel.strip("/")
            if not rel or rel == MANAGER_FILE or ".." in rel.split("/"):
                raise MetasError("invalid", f"文件名不合法:{rel!r}", 400)
            ...
            if new is None:
                ...
                after[rel] = new
        if not changes:
            raise MetasError("invalid", "没有任何改动", 400)
    ...
    def create(self, layer: str, path: str, files: dict[str, str | bytes | None], reason: str, ctx: Ctx,
        ...
        if self.exists(layer, path):
            raise MetasError("exists", f"{layer}:{path} 已存在", 409)
    ...
    def unset_manager(self, dir_: str, reason: str, ctx: Ctx) -> None:
        ...
        if not self.repo.exists(file):
            raise MetasError("not_found", f"{file} 不存在", 404)
...
__all__ = ["MetasService", "MetasError", "Ctx"]
