...
agent server 把手的「读 round」那一半:在平台自己的记录目录里找到这个工作单元的文件,解析成 `Round{id, timestamp, role, text}`。每个平台一个文件,实现同一个接口:

| 文件 | 平台 | 找文件 | 解析 |
|---|---|---|---|
| `base.py` | — | `TranscriptAdapter` Protocol:`find(cwd, since_mtime) -> Path | None`(按工作目录和工作单元开始时间定位记录文件)/ `rounds(path) -> list[Round]` | |
