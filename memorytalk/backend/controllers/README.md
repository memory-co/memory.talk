...
| `works.py` | `/api/works` | 树:`forest` / `create` / `get` / `update`;`events` / `inbox` / `get_manager` / `put_manager`;`users` / `touch`;画布 `get_canvas` / `put_canvas`;工作单元 `worklets` / `attach` / `reattach` / `detach` / `rounds`;`servers`。`user(request)` 依赖 = 登录态里的名字 |
