"""/api/metas —— 认知层:层、树、最近、对象(目录里的一组文件)CRUD、历史。检索在 /api/search。"""
from __future__ import annotations

from memorytalk.backend.models.result import Result, ok
from fastapi import APIRouter, Depends, Header, Query, Request

from memorytalk.backend.models.metas import (CheckResult, LayerInfo, Manager, ManagerPut, Obj, ObjWrite, RecentPage, Revision,
                                                     TreeItem, TreeView)
from memorytalk.backend.services.metas import MetasError, MetasService, Ctx
...
def layers(svc: MetasService = Depends(metas)):
    return ok(svc.layer_infos())


@router.get("/config", summary="metas.json 本体 + 它的 git 历史(层的变化史)", response_model=Result[dict])
def config(svc: MetasService = Depends(metas)) -> dict:
...
def tree(path: str = "", candidate: str | None = None, layer: str | None = None, recursive: bool = False, svc: MetasService = Depends(metas)):
...
def recent(layer: str | None = None, path: str = "", limit: int = Query(20, ge=1, le=200), before: str | None = None,
           svc: MetasService = Depends(metas)):
...
# def get_manager(path: str = "", svc: MetasService = Depends(metas)):
#     return ok(svc.manager(path))
#
#
# @router.put("/manager", response_model=Result[Manager], summary="在这个目录(或对象)下放 manager.json,绑到一个 work")
# def put_manager(req: ManagerPut, path: str = "", svc: MetasService = Depends(metas), c: Ctx = Depends(ctx)):
#     return ok(svc.set_manager(path, req.work, req.reason, c))
#
#
# @router.delete("/manager", summary="解绑:删这个目录的 manager.json")
# def delete_manager(path: str = "", reason: str = "", svc: MetasService = Depends(metas), c: Ctx = Depends(ctx)):
...
# def managed(work: str | None = None, svc: MetasService = Depends(metas)):
...
def history(layer: str, path: str, svc: MetasService = Depends(metas)):
    return ok(svc.history(layer, path))


# ---- 对象 ----

def _files(svc: MetasService, layer: str, req: ObjWrite) -> dict:
    """请求体 → 目录里的文件改动:origin 用 content,其余用 files。"""
    if svc.layer(layer).suffix is None:
        if req.content is None:
            raise MetasError("invalid", "origin 要给 content", 400)
        return {"": req.content}
    if not req.files:
        raise MetasError("invalid", "要给 files:目录里的文件 {相对路径: 内容}", 400)
...
def create(layer: str, path: str, req: ObjWrite, dry_run: bool = False, svc: MetasService = Depends(metas), c: Ctx = Depends(ctx)):
    if dry_run:
...
def get(layer: str, path: str, rev: str | None = Query(None), svc: MetasService = Depends(metas)):
...
def update(layer: str, path: str, req: ObjWrite, dry_run: bool = False, svc: MetasService = Depends(metas), c: Ctx = Depends(ctx)):
    if dry_run:
...
def delete(layer: str, path: str, reason: str = "", svc: MetasService = Depends(metas), c: Ctx = Depends(ctx)):
