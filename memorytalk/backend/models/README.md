...
| `work.py` | `Work` / `WorkCreate` / `WorkUpdate` / `WorkNode`(带 children);画布 `Canvas{version, columns[]}` / `Column{id, panels[], collapsed}` / `Panel{worklet, collapsed}` / `CanvasPut`;工作单元 `Worklet` / `WorkletCreate` / `WorkletView`(+ alive / window / handle);`Round` / `Event` / `WorkUser` / `WorkUsers` |
