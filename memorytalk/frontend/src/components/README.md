...
| `MarkdownEditor.tsx` | 默认导出:Milkdown Crepe 的包装——非受控,挂载时用 `value` 初始化、之后只往外报 `onChange`;`readOnly` 就是阅读视图(元认知页读写都用它,渲染一致)。关掉 Latex / AI,只读时再关掉块手柄 / 工具栏 / 占位符。主题变量在 `index.css` 里映射到 shadcn token。体积大,`Metas.tsx` 挂载时预取 |
