...
/** 正文编辑器:Milkdown Crepe(所见即所得的 Markdown,ProseMirror 之上)。
...
export default function MarkdownEditor({ value, onChange, placeholder, readOnly = false, className }: {
  ...
  useEffect(() => {
    ...
    const editor = new Crepe({ root: root.current, defaultValue, features: { [Crepe.Feature.Latex]: false, [Crepe.Feature.AI]: false }, featureConfigs: { [Crepe.Feature.Placeholder]: { text: text || '', mode: 'doc' } } });
