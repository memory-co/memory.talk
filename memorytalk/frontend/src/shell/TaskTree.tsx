...
function WorkRow({ work, selected, onNavigate, depth }: { work: Work; selected?: string; onNavigate: () => void; depth: number }) {
  ...
  return <Collapsible open={open} onOpenChange={setOpen}>
    <div className={`work-row ${selected === work.id ? 'selected' : ''}`} style={{ paddingLeft: 8 + Math.min(depth, 7) * 14 }}>
      {children.length ? <CollapsibleTrigger asChild><Button variant="ghost" className="tree-toggle h-6 w-[18px] p-0" aria-label={`${open ? t('common.collapse') : t('common.expand')} ${work.goal}`} aria-expanded={open}>{open ? <ChevronDown size={13} /> : <ChevronRight size={13} />}</Button></CollapsibleTrigger>
        : <span className="tree-status" title={statusLabel(t, work.status)}>{work.status === 'done' ? <CircleCheck size={14} /> : work.status === 'doing' ? <span className="live-dot" /> : depth ? <Circle size={11} /> : <GitBranch size={14} />}</span>}
