...
| `Workspace.tsx` | `Workspace`:一个 work 的页面——头部(目标 / 状态 / 拆分 / 添加一列),下面是画布:**几列,每列从上到下摆会话卡片**。`layout()` 把后端画布和会话清单对齐(没提到的会话补到第一列,没了的丢掉);`edit()` 整份 PUT 画布带 version,409 就重载;会话可收起、上下左右挪;列可收起成窄边,空了才能删;每列底部「添加会话」(`AttachDialog`,建好后 `placeNew` 挪到那一列) |
| `PanelView.tsx` | `PanelView`:一个会话的身体——终端(有 ttyd 就 iframe,否则快照轮询)/ agent 的对话记录 / 网页 iframe;复制地址、新窗口打开、重连、结束会话 |
