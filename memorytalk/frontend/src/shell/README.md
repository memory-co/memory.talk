...
| `Workspace.tsx` | `Workspace`:一个 work 的页面——头部(目标 / 状态 / 拆分 / 添加一列),下面是画布:**几列,每列从上到下摆工作单元卡片**。`layout()` 把后端画布和工作单元清单对齐(没提到的工作单元补到第一列,没了的丢掉);`edit()` 整份 PUT 画布带 version,409 就重载;工作单元可收起、上下左右挪;列可收起成窄边,空了才能删;每列底部「添加工作单元」弹出 `NewWorklet`(弹层里像浏览器新标签页:上面一条地址栏,块即 URI;下面几块应用,点一块只是把 URI 填进地址栏,回车或「打开」才建),建好后 `placeNew` 挪到那一列 |
