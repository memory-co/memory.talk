...
import { dateLabel, layerLabel, type MetaObject, type RecentPage, type Revision, type TreeView } from '@/lib/types';
...
export function Library({ filter = ALL, layer, path, file, dir: dirProp, onSelect, compact = false, work }: Selection & {
  ...
  const tree = useQuery({ queryKey: ['tree', filter, here], queryFn: ({ signal }) => api<TreeView>(`/metas/tree?${new URLSearchParams({ path: here, ...(filter === ALL ? {} : { layer: filter }) })}`, { signal }) });
...
function RecentList({ filter, selected, onOpen }: { filter: string; selected: FileRef | null; onOpen: (ref: FileRef) => void }) {
  ...
  const page = useQuery({ queryKey: ['recent', filter, cursor], queryFn: ({ signal }) => api<RecentPage>(`/metas/recent?${new URLSearchParams({ ...(filter === ALL ? {} : { layer: filter }), limit: '30', ...(cursor ? { before: cursor } : {}) })}`, { signal }) });
...
export function FilePage({ layer, path, file, creating, onClose, onOpen, work }: {
  ...
  const object = useQuery({ queryKey: ['object', layer, objectPath, revision], queryFn: ({ signal }) => api<MetaObject>(`/metas/${encodeURIComponent(layer)}/${pathPart(objectPath)}${revision ? `?rev=${encodeURIComponent(revision)}` : ''}`, { signal }), enabled: !!objectPath });
  const history = useQuery({ queryKey: ['history', layer, objectPath], queryFn: ({ signal }) => api<Revision[]>(`/metas/history/${encodeURIComponent(layer)}/${pathPart(objectPath)}`, { signal }), enabled: !!objectPath && tab === 'history' });
  ...
  const target = `/metas/${encodeURIComponent(layer)}/${pathPart(targetPath)}`;
