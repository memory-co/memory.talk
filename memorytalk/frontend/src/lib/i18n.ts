...
const en = {
  ...
  'worklet.iframeTitle': '{scheme} worklet',
  ...
  'settings.environment': 'Environment', 'settings.terminal': 'Terminal window (tmuxd)', 'settings.tmuxSocket': 'tmux socket', 'worklet.noWindow': 'No window for this worklet', 'worklet.noWindowText': 'The server did not return a browser address.', 'settings.serviceStatus': 'Service', 'settings.connected': 'Connected', 'settings.workspace': 'Default workspace', 'settings.home': 'Data directory', 'settings.store': 'Storage',
...
const zh: Record<Key, string> = {
  ...
  'worklet.iframeTitle': '{scheme} 工作单元',
  ...
  'settings.environment': '工作环境', 'settings.terminal': '终端窗口（tmuxd）', 'settings.tmuxSocket': 'tmux socket', 'worklet.noWindow': '这个工作单元没有画面', 'worklet.noWindowText': 'server 没有交回可打开的地址。', 'settings.serviceStatus': '服务状态', 'settings.connected': '已连接', 'settings.workspace': '默认工作目录', 'settings.home': '数据目录', 'settings.store': '存储方式',
