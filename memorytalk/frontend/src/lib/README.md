...
| `store.ts` | `usePreferences`(zustand + persist):`user` / `role` / `token` / `setupRequired`(不落盘)/ `collapsed` / `sessions`(每个 work 选中的会话)/ `locale`;`setSession` / `clearSession`(名字留着给登录页预填)/ `setLocale` … |
...
| `types.ts` | 后端模型的 TS 形状(`Work` / `Session` / `Canvas` / `User` / `Layer` / `TreeView` / `RecentPage` / `SearchResult` …)和几个标签函数(`statusLabel` / `layerLabel` / `sessionLabel` / `dateLabel` / `flattenWorks`) |
