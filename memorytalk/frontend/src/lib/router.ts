...
export type Route = { page: 'home' | 'work' | 'library' | 'settings'; work?: string; filter?: string; layer?: string; path?: string };
...
export function useRoute(): Route {
  ...
  if (pathname === '/library') return { page: 'library', filter: query.get('filter') || 'all', layer: query.get('layer') || undefined, path: query.get('path') || undefined };
...
export function navigate(route: Route) {
  ...
  if (route.layer && route.path) { query.set('layer', route.layer); query.set('path', route.path); }
