...
async def _seed(services):
    ingest = await services.sessions.ingest(IngestSessionRequest(
    ...
    card1 = (await services.cards.create(CreateCardRequest(
        summary="one", rounds=[CardRoundsItem(session_id=sid, indexes="1")],
    ))).card_id
    card2 = (await services.cards.create(CreateCardRequest(
        summary="two", rounds=[CardRoundsItem(session_id=sid, indexes="1")],
    ))).card_id
...
async def test_create_user_link_card_to_card(services):
    ...
    r = await services.links.create(CreateLinkRequest(
