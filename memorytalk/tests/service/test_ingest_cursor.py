...
# IngestService no longer prefixes/mints session_ids — the caller
# (SyncWatcher) mints the canonical id via the adapter's
# ``mint_session_id`` and hands it in. These tests use a canonical-
# looking id so the service treats it as authoritative.
_SID = "sess-deadbeef-abc"


async def test_ensure_session_returns_none_for_unknown(app):
    res = await app.state.ingest.ensure_session(EnsureSessionRequest(
        source="claude-code", session_id=_SID,
    ))
    assert res.session_id == _SID
...
async def test_append_rounds_first_call_creates_session(app):
    ...
    res = await app.state.ingest.append_rounds(AppendRoundsRequest(
        session_id=_SID, source="claude-code",
