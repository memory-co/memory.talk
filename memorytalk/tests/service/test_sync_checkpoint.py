...
async def test_checkpoint_roundtrip(tmp_path):
    ...
    try:
        assert await store.get("claude-code", "s1") is None
        await store.upsert(
            "claude-code", "s1",
            sha256="aaa", last_round_id="r5", line_offset=42,
            updated_at="2026-05-20T00:00:00Z",
        )
        row = await store.get("claude-code", "s1")
        assert row == {
        ...
        assert (await store.get("claude-code", "s1"))["sha256"] == "bbb"
        assert await store.count() == 1
    finally:
        await store.close()
...
async def test_checkpoint_keyed_by_source_and_session(tmp_path):
    """(source, session_id) PK lets two adapters share a session_id
    without colliding — important for the future remote ingest path."""
    store = await SyncCheckpointStore.create(tmp_path / "sync.db")
    try:
        await store.upsert("claude-code", "s1", "a", "r1", 1, "t1")
        await store.upsert("codex",       "s1", "b", "r2", 2, "t2")
        cc = await store.get("claude-code", "s1")
        cx = await store.get("codex", "s1")
