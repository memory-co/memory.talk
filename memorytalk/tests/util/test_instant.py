"""Timezone-aware instant parsing + a session's last_round_update_time.

These underpin explore's prior/posterior split, so the ordering must be
*temporal*, never lexical over the raw heterogeneous timestamp strings.
"""
...
def test_offset_timestamps_compared_temporally_not_lexically():
    ...
    # +08:00 string sorts LARGER ('2' > '1'), so a string-max impl would
    # wrongly pick the earlier one. Temporal max must pick 13:00Z.
...
def test_falls_back_to_created_at_when_no_round_timestamp():
    # Rounds carry no (parseable) timestamps → use created_at (the
    # conversation-start proxy), not synced_at.
    got = last_round_update_time(
        [None, "", "garbage"],
...
def test_falls_back_to_synced_at_when_no_created_at():
    got = last_round_update_time(
        [],
        created_at="",
        synced_at="2026-06-01T08:30:00Z",
    )
