"""GET /v3/insights — structural list (tag / since / until).
...
async def _create_card(client, sid: str, insight: str, *, tags: dict | None = None) -> str:
    ...
    if tags is not None:
        body["tags"] = tags
    r = await client.post("/v3/insights", json=body)
    r.raise_for_status()
...
async def test_empty_db_returns_zero(client):
    r = await client.get("/v3/insights")
...
async def test_lists_existing_cards(client):
    ...
    body = (await client.get("/v3/insights")).json()
    assert body["total"] == 2
...
async def test_limit_caps_returned_not_total(client):
    ...
    r = await client.get("/v3/insights", params={"limit": 2})
    body = r.json()
    assert body["total"] == 4
...
async def test_filter_tag_equality(client):
    ...
    r = await client.get("/v3/insights", params={"tag": "project=billing"})
    body = r.json()
    assert body["total"] == 1
...
async def test_filter_tag_presence(client):
    ...
    r = await client.get("/v3/insights", params={"tag": "project"})
    body = r.json()
...
async def test_filter_tag_ne_excludes_null(client):
    ...
    r = await client.get("/v3/insights", params={"tag": "status!=draft"})
    body = r.json()
...
async def test_filter_tag_in_list(client):
    ...
    r = await client.get("/v3/insights", params={"tag": "status=wip,review"})
    body = r.json()
...
async def test_filter_since_until(client, app):
    ...
    r = await client.get("/v3/insights", params={"since": "2026-05-01T00:00:00Z"})
    assert r.json()["total"] == 1
    assert r.json()["cards"][0]["card_id"] == b


async def test_invalid_iso_rejected(client):
    r = await client.get("/v3/insights", params={"since": "yesterday"})
...
async def test_create_with_invalid_tags_rejected(client):
    ...
    assert r.status_code == 400
    # And nothing landed.
    assert (await client.get("/v3/insights")).json()["total"] == 0
