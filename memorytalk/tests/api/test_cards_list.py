"""GET /v3/cards — structural list (tag / since / until).
...
async def _create_card(client, sid: str, insight: str, *, tags: dict | None = None) -> str:
    ...
    if tags is not None:
        body["tags"] = tags
    r = await client.post("/v3/cards", json=body)
    r.raise_for_status()
...
async def _set_created_at(app, cid: str, iso: str) -> None:
    await app.state.db.conn.execute(
        "UPDATE cards SET created_at = ? WHERE card_id = ?", (iso, cid),
    )
...
async def test_lists_existing_cards(client):
    ...
    body = (await client.get("/v3/cards")).json()
    assert body["total"] == 2
...
async def test_limit_caps_returned_not_total(client):
    ...
    r = await client.get("/v3/cards", params={"limit": 2})
    body = r.json()
    assert body["total"] == 4
...
async def test_filter_tag_equality(client):
    ...
    r = await client.get("/v3/cards", params={"tag": "project=billing"})
    body = r.json()
    assert body["total"] == 1
...
async def test_filter_tag_presence(client):
    ...
    r = await client.get("/v3/cards", params={"tag": "project"})
    body = r.json()
...
async def test_filter_tag_ne_excludes_null(client):
    ...
    r = await client.get("/v3/cards", params={"tag": "status!=draft"})
    body = r.json()
...
async def test_filter_tag_in_list(client):
    ...
    r = await client.get("/v3/cards", params={"tag": "status=wip,review"})
    body = r.json()
...
async def test_filter_since_until(client, app):
    ...
    r = await client.get("/v3/cards", params={"since": "2026-05-01T00:00:00Z"})
    assert r.json()["total"] == 1
    assert r.json()["cards"][0]["card_id"] == b


async def test_invalid_iso_rejected(client):
    r = await client.get("/v3/cards", params={"since": "yesterday"})
...
async def test_create_with_invalid_tags_rejected(client):
    ...
    assert r.status_code == 400
    # And nothing landed.
    assert (await client.get("/v3/cards")).json()["total"] == 0
