"""PATCH /v3/cards/{cid}/tags — kv tag set / unset / query.
...
async def test_empty_patch_returns_current_tags(client):
    ...
    r = await client.patch(f"/v3/cards/{cid}/tags", json={})
...
async def test_unset_removes_keys(client):
    ...
    r = await client.patch(f"/v3/cards/{cid}/tags", json={"unset": ["a"]})
...
async def test_invalid_key_rejected(client):
    cid = await _seed(client)
    r = await client.patch(f"/v3/cards/{cid}/tags", json={"set": {"1bad": "x"}})
...
async def test_too_many_tags_rejected(client):
    ...
    r = await client.patch(f"/v3/cards/{cid}/tags", json={"set": payload})
    assert r.status_code == 400
...
async def test_404_on_missing_card(client):
    r = await client.patch(
        "/v3/cards/card_nonexistent/tags",
        json={"set": {"x": "y"}},
    )
