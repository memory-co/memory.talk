"""memory.talk meta(col)—— 认知层:层、树、检索、对象(目录里的文件)CRUD、历史。"""
...
def c_layers(api, a):
    ls = api.call("GET", "/api/metas/layers")
    lines = []
    for l in ls:
    ...
    out(ls, a.json, "\n".join(lines))
def c_tree(api, a):
    v = api.call("GET", "/api/metas/tree", params={"path": a.path or "", "candidate": a.candidate, "layer": a.layer, "recursive": "1" if a.recursive else None})
    if a.json:
        return out(v, True)
    for i in v["items"]:
...
def c_recent(api, a):
    page = api.call("GET", "/api/metas/recent", params={"layer": a.layer, "path": a.path or "", "limit": a.limit, "before": a.before})
    lines = [f"{i['sha'][:7]}  {i['date'][:16]}  {i['author']:<10} [{i['layer']}] {i['path']}" + (f"  ({', '.join(i['files'])})" if i['files'] else "") for i in page["items"]]
    if page.get("next"):
        lines.append(f"…  再往下:--before {page['next']}")
    out(page, a.json, "\n".join(lines) or "(空)")
def c_read(api, a):
    o = api.call("GET", f"/api/metas/{a.layer}/{a.path}", params={"rev": a.rev})
    if a.json:
        return out(o, True)
...
def _body(a) -> dict:
    ...
    if a.put:
        ...
        body["files"] = files
    return body
def c_write(api, a): out(api.call("POST", f"/api/metas/{a.layer}/{a.path}", json_body=_body(a)), a.json, f"[{a.layer}] {a.subject or 'write ' + a.path}")
def c_edit(api, a): out(api.call("PUT", f"/api/metas/{a.layer}/{a.path}", json_body=_body(a)), a.json, f"[{a.layer}] {a.subject or 'edit ' + a.path}")
def c_rm(api, a): api.call("DELETE", f"/api/metas/{a.layer}/{a.path}", params={"reason": a.reason}); print(f"[{a.layer}] delete {a.path}")
def c_log(api, a):
    revs = api.call("GET", f"/api/metas/history/{a.layer}/{a.path}")
    out(revs, a.json, "\n".join(f"{r['sha'][:7]}  {r['author']:<10} {r['date'][:19]}  {r['subject']}" + (f"\n         {r['body']}" if r['body'] else "") for r in revs))
def c_manager(api, a):
    if a.set:
        r = api.call("PUT", "/api/metas/manager", params={"path": a.path or ""}, json_body={"work": a.set, "reason": a.reason or ""})
    elif a.unset:
        api.call("DELETE", "/api/metas/manager", params={"path": a.path or "", "reason": a.reason or ""}); r = None
    else:
        r = api.call("GET", "/api/metas/manager", params={"path": a.path or ""})
    out(r, a.json, f"manager: {r['work']}  (在 {r['dir'] or '/'})" if r else "没人管")
def c_managed(api, a):
    items = api.call("GET", "/api/metas/managed", params={"work": a.work_filter})
...
def register(top) -> None:
    c = top.add_parser("meta", aliases=["metas"], help="认知层").add_subparsers(dest="sub", required=True)
