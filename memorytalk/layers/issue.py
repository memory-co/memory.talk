...
from memorytalk.models.collections import FieldSpec
...
class Origin(BaseModel):
    """出处:work 的哪些 round,或 Collections 里的一个 origin 路径。"""
...
# ---- 行为:schema 之上的领域动作。签名统一 (collections, path, payload, ctx) -> Obj ----

def _position(issue: dict, pid: str) -> dict:
    for p in issue["positions"]:
        if p["id"] == pid:
            return p
    from memorytalk.services.collections import CollectionsError
    raise CollectionsError("not_found", f"{pid} 不是这个 issue 的立场", 404)


def position(collections, path: str, payload: dict, ctx) -> dict:
    """加一个立场(只增不改)。payload: claim, origin?, reason?"""
    issue = collections.get("issue", path).body
    ...
    collections.write("issue", path, issue, f"position {path}#{pos['id']}: {pos['claim'][:60]}",
...
def argue(collections, path: str, payload: dict, ctx) -> dict:
    ...
    collections.write("issue", path, issue, f"argue {path}#{pos['id']} {sign}", payload.get("reason", ""), ctx)
    return view(issue)
...
def link(collections, path: str, payload: dict, ctx) -> dict:
    ...
    if edge not in issue["links"]:
        ...
        collections.write("issue", path, issue, f"link {path} {edge['type']} {edge['target']}", payload.get("reason", ""), ctx)
    return view(issue)


def spawn(collections, path: str, payload: dict, ctx) -> dict:
    ...
    if payload["work_id"] not in pos["spawned_works"]:
        ...
        collections.write("issue", path, issue, f"spawn {path}#{pos['id']} -> {payload['work_id']}", payload.get("reason", ""), ctx)
    return view(issue)


def decide(collections, path: str, payload: dict, ctx) -> dict:
    ...
    if collections.exists("card", card_path):
        raise CollectionsError("exists", f"卡已存在:{card_path}", 409)
    ...
    collections.write("issue", path, issue, f"decide {path}#{pos['id']} -> card {card_path}", payload.get("reason", ""), ctx,
    ...
    try:
        collections.create("card", card_path, card, payload.get("reason", ""), ctx, extra_trailer=trailer)
    except Exception:
        collections.write("issue", path, before, f"revert decide {path}#{pos['id']}", "card 没写成,退回", ctx,
                      extra_trailer=trailer)
        raise
    return view(issue)


def read_view(collections, path: str, obj: dict) -> dict:
    return view(obj)
