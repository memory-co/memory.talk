...
def test_card_is_one_file_with_frontmatter_and_body(client, svc):
    r = client.post(f"/api/collections/card/{CP}", json={"files": {"readme.md": CARD}})
    assert r.status_code == 201 and r.json()["title"] == "配置只来自环境变量"
    assert svc.collections.repo.read(f"{CP}.card/readme.md").decode() == CARD
    assert client.get(f"/api/collections/card/{CP}").json()["files"] == {"readme.md": CARD}
...
def test_three_layers_live_side_by_side(client):
    ...
    names = {i["name"]: i for i in client.get("/api/collections/tree", params={"path": "memory.talk/配置"}).json()["items"]}
...
def test_rejections_carry_the_reason(client):
    r = client.post(f"/api/collections/card/{CP}", json={"files": {"notes.md": "x"}})
    assert r.status_code == 422 and "notes.md" in r.json()["message"] and "readme.md" in r.json()["message"]
    r = client.post(f"/api/collections/card/{CP}", json={"files": {"readme.md": "---\nscore: 1\n---\n\nx"}})
...
def test_edit_touches_only_the_named_files_and_null_deletes(client):
    client.post(f"/api/collections/issue/{IP}", json={"files": {"readme.md": "旧", "positions/甲.md": "a", "positions/乙.md": "b"}})
    o = client.put(f"/api/collections/issue/{IP}", json={"files": {"readme.md": "新"}}).json()
    assert o["files"] == {"positions/甲.md": "a", "positions/乙.md": "b", "readme.md": "新"}
    o = client.put(f"/api/collections/issue/{IP}", json={"files": {"positions/乙.md": None}}).json()
    assert o["files"] == {"positions/甲.md": "a", "readme.md": "新"}
    r = client.put(f"/api/collections/issue/{IP}", json={"files": {"readme.md": None}})
    assert r.status_code == 422 and "不能删" in r.json()["message"]
    assert client.put(f"/api/collections/issue/{IP}", json={"files": {"readme.md": "新"}}).status_code == 400   # 没变
...
def test_dry_run_checks_without_committing(client):
    r = client.post(f"/api/collections/issue/{IP}", json={"files": {"readme.md": ""}}, params={"dry_run": 1})
    assert r.status_code == 201 and r.json() == {"ok": True, "reason": None}
    assert client.get(f"/api/collections/issue/{IP}").status_code == 404
    r = client.post(f"/api/collections/issue/{IP}", json={"files": {"notes.txt": ""}}, params={"dry_run": 1}).json()
