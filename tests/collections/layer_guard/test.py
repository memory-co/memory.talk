...
def test_cross_layer_commit_is_rejected(client, svc):
    ...
    with pytest.raises(CollectionsError) as e:
        svc.collections.commit("card", "touch", {f"{IP}.issue/readme.md": b"x"}, [], "", Ctx())
    assert e.value.code == "guard" and "[issue]" in str(e.value)


def test_same_layer_edit_passes(client, svc):
    client.post(f"/api/collections/issue/{IP}", json={"files": {"readme.md": ""}})
    assert svc.collections.commit("issue", "edit", {f"{IP}.issue/readme.md": "改了".encode()}, [], "", Ctx())


def test_each_layer_branch_only_has_its_own_commits(client):
    client.post(f"/api/collections/issue/{IP}", json={"files": {"readme.md": ""}})
    client.post("/api/collections/card/x/卡", json={"files": {"readme.md": "卡"}})
