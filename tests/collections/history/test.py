...
def test_history_lists_edits_newest_first_with_reason(client):
    client.post(f"/api/collections/card/{CP}", json={"files": {"readme.md": "v1"}, "reason": "初写"})
    client.put(f"/api/collections/card/{CP}", json={"files": {"readme.md": "v2"}, "reason": "补理由"})
...
def test_read_at_revision_returns_the_old_body(client):
    client.post(f"/api/collections/card/{CP}", json={"files": {"readme.md": "v1"}})
    client.put(f"/api/collections/card/{CP}", json={"files": {"readme.md": "v2"}})
    old_sha = client.get(f"/api/collections/history/card/{CP}").json()[1]["sha"]
    assert client.get(f"/api/collections/card/{CP}", params={"rev": old_sha}).json()["files"]["readme.md"] == "v1"
