"""collections/recent -- recently changed objects, paginated. See README.md."""
...
def _seed(client):
    ...
    client.put(f"/api/collections/issue/{IP}", json={"files": {"positions/甲.md": "a"}, "subject": f"position {IP}: 甲"})


def test_objects_once_each_newest_first_with_touched_files(client):
    _seed(client)
    page = client.get("/api/collections/recent").json()
...
def test_filters_by_layer_and_path(client):
    _seed(client)
    assert [i["path"] for i in client.get("/api/collections/recent", params={"layer": "card"}).json()["items"]] == [CP]
    assert [i["layer"] for i in client.get("/api/collections/recent", params={"path": "memory.talk/配置"}).json()["items"]] == ["issue", "card"]
    assert client.get("/api/collections/recent", params={"layer": "nope"}).status_code == 404


def test_pagination_walks_down_the_log(client):
    _seed(client)
    first = client.get("/api/collections/recent", params={"limit": 1}).json()
    assert [i["layer"] for i in first["items"]] == ["issue"] and first["next"]
    second = client.get("/api/collections/recent", params={"limit": 1, "before": first["next"]}).json()
    assert [i["layer"] for i in second["items"]] == ["card"] and second["next"]
    rest = client.get("/api/collections/recent", params={"before": second["next"]}).json()
...
def test_mechanism_files_do_not_count(client):
    assert client.get("/api/collections/recent").json() == {"items": [], "next": None}       # 只有 collections.json 的始祖提交
