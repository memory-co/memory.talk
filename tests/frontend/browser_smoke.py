    ...
    with open(tmp + '/server.log', 'w') as log:
        ...
        try:
            ...
            with sync_playwright() as p:
                ...
                page.get_by_role('button', name='添加会话', exact=True).last.click()
                page.get_by_role('combobox', name='会话类型').click()
                ...
                end_button = page.get_by_role('button', name='结束并移除会话')
                end_button.click()
                expect(page.get_by_role('alertdialog')).to_be_visible()
                expect(page.get_by_role('button', name='保留会话')).to_be_focused()
                page.keyboard.press('Escape')
                expect(end_button).to_be_focused()
                end_button.click()
                page.route('**/sessions/*', lambda route: route.fulfill(status=503, content_type='application/json', body=json.dumps({'data':None,'message':'临时失败','error':'unavailable'})) if route.request.method == 'DELETE' else route.continue_())
                page.get_by_role('button', name='结束会话', exact=True).click()
                expect(page.get_by_role('alertdialog')).to_be_visible()
                expect(page.get_by_role('button', name='结束会话', exact=True)).to_be_enabled()
                page.get_by_role('button', name='保留会话').click()
                page.unroute('**/sessions/*')
                expect(end_button).to_be_focused()
                # Add a second real session and switch with the keyboard.
                page.get_by_role('button', name='添加会话', exact=True).click()
                page.get_by_role('button', name='打开会话', exact=True).click()
                tabs = page.get_by_role('tablist', name='会话切换')
                ...
                browser.close()
            print('PASS: real API create/update work, nested work, bash session, library sidebar, card create/edit, metadata preservation, issue rendering, history, search, user registration, persisted identity, shortcuts, mobile navigation, error UI; dialog focus, keyboard tabs, mobile focus trap, failed destructive action; no browser runtime errors or accessibility warnings.')
