...
def test_unregistered_name_in_header_is_404(client, H):
    ...
    assert client.post("/api/collections/card/x/y", json={"files": {"readme.md": "y"}}, headers=H("nobody")).status_code == 404
...
def test_anonymous_requests_still_work(client):
    ...
    assert client.post("/api/collections/card/x/匿名卡", json={"files": {"readme.md": "匿名卡"}}).status_code == 201
