...
**场景化**:每个目录一个场景,自带 `README.md`(在测什么 / 不在这测什么 / fixture 来源)和 `test.py`;test.py 里每个函数只断言一件事,函数名就是结论。目录按层分:`providers/` `auth/`(门:setup / 登录 / token) `collections/`(从 `basic/` 看起:两层各一个文件,stack 看到两个) `works/` `work_servers/` `adapters/` `users/` `search/` `cli/`。
