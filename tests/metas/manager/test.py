"""metas/manager -- nearest manager.json. See README.md."""
...
def test_unmanaged_object_is_listed_until_a_manager_exists(client):
    w = _setup(client)
    assert client.get("/api/metas/manager", params={"path": IP}).json() is None
    assert [o["path"] for o in client.get("/api/metas/managed").json()] == [IP]
    client.put("/api/metas/manager", params={"path": "memory.talk"}, json={"work": w["id"]})
    assert client.get("/api/metas/managed").json() == []
    assert [o["path"] for o in client.get("/api/metas/managed", params={"work": w["id"]}).json()] == [IP]


def test_folder_manager_covers_objects_beneath(client):
    w = _setup(client)
    m = client.put("/api/metas/manager", params={"path": "memory.talk"}, json={"work": w["id"]}).json()
    assert m == {"dir": "memory.talk", "work": w["id"]}
    assert client.get("/api/metas/manager", params={"path": IP}).json()["work"] == w["id"]
...
def test_object_level_manager_wins_over_folder(client):
    ...
    client.put("/api/metas/manager", params={"path": "memory.talk"}, json={"work": w["id"]})
    client.put("/api/metas/manager", params={"path": IP}, json={"work": w2["id"]})
    assert client.get("/api/metas/manager", params={"path": IP}).json() == {"dir": f"{IP}.issue", "work": w2["id"]}
...
def test_unset_falls_back_to_the_parent_folder(client):
    ...
    client.put("/api/metas/manager", params={"path": "memory.talk"}, json={"work": w["id"]})
    client.put("/api/metas/manager", params={"path": IP}, json={"work": w2["id"]})
    assert client.delete("/api/metas/manager", params={"path": IP}).status_code == 200
    assert client.get("/api/metas/manager", params={"path": IP}).json()["work"] == w["id"]
...
def test_manager_json_belongs_to_the_layer_of_its_directory(client):
    w = _setup(client)
    client.put("/api/metas/manager", params={"path": "memory.talk"}, json={"work": w["id"]})
    client.put("/api/metas/manager", params={"path": IP}, json={"work": w["id"]})
    log = git_log(client)
