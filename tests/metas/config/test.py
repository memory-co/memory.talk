"""metas/config -- metas.json anchor. See README.md."""
import json
import pytest
import subprocess
...
def test_anchor_is_the_only_file_in_the_root_commit(svc):
    ...
    assert files == ["metas.json"]


def test_config_lists_builtin_layers_bottom_first(client):
    cfg = client.get("/api/metas/config").json()["config"]
...
def test_a_new_layer_file_is_registered_by_a_commit_on_startup(client):
    with _restart_with_layer("decision", DECISION) as c2:
        ...
        r = c2.get("/api/metas/config").json()
    assert r["config"]["layers"][-1] == {"name": "decision", "builtin": False}
    assert r["history"][0]["subject"] == "[origin] metas: add layers decision"
    assert r["history"][-1]["subject"].startswith("[origin] metas: init")


def test_no_other_mechanism_files_in_the_repo(client, svc):
    with _restart_with_layer("decision", DECISION):
        pass
    files = set(svc.metas.repo.tree())
    assert files == {"metas.json"}


@pytest.mark.skip(reason="/api/metas/manager 暂时注释掉了,等 work 实现后一起启用")
def test_config_file_is_invisible_to_tree_catalog_and_inbox(client):
    assert client.get("/api/metas/tree").json()["items"] == []
    assert client.get("/api/metas/tree", params={"layer": "origin", "recursive": 1}).json()["items"] == []
    w = client.post("/api/works", json={"goal": "x"}).json()
    client.put("/api/metas/manager", params={"path": ""}, json={"work": w["id"]})
    with _restart_with_layer("decision", DECISION):
        pass
