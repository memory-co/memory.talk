"""metas/protocol_rules -- the engine checks each protocol rule. See README.md."""
import pytest
...
@pytest.fixture
def issue(client):
    client.post(f"/api/metas/issue/{IP}", json={"files": {"readme.md": "背景", A: "为什么"}})
    return IP


def _put(client, files, headers=None, **kw):
    return client.put(f"/api/metas/issue/{IP}", json={"files": files, **kw}, headers=headers)
...
def test_object_directory_rules(client, home, svc):
    ...
    assert client.post(f"/api/metas/card/{IP}.issue/里面的卡", json={"files": {"readme.md": ""}}).status_code == 422   # 嵌套
    client.post(f"/api/metas/issue/{IP}", json={"files": {"readme.md": ""}})
    r = client.post("/api/metas/origin/" + IP + ".issue/attach.txt", json={"content": "x"})
    assert r.status_code == 400 and "对象目录里" in r.json()["message"]                                     # origin 不能进对象目录
    spec = svc.metas.layer("issue")
...
def test_one_bad_file_rejects_the_whole_batch(client, svc, issue):
    ...
    assert svc.metas.repo.read(f"{IP}.issue/positions/乙.md") is None
