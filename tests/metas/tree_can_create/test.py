"""metas/tree_can_create -- can_create and candidate on the tree endpoint. See README.md."""
IP = "memory.talk/配置/该走文件还是环境变量"


def test_plain_directory_lists_object_kinds_per_layer(client):
    v = client.get("/api/metas/tree", params={"path": "memory.talk/配置"}).json()
...
def test_object_directory_lists_file_kinds_with_state(client):
    ...
    v = client.get("/api/metas/tree", params={"path": f"{IP}.issue"}).json()
...
def test_inside_an_object_items_are_its_files(client):
    ...
    inner = client.get("/api/metas/tree", params={"path": f"{IP}.issue/positions"}).json()["items"]
...
def test_subdirectory_only_lists_kinds_that_land_there(client):
    ...
    v = client.get("/api/metas/tree", params={"path": f"{IP}.issue/positions"}).json()
...
def test_candidate_in_a_plain_directory_is_an_object_name(client):
    ok = client.get("/api/metas/tree", params={"path": "memory.talk/配置", "candidate": "要不要加配置文件.issue"}).json()["candidate"]
    assert ok == {"name": "要不要加配置文件.issue", "matches": {"layer": "issue", "name": "要不要加配置文件"}, "exists": False, "can": True}
    client.post(f"/api/metas/issue/{IP}", json={"files": {"readme.md": ""}})
    dup = client.get("/api/metas/tree", params={"path": "memory.talk/配置", "candidate": "该走文件还是环境变量.issue"}).json()["candidate"]
    assert dup["exists"] and not dup["can"]
    no = client.get("/api/metas/tree", params={"path": "memory.talk/配置", "candidate": "笔记.txt"}).json()["candidate"]
    assert no["matches"] is None and not no["can"]


def test_candidate_in_an_object_is_a_file_path(client):
    client.post(f"/api/metas/issue/{IP}", json={"files": {"readme.md": ""}})
    q = lambda c: client.get("/api/metas/tree", params={"path": f"{IP}.issue", "candidate": c}).json()["candidate"]
