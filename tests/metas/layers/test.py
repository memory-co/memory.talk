"""metas/layers -- built-in layer protocols. See README.md."""
import subprocess


def test_three_builtin_layers_bottom_first(client):
    assert [l["name"] for l in client.get("/api/metas/layers").json()] == ["origin", "issue", "card"]


def test_origin_has_no_suffix_and_no_object_rule(client):
    origin = client.get("/api/metas/layers").json()[0]
...
def test_issue_protocol_describes_two_kinds_of_files(client):
    issue = client.get("/api/metas/layers").json()[1]
...
def test_card_protocol_is_one_file_with_fields(client):
    card = client.get("/api/metas/layers").json()[2]
...
def test_repo_has_a_branch_per_layer_and_stack(svc):
    out = subprocess.run(["git", "branch", "--list"], cwd=svc.metas.repo.root, capture_output=True, text=True).stdout
