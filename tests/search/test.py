...
def _seed(client):
    ...
    client.post(f"/api/metas/issue/{IP}", json={"files": {"readme.md": "配置该走文件还是环境变量?", "positions/只用环境变量.md": "够用"}})
    client.post("/api/metas/card/其他/一张卡", json={"files": {"readme.md": "多一份状态要同步"}})
...
def test_hits_come_grouped_by_kind(client):
    ...
    assert r["counts"] == {"work": 1, "meta": 1, "user": 1}


def test_metas_search_is_grep_over_every_file(client):
    _seed(client)
    hits = client.get("/api/search", params={"q": "一份状态"}).json()["hits"]
    assert [(h["kind"], h["layer"], h["id"]) for h in hits] == [("meta", "card", "其他/一张卡")]


def test_limit_is_per_kind_and_counts_are_before_truncation(client):
    _seed(client)
    r = client.get("/api/search", params={"q": "配", "limit": 1}).json()
    assert [h["kind"] for h in r["hits"]] == ["work", "meta"] and r["counts"]["work"] == 1 and r["counts"]["meta"] >= 1
