...
async def test_creates_file_for_utc_date(tmp_path):
    ...
    await w.append({"k": "v"}, now=datetime(2026, 4, 22, 14, 30, tzinfo=timezone.utc))
    p = base / "2026-04-22.jsonl"
    assert p.exists()
    assert json.loads(p.read_text().strip()) == {"k": "v"}


async def test_multiple_records_same_day(tmp_path):
    base = tmp_path / "logs"
    w = DatedJsonlWriter(base)
    d = datetime(2026, 4, 22, 14, 30, tzinfo=timezone.utc)
    await w.append({"a": 1}, now=d)
    await w.append({"b": 2}, now=d)
    lines = (base / "2026-04-22.jsonl").read_text().strip().split("\n")
    assert [json.loads(l) for l in lines] == [{"a": 1}, {"b": 2}]


async def test_splits_by_utc_day(tmp_path):
    base = tmp_path / "logs"
    w = DatedJsonlWriter(base)
    await w.append({"k": "x"}, now=datetime(2026, 4, 22, 23, 59, 59, tzinfo=timezone.utc))
    await w.append({"k": "y"}, now=datetime(2026, 4, 23, 0, 0, 0, tzinfo=timezone.utc))
...
async def test_ensure_ascii_false(tmp_path):
    base = tmp_path / "logs"
    w = DatedJsonlWriter(base)
    await w.append({"query": "向量选型"}, now=datetime(2026, 4, 22, 0, 0, 0, tzinfo=timezone.utc))
