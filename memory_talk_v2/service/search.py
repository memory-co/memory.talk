...
class SearchService:
    ...
    def search(self, payload: dict) -> dict:
        ...
        self.vectors.ensure_fts_index("cards")
        self.vectors.ensure_fts_index("sessions")
        ...
        self.search_jsonl.append(persisted, now=now)
    ...
    def _active_links(self, object_id: str, now) -> list[dict]:
        out: list[dict] = []
        for l in self.db.links_touching(object_id):
    ...
    def _search_cards(self, query, whitelist, top_k, now) -> list[dict]:
        ...
        if query.strip():
            ...
            raw = self.vectors.hybrid_search_cards(vector, query, whitelist=whitelist, top_k=top_k)
    ...
    def _search_sessions(self, query, whitelist, top_k, now) -> list[dict]:
        ...
        if whitelist is not None and len(whitelist) == 0:
            return hits
        if query.strip():
            raw = self.vectors.fts_search_sessions(query, whitelist=whitelist, top_k=top_k)
            for rank, h in enumerate(raw, start=1):
                sid = h.get("session_id")
                s = self.db.get_session(sid)
                if not s:
                    continue
                rounds = self.db.list_rounds(sid)
                hits.append({
