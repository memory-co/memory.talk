"""/api/collect —— 认知层:层、树、检索、manager、对象 CRUD、历史、行为。"""
...
from models.collect import (CatalogDir, InboxItem, LayerCreate, LayerInfo, Manager, ManagerPut, Obj,
                            ObjCreate, ObjUpdate, Revision, SearchHit, TreeItem)
from services.collect import CollectError, CollectService, Ctx
...
def layers(svc: CollectService = Depends(collect)):
    return svc.layer_infos()


@router.post("/layers", response_model=LayerInfo, status_code=201, summary="加一个用户层:一份 schema YAML")
def add_layer(req: LayerCreate, svc: CollectService = Depends(collect), c: Ctx = Depends(ctx)):
...
def tree(path: str = "", svc: CollectService = Depends(collect)):
    return svc.tree(path)


@router.get("/search", response_model=list[SearchHit], summary="git grep 整个 Collect(可限定层)")
def search(q: str, layer: str | None = None, svc: CollectService = Depends(collect)):
    return svc.search(q, layer)


@router.get("/manager", response_model=Manager | None, summary="这个路径归谁管(最近的 manager.json)")
def get_manager(path: str = "", svc: CollectService = Depends(collect)):
...
def delete_manager(path: str = "", reason: str = "", svc: CollectService = Depends(collect), c: Ctx = Depends(ctx)):
...
def history(layer: str, path: str, svc: CollectService = Depends(collect)):
...
def act(layer: str, action: str, path: str, payload: dict[str, Any], svc: CollectService = Depends(collect),
...
def get(layer: str, path: str, rev: str | None = Query(None), svc: CollectService = Depends(collect)):
    return svc.get(layer, path, rev)


@router.put("/{layer}/{path:path}", response_model=Obj, summary="改一个对象(字段合并;origin 整体替换)")
def update(layer: str, path: str, req: ObjUpdate, svc: CollectService = Depends(collect), c: Ctx = Depends(ctx)):
...
def delete(layer: str, path: str, reason: str = "", svc: CollectService = Depends(collect), c: Ctx = Depends(ctx)):
