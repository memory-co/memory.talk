...
def test_tree_status_canvas_events(client):
    root = client.post("/api/works", json={"goal": "把 v5 做出来"}).json()
    a = client.post("/api/works", json={"goal": "实现 issue", "parent": root["id"]}).json()
    b = client.post("/api/works", json={"goal": "实现 work", "parent": root["id"]}).json()
    ...
    assert client.get("/api/works", params={"root": a["id"]}).json()[0]["children"] == []

    # 父 work 不能先于子 work 做完
    r = client.patch(f"/api/works/{root['id']}", json={"status": "done"})
    assert r.status_code == 409 and a["id"] in r.json()["message"]
    assert client.patch(f"/api/works/{a['id']}", json={"status": "done"}).json()["done_at"]
    assert client.patch(f"/api/works/{b['id']}", json={"status": "abandoned"}).status_code == 200
    assert client.patch(f"/api/works/{root['id']}", json={"status": "done"}).json()["status"] == "done"

    # 结束后不再是干活的地方
    assert client.post(f"/api/works/{a['id']}/sessions", json={"uri": "bash://"}).status_code == 409

    # 画布:视图 + 乐观锁 + 越界
    cv = client.get(f"/api/works/{root['id']}/canvas").json()
    assert cv["version"] == 0 and cv["panels"] == []
    panels = [{"id": "p1", "uri": "file:///w", "x": 0, "y": 0, "w": 12, "h": 16},
              {"id": "p2", "uri": "https://example.com", "x": 12, "y": 0, "w": 12, "h": 16}]
    cv = client.put(f"/api/works/{root['id']}/canvas", json={"version": 0, "panels": panels}).json()
    assert cv["version"] == 1
    assert client.put(f"/api/works/{root['id']}/canvas", json={"version": 0, "panels": []}).status_code == 409
    bad = [{"id": "p1", "uri": "file:///w", "x": 20, "y": 0, "w": 12, "h": 16}]
    assert client.put(f"/api/works/{root['id']}/canvas", json={"version": 1, "panels": bad}).status_code == 409
    ...
    inbox = client.get(f"/api/works/{root['id']}/inbox").json()
    assert inbox[-1]["layer"] == "work" and inbox[-1]["path"] == c["id"] and inbox[-1]["routed_by"] == "parent"
    ...
    assert client.get(f"/api/works/{other['id']}/inbox").json()[-1]["subject"] == "status todo -> doing"
