...
def test_members_visibility(client):
    ...
    m = client.get(f"/api/works/{t['id']}/members").json()
    ...
    # 心跳
    m = client.post(f"/api/works/{t['id']}/members/touch", headers={"X-Memory-Talk-User": "carol"}).json()
    assert m["history"][0]["user"] == "carol" and m["history"][0]["ops"] == 1

    # 过了活跃窗口就不在 current 里(把 last_seen 拨回去)
    import json, os
    p = os.path.join(os.environ["MEMORY_TALK_HOME"], "works", t["id"], "members.json")
    ...
    assert client.get("/api/works/work_nope/members").status_code == 404
