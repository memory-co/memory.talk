"""Collections:层、对象 CRUD、行为(两个相邻提交)、守卫、历史、检索、树、manager 与收件箱、用户层。"""
import subprocess
...
def test_layers_and_objects(client):
    layers = client.get("/api/collections/layers").json()
    ...
    assert client.post("/api/collections/origin/memory.talk/配置/旧方案.md", json={"content": "这是原文"}).status_code == 201
    iss = client.post("/api/collections/issue/memory.talk/配置/该走文件还是环境变量",
    ...
    assert client.post("/api/collections/issue/" + ip, json={"data": {"question": "x"}}).status_code == 409

    tree = {i["name"]: i for i in client.get("/api/collections/tree", params={"path": "memory.talk/配置"}).json()}
    ...
    # 行为:立场、表态、派活、连边
    client.post(f"/api/collections/act/issue/position/{ip}", json={"claim": "加一个 settings.json"})
    client.post(f"/api/collections/act/issue/position/{ip}", json={"claim": "只用环境变量"})
    client.post(f"/api/collections/act/issue/spawn/{ip}", json={"position": "p2", "work_id": "work_try"})
    r = client.post(f"/api/collections/act/issue/argue/{ip}", json={"position": "p2", "stance": 1, "work_id": "work_try",
                                                                  "evidence": {"work_id": "work_try", "rounds": [9]}}).json()
    client.post(f"/api/collections/act/issue/argue/{ip}", json={"position": "p1", "stance": -1, "comment": "多一份状态"})
    ...
    assert client.post(f"/api/collections/act/issue/argue/{ip}", json={"position": "p9", "stance": 1}).status_code == 404
    assert client.post(f"/api/collections/act/issue/nope/{ip}", json={}).status_code == 404

    # decide:两个相邻提交,各在自己的层,同一个 Decision trailer
    cp = "memory.talk/配置/配置只来自环境变量"
    r = client.post(f"/api/collections/act/issue/decide/{ip}", json={"position": "p2", "card": cp, "context": "memory.talk v5",
    ...
    from services.collections import CollectionsError, Ctx
    ...
    # 改卡、历史、读旧版本、删
    client.put(f"/api/collections/card/{cp}", json={"data": {"body": "只用环境变量。配置文件是多出来的一份状态。"}, "reason": "补理由"})
...
def test_manager_and_inbox(client):
    ...
    assert [o["path"] for o in client.get("/api/collections/managed").json()] == [ip]          # 没人管

    # 主题文件夹绑 work;之后这一片(不分层)的变动都进它的收件箱
    m = client.put("/api/collections/manager", params={"path": "memory.talk"}, json={"work": t["id"]}).json()
    assert m == {"dir": "memory.talk", "work": t["id"]}
    assert client.get("/api/collections/manager", params={"path": ip}).json()["work"] == t["id"]
    assert [o["path"] for o in client.get("/api/collections/managed", params={"work": t["id"]}).json()] == [ip]
    assert client.get("/api/collections/managed").json() == []

    client.post(f"/api/collections/act/issue/position/{ip}", json={"claim": "a"}, headers=H)
    client.post("/api/collections/origin/memory.talk/资料.md", json={"content": "x"})
    client.post("/api/collections/card/memory.talk/一张卡", json={"data": {"title": "一张卡"}}, headers={"X-Memory-Talk-Work": t["id"]})
    ...
    client.put("/api/collections/manager", params={"path": ip}, json={"work": t2["id"]})
...
"""
    ...
    assert r.status_code == 201, r.text
    d = client.get("/api/collections/decision/memory.talk/配置/定了用环境变量").json()
